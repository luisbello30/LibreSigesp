...
<?php
	if(array_key_exists("OPERACION",$_POST))
	{
		...
		elseif($operacion="SELEMPRESA")
		{
			...
			$li_pos=0;
			$ls_sql_dir="SELECT * FROM sigesp_config where entry like '%DIRECTOR RRHH%' order by entry";
			$result_dir=$obj_sql->select($ls_sql_dir);
			if($row=$obj_sql->fetch_row($result))
			{
				$la_empresa=$row;   
				...
				$i=0;
				while ($row_dir=$obj_sql->fetch_row($result_dir)){
				  $i++;
				   $_SESSION["director"][$i]=$row_dir["value"];
				 }//var_dump($_SESSION["director"]);die();
