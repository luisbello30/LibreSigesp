...
class sigesp_sno_c_calcularnomina
{
	...
	function uf_obtener_totalpersonal(&$ai_nropro)
	{
		...
		//		   Access: private
		//	    Arguments: ai_nropro  // N�mero de personas a procesar
		//	      Returns: lb_valido True si se ejecut� con �xito el select y false si hubo ag�n error
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_obtener_totalpersonal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if(!$rs_data->EOF)
	...
	function uf_obtener_resumenpago($as_peractnom,&$ai_totasi,&$ai_totded,&$ai_totapoemp,&$ai_totapopat,&$ai_totnom,&$ai_nropro)
	{
		...
		//	  Description: Funcion que obtiene el la suma de todo lo que se pago en la n�mina 
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_obtener_resumenpago ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if(!$rs_data->EOF)
	...
	function uf_existesalida()
	{
		...
		{
			$lb_valido=true;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_existesalida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			if(!$rs_data->EOF)
	...
	{	
		...
		//	      Returns: lb_valido True si se proceso correctamente � False si hubo error 
		//	  Description: funci�n que selecciona el personal y procesa el calculo de la n�mina
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 16/02/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{ 
			...
			if ($ai_totalperpro!=0)
			{
				...
				if ($li!=0)
				{
					if ($li>10)
					{
						$ls_mensaje='Existen '.$li.' Personas que no posee Proyectos Asociados en este N�mina \n';
		...
		{   
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_procesarnomina ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}
			else
			{
				...
				{
					$this->io_sql->commit();
					$this->io_mensajes->message("El c�lculo de la n�mina fue procesado.");
				}
				else
				{
					$this->io_sql->rollback();
					$this->io_mensajes->message("Ocurrio un error al calcular la n�mina.");
				}
			}
	...
	{	
		...
		//	               ad_totnom  // Total acumulado de la n�mina
		//	      Returns: lb_valido  True si se calcul� la n�mina completa � False si no se calcul� completa
	...
	function uf_procesar_nomina_personal($as_codper,&$ad_totnom)
	{
		...
		if($lb_valido)
		{
			...
			$lb_cuotact=$this->uf_select_pericuota_canc_vacac($as_codper);
		...
		if($ld_asires<($ld_dedres+$ld_apoempres))
		{
			...
			{
				$this->io_mensajes->message("Se ha detectado que la persona C�digo ".$as_codper." Nombre ".$ls_nombre."  posee Deducciones mayores a las Asignaciones.");
				$lb_valido=false;
			}
		}
		if ($lb_valido)
	...
	function uf_select_persona_prestamo_desincorpora($as_codper,$ad_desde_r,$ad_hasta_r)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_select_persona_prestamo_desincorpora ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_select_fecha_finvacper($as_codper,$ad_desde_r,$ad_hasta_r)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_select_fecha_finvacper ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_insert_resumen($as_codper)
	{
		...
		//		   Access: private
		//	    Arguments: as_codper  // C�digo del Personal
		//	      Returns: lb_valido  True si inserta correctamenta en la tabla  � False si hubo error
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_insert_resumen ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
	   return $lb_valido;	
	 }
	//-----------------------------------------------------------------------------------------------------------------------------------		
	                                     ...
	                                     &$ad_monnetres,&$ad_totnom)
	{
		...
		//		   Access: private
		//	    Arguments: as_codper // c�digo de personal
		//                 ad_asires // asignaci�n del resumen 
		//                 ad_dedres //  deducciones  del resumen 
		//                 ad_apoempres // aporte del empleado
		//                 ad_apopatres // aporte del patr�n
		//                 ad_priquires // monto primera quincena
		//                 ad_segquires // monto segunda quincena
		//                 ad_monnetres // monto neto del per�odo
		//                 ad_totnom  //   total de la nomina
		//	      Returns: lb_valido True si se evaluaron los conceptos � False si hubo error
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_evaluar_conceptopersonal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_i=1;
	...
	function uf_guardar_salida($as_codper,$as_codconc,$as_tipsal,$ad_valsal,$ad_monacusal,$as_quirepcon)
	{
		...
		//	      Returns: lb_valido True si se inserto correctamente � False si hubo error
		...
		if($li_row===false)
		{
			...
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_guardar_salida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
                //var_dump($lb_valido);
		return $lb_valido; 
	}
	                                     ...
	                                     $ad_monnetres)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_update_resumen_acumulado ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
	   return $lb_valido;	
	 }
    //-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_calcular_sueldo_promedio($as_codper)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_calcular_sueldo_promedio ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_update_personalnomina($as_codper,$ad_sueproper)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_update_personalnomina ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;	
	 }
    //-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_delete_final_resumen()
	{
		...
		//	  Description: Funcion que elimina en la tabla  sno_resumen aquellos registros que no est�n en salida
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		    
		$lb_valido=true;
        ...
        {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_delete_final_resumen ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	   	}
		return $lb_valido;
    }
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_update_periodos($aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_update_periodos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			...
			if($li_row===false)
		   	...
		   	else
		   	{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="PROCESS";
				$ls_descripcion="Calcul� la n�mina ".$this->ls_codnom." para el per�odo ".$this->ls_peractnom." ".
								"Total a Asignaci�n ".$li_totasi.", Total Deducci�n ".$li_totded.", ".
								"Total a Aporte Empleado ".$li_totapoemp.", Total Aporte Patr�n ".$li_totapopat.", ".
								"Total N�mina ".$li_totnom;
	...
	function uf_generar_rep_vacaciones()
	{
		...
		//	     Function: uf_generar_rep_vacaciones
		//		   Access: private
		//	      Returns: lb_valido True si se genero el reporte de  vacaciones  � False si no se genero correctamente
		...
		if(($lb_valido)&&($lb_ok))
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_generar_rep_vacaciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				while($row=$this->io_sql->fetch_row($rs_data))
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------	
	...
	function uf_select_vac_config(&$as_codvac)
	{
		...
		//	      Returns: lb_valido True si se evaluo la vacaciones  � False si no se evaluo en el sigesp_config
		//	  Description: Funcion que devuelve de la tabla sigesp config los valores y el codigo de la vacaciones
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_delete_salidas_vac($as_codvac)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_delete_salidas_vac ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;
    }
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_count_resumen($as_codper)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_count_resumen ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_insert_resumen_vac($as_codper,$ad_totneto)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_insert_resumen_vac ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;	
	 }
	//-----------------------------------------------------------------------------------------------------------------------------------		
	...
	function uf_obtener_totalpersonalproyecto(&$as_totalperpro)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_obtener_totalpersonalproyecto ERROR->".
    ...
    function uf_obtener_informacionpersonalpro(&$as_mensaje,&$li)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_obtener_informacionpersonalpro ERROR->".
			                            $this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			while((!$rs_data->EOF))
			{
				...
				$as_mensaje = $as_mensaje.' C�digo: '.$ls_codper.'  -  '.$ls_apeper.', '.$ls_nomper.'\n';
				$rs_data->MoveNext();
			}
			$this->io_sql->free_result($rs_data);		
		}
	...
	function uf_procesar_encargaduria()
	{
		...
		//		   Access: private
		//	      Returns: lb_valido 
		//	  Description: Funcion realizar el c�culo del pago por encargadur�a
		//	   Creado Por: Ing. Mar�a Beatriz Unda
		// Fecha Creaci�n: 31/12/2008 								Fecha �ltima Modificaci�n : 		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ld_fecdesper=$_SESSION["la_nomina"]["fecdesper"];
		$ld_fechasper=$_SESSION["la_nomina"]["fechasper"];
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_procesar_encargaduria ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			while((!$rs_data->EOF)&&($lb_valido))
			{
				...
				// Para calcular la diferencia a pagar por conceptos de encargadur�a
				...
				else
				{
					...
					//Se busca el c�digo del concepto de resumen de encargadur�a
					...
					{
						$lb_valido=false;
						$this->io_mensajes->message("No existe concepto para el Resumen de la Encargadur�a. No se puede procesar el c�lculo de la N�mina.");
					}
				}
				$rs_data->MoveNext();
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------	
	...
	function uf_buscar_concepto_resumen_encargaduria(&$as_codconc)
	{
		...
		//		   Access: private
		//	      Returns: lb_valido 
		//	  Description: Funcion busca el c�digo del concepto tildado como resumen de encargadur�a
		//	   Creado Por: Ing. Mar�a Beatriz Unda
		// Fecha Creaci�n: 31/12/2008 								Fecha �ltima Modificaci�n : 		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;	
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_buscar_concepto_resumen_encargaduria ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_actualizar_resumen_personal_encargado($as_codper, $ad_monto)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_actualizar_resumen_personal_encargado ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if((!$rs_data->EOF)&&($lb_valido))
			{
				...
				{
					$lb_valido=false;
					$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_actualizar_resumen_personal_encargado ERROR->".
			                                    $this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				}				
			} 
		} 
	...
	function uf_update_salida($as_codper,$as_codconc,$as_tipsal,$ad_valsal,$ad_monacusal,$ad_salsal,$as_quirepcon)
	{
	   ...
	   {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Calcular N�mina M�TODO->uf_update_salida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	   }
	   return $lb_valido;	
...
//-----------------------------------------------------------------------------------------------------------------------------------
function uf_select_pericuota_canc_vacac($as_codper)
{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    /*$ls_sql="SELECT * FROM sno_prestamosperiodo
	    ...
	    $ls_sql="SELECT * FROM sno_prestamosperiodo a, sno_prestamos b
				WHERE a.codnom='".$this->ls_codnom."'
				AND b.codnom='".$this->ls_codnom."'
				AND a.numpre=b.numpre
				AND b.stapre='1'
				AND a.feciniper BETWEEN '".$ld_fecdesper."' AND '".$ld_fechasper."'
				AND a.codper='".$as_codper."'
				AND a.percob='".$ls_peractnom."'
				AND a.estcuo='1'; ";
		//print "<br>".$ls_sql."<br>";
		//break;
		$rs_data=$this->io_sql->select($ls_sql);
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_pericuota_canc_vacac ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data)>0)
