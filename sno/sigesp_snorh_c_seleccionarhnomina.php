...
class sigesp_snorh_c_seleccionarhnomina
{
	...
	{		
		...
		//				   ai_anocurnom  // a�o en curso de la n�mina
		...
		//	  Description: Funcion que elimina de las tablas temporales los registros que existen e inserta los de la n�mina, a�o y 
		...
		{
			$lb_valido=false;
        	$this->io_mensajes->message("Ocurrio un error al procesar la Data Hist�rica."); 
	...
	function uf_delete_temporal($as_codnom)
	{
		...
		//	  Description: Funcion que elimina todos los registros del personal con el viejo c�digo
		//	   Creado Por: Ing. Yesenia Moreno
		...
		if($lb_valido)
		{// eliminamos el temporal de asignaci�n de cargo
			$ls_sql="DELETE ".
		...
		if($lb_valido)
		{// eliminamos el temporal de concepto vacaci�n
			$ls_sql="DELETE ".
		...
		if($lb_valido)
		{// eliminamos el temporal de n�mina
			$ls_sql="DELETE ".
					"  FROM sno_thnomina ".
					" WHERE codemp='".$this->ls_codemp."'".
					"   AND codnom='".$as_codnom."'";
		...
		if($lb_valido)
		{// eliminamos el temporal de per�odo
			$ls_sql="DELETE ".
					"  FROM sno_thperiodo ".
					" WHERE codemp='".$this->ls_codemp."'".
					"   AND codnom='".$as_codnom."'";
		...
		if($lb_valido)
		{// eliminamos el temporal de personal n�mina
			$ls_sql="DELETE ".
		...
		if($lb_valido)
		{// eliminamos el temporal de pren�mina
			$ls_sql="DELETE ".
		...
		if($lb_valido)
		{// eliminamos el temporal de prestamos per�odo
			$ls_sql="DELETE ".
		...
		if($lb_valido)
		{// eliminamos el temporal de subn�mina
			$ls_sql="DELETE ".
		...
		if($lb_valido)
		{// eliminamos el temporal de Clasificaci�n de Obreros
			$ls_sql="DELETE ".
	...
	function uf_insert_temporal($as_codnom,$ai_anocurnom,$as_peractnom)
	{
		...
		$lb_valido=true;
		if($lb_valido)
		{// insertamos el temporal de asignaci�n de cargo
		...
		{// insertamos el temporal de concepto
			$ls_sql="INSERT INTO sno_thconcepto (codemp, codnom, anocur, codperi, codconc, nomcon, titcon, sigcon, forcon, glocon, ".
					...
					"			 consunicon, quirepcon, asifidper, asifidpat, frevarcon, persalnor,aplresenc,conperenc, codente,guarrepcon,aplidiasadd ".
					"       FROM sno_hconcepto ".
					"      WHERE codemp='".$this->ls_codemp."'".
					"        AND codnom='".$as_codnom."'".
					"        AND anocur='".$ai_anocurnom."'".
		...
		if($lb_valido)
		{// insertamos el temporal de concepto vacaci�n
		...
		if($lb_valido)
		{// insertamos el temporal de n�mina
			...
			$lb_valido=$this->uf_procesar_sql($ls_sql);
               ...
               // var_dump($lb_valido);die();
		if($lb_valido)
		{// insertamos el temporal de per�odo
		...
		if($lb_valido)
		{// insertamos el temporal de personal n�mina
		...
		if($lb_valido)
		{// insertamos el temporal de Personal Pensi�n
		...
		if($lb_valido)
		{// insertamos el temporal de pren�mina
		...
		if($lb_valido)
		{// insertamos el temporal de prestamos per�odo
		...
		if($lb_valido)
		{// insertamos el temporal de subn�mina
	...
	function uf_procesar_sql($as_sql)
	{
		...
		{
 			$lb_valido=false;
        	$this->io_mensajes->message("CLASE->Selecionar hnomina M�TODO->uf_procesar_sql ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		return $lb_valido;
