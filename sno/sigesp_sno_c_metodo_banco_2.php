...
//		$ls_nombrearchivo=$as_ruta."/nomina.txt";
		...
		if($li_count>0)
		{
//crea un archivo por cada 200 personas
		for( $personas=0;$personas<=$li_count;$personas=$personas+200)
		{
			$num_nombre++;
			$ls_nombrearchivo=$as_ruta."/nomina".$num_nombre.".txt";
			//Chequea si existe el archivo.
			if (file_exists("$ls_nombrearchivo"))
			{
				//$ls_creararchivo=@fopen("$ls_nombrearchivo","a+");
				$ls_creararchivo="ls_creararchivo".$num_nombre;
				$$ls_creararchivo=@fopen("$ls_nombrearchivo","a+");
				$lb_adicionado=true;
			}
			else
...
//				if ($ls_creararchivo)
				...
				$rs_data->MoveNext();		
				//contador para saber cuando ya se han procesado 200 personas
				$personas++;
			}
			//*-Si estoy acumulando reemplazo la cabecera con la informacion acumulada
			if (($lb_valido)&&($lb_adicionado))
