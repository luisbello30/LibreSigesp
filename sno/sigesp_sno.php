...
class sigesp_sno
{
	...
	{	
		...
		//	      Returns: lb_valido True si se cumple la condici�n de fin de mes � False si no se cumple
		//	  Description: funci�n que dada una fecha de inicio y una fecha fin se verifica si es fin de mes
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
	...
	{	
		...
		//	  Description: funci�n que dada una fecha de inicio y una fecha fin se cuentan cuantos lunes tienen
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$li_valor=0;
	...
	{	
		...
		//	    Arguments: ad_fecdes // Fecha Desde
		//				   ad_fechas // fecha Hasta
		//				   ai_dia // d�a de la semana 6->s�bado y 0->Domingo
		//	      Returns: $li_valor cantidad de s�bados y domingos del rango de fecha
		//	  Description: funci�n que dada una fecha de inicio y una fecha fin se cuentan cuantos s�bados y domingos tienen
		//	   Creado Por: Ing. Yesenia Moreno
	...
	function uf_periodo_previo(&$ai_anoprev,&$ai_periprev)
    {
		...
		//	  Description: funci�n que devuelve el per�odo previo de la n�mina
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 01/01/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($ai_periprev<1)
		{
			...
			if($rs_data===false)
			{
				$this->io_mensajes->message("CLASE->SNO M�TODO->uf_periodo_previo ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				$lb_valido=false;
			}
			else
			{
	...
	function uf_crear_sessionnomina()
    {
		...
		//	  Description: Funci�n que crea la sessi�n de la n�mina actual
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 20/02/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_crear_sessionnomina ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ls_desper=" A�o <strong>".$_SESSION["la_nomina"]["anocurnom"]."</strong> Per�odo <strong>".$_SESSION["la_nomina"]["peractnom"]."</strong> ".$ld_fecdesper." - ".$ld_fechasper."";
			...
			else
			{
				unset($_SESSION["la_nomina"]);
				$lb_valido=false;
				$this->io_mensajes->message("Favor verifique los datos de la n�mina. No se pueden cargar los datos."); 
	...
	function uf_crear_sessionhnomina()
    {
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_crear_sessionhnomina ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ld_fecdesper=$this->io_funciones->uf_convertirfecmostrar($ld_fecdesper);
				$ld_fechasper=$this->io_funciones->uf_convertirfecmostrar($ld_fechasper);
				$ls_desper="<strong>Hist�rico</strong> - A�o <strong>".$_SESSION["la_nomina"]["anocurnom"]."</strong> Per�odo <strong>".$_SESSION["la_nomina"]["peractnom"]."</strong> ".$ld_fecdesper." - ".$ld_fechasper."";
				$_SESSION["la_nomina"]["descripcionperiodo"]=$ls_desper;
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("Favor verifique los datos de los hist�ricos. No se pueden cargar los datos."); 
				print "<script language=JavaScript>";
				print "location.href='sigespwindow_blank.php'";
				print "</script>";		
			}
	...
	function uf_guardar_configuracion($ai_vac_reportar,$as_vac_codconvac,$as_vac_metvac,$as_est_codconcsue,$as_est_estnom,$as_est_ordcons,
									  ...
									  $aa_seguridad,$ls_director,$ls_ceddirector,$ls_telfdirector,$ls_cordirector,$ls_cargodirector,$ls_tipperrif,$ls_numpririf, $ls_numterrif,$as_con_congua)
    {
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar_configuracion
		//		   Access: public (sigesp_snorh_p_configuracion.php)
		//	    Arguments: ai_vac_reportar // reportar vacaciones             as_vac_codconvac // c�digo de concepto de vacaciones
		//				   as_vac_metvac // m�todo de vacaciones              as_est_codconcsue // c�digo de concepto de sueldo
		//				   as_est_estnom // estilo de la n�mina				  as_est_ordcons // ordenar las constantes
		//				   as_est_ordconc // ordenar los conceptos            as_est_estrec // estilo de la n�mina  
		//				   ai_est_numlin // n�mero de l�neas del recibo       as_est_prilpt // reporte 
		//				   as_est_agrsem // agrupaci�n de semana              ai_con_parnom // par�metros por n�mina
		//				   as_con_consue // contabilizaci�n de los sueldos	  as_con_cuecon // cuenta contable 
		//				   as_con_conapo // contabilizaci�n de aportes        as_con_conpro // contabilizaci�n de program�tica 
		//				   ai_con_agrcon // agrupar contable   				  ai_con_gennotdeb // generar nota de d�bito
		//				   ai_con_genvou // generar voucher a nota de d�bito  as_con_descon // contabilizaci�n de program�tica 
		//				   ai_par_excpersus // excluir personas suspendidas   ai_par_perrep // no permitir repetidos
		//				   ad_par_fecfinano // fecha tope de bono de fin a�o  as_par_metcalfid // m�todo de fideicomiso
		//				   as_fpj_codorgfpj // c�digo de organismo            as_fpj_codconcfpj // c�digo de concepto 
		//				   as_fpj_metfpj // m�todo de fpj                     as_lph_codconclph // c�digo de concepto 
		//				   as_lph_metlph // m�todo de lph 					  as_fpa_codconcfpa // c�digo de concepto
		//				   as_fpa_metfpa // m�todo de fpa                     ai_fps_antcom // Antiguedad complementaria 
		//				   ai_fps_fraali // Fracci�n de Alicuota              as_fps_metfps // m�todo de fps 
		//				   as_man_cueconc // cuentas de conceptos   		  as_man_cueconccaj // cuentas de conceptos caja
		//				   ai_man_actblofor // bloqueo formulas conceptos 	  ai_man_actblocalnom // activar bloqueo de c�lculo de la n�mina 
		//				   as_man_metrescon // m�todo de resumen contable     as_dis_metdisnom // m�todo de disco de n�mina
		//				   ai_con_genrecdoc // generar recepci�n documento    ai_con_genrecdocapo // Generar recepci�n de documento aportes
		//				   as_con_tipdocnom // Tipo de documento de nom       as_con_tipdocapo // tipo de documento de aporte
		//				   as_ipas_codorgipas // c�digo de Organismo IPASME   as_ipas_codconcahoipas // C�digo Concepto Ahorro IPASME
		//				   as_ipas_codconcseripas //  C�digo Concepto Servicio Asistencial IPASME
		//				   as_ipas_conhipespipas //  C�digo Concepto Hipotecario Especial IPASME
		//				   as_ipas_conhipampipas //  C�digo Concepto Hipotecario Ampliaci�n IPASME
		//				   as_ipas_conhipconipas //  C�digo Concepto Hipotecario Construcci�n IPASME
		//				   as_ipas_conhiphipipas //  C�digo Concepto Hipotecario Hipoteca IPASME
		//				   as_ipas_conhiplphipas //  C�digo Concepto Hipotecario LPH IPASME
		//				   as_ipas_conhipvivipas //  C�digo Concepto Hipotecario Vivienda IPASME
		//				   as_ipas_conperipas //  C�digo Concepto Personal IPASME
		//				   as_ipas_conturipas //  C�digo Concepto Turisticos IPASME
		//				   as_ipas_conproipas //  C�digo Concepto Proveduria IPASME
		//				   as_ipas_conasiipas //  C�digo Concepto Asistenciales IPASME
		//				   as_ipas_convehipas //  C�digo Concepto Vehiculo IPASME
		//				   as_ipas_concomipas //  C�digo Concepto Comerciales IPASME	
		//				   as_ivss_numemp	 // C�digo de empresa que asigna el  IVSS
		//				   ai_vac_desincorporar	 // Si se desincorpora cuando el personal este de vacaciones 
		//				   as_par_concsuelant // C�digo de concepto de sueldo anterior
		...
		//				   as_par_codunirac // C�digo �nico de RAC
		//				   as_par_comautrac // Compensaci�n autom�tica de RAC
		...
		//				   ai_par_alfnumcodper // Permitir alfanumericos en el c�digo de personal
		//                 ai_con_parfpj  // configuraci�n de FPJ
		...
		//	  Description: Funci�n que graba todos los campos de la configuraci�n
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 04/02/2006 								Fecha �ltima Modificaci�n : 20/10/2008
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($lb_valido)
		{// generar voucher a nota de d�bito
		...
		{// contabilizaci�n de program�tica
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","CONTABILIZACION FIDEICOMISO",$as_con_confidnom,"C");
                ...
                if($lb_valido)
		{// contabilizaci�n de program�tica
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","CONTABILIZACION GUARDERIA",$as_con_congua,"C");
		}
		if($lb_valido)
		{// contabilizaci�n de program�tica
		...
		if($lb_valido)
		{// Configuraci�n de generar recepci�n de documento para el pago del personal
			$lb_valido=$this->uf_insert_config("SNO","CONFIG","GENERAR RECEPCION DOCUMENTO PAGO PERSONAL CHEQUE",$ai_genrecdocpagperche,"I");
		}
		if($lb_valido)
		{// Tipo de Documento de la Recepci�n de Documento del pago del personal
		...
		if($lb_valido)
		{// Configuraci�n del validacion de lontitud de cuenta de banco
		...
		if($lb_valido)
		{// Configuraci�n del validacion de prestamos al personal del mismo tipo
			$lb_valido=$this->uf_insert_config("SNO","CONFIG","VAL_TIPO_PRESTAMO",$ai_prestamo,"I");
		}
		if($lb_valido)
		{// Configuraci�n de si las n�minas est�n por rac que se pueda cambiar el sueldo
			$lb_valido=$this->uf_insert_config("SNO","CONFIG","CAMBIAR_SUELDO_RAC",$la_par_campsuerac,"I");
		}
		if($lb_valido)
		{// Configuraci�n de si las n�minas est�n por rac que se pueda cambiar la dedicaci�n y el tipo de personal
			$lb_valido=$this->uf_insert_config("SNO","CONFIG","CAMBIAR_DEDICACION_TIPO_PERSONAL_RAC",$ai_par_camdedtipper,"I");
		}
		if($lb_valido)
		{// Configuraci�n de permiritir sobregiro en las cuentas del trabajador al calcular la n�mina
		...
		if($lb_valido)
		{// activar bloqueo de c�lculo de la n�mina
			$lb_valido=$this->uf_insert_config("SNO","CONFIG","BLOQUEO_ACTIVAR",$ai_man_actblocalnom,"I");
		}
		if($lb_valido)
		{// m�todo de resumen contable
			$lb_valido=$this->uf_insert_config("SNO","CONFIG","METODO RESUMEN CONTABLE",$as_man_metrescon,"C");
		}
		//-----------------------------------------------------------------------------------------------------

		//-------------------------------------DIRECTOR RRHH---------------------------------------------------
		$rif=$ls_tipperrif."-".$ls_numpririf."-".$ls_numterrif;
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","NOMBRE DIRECTOR RRHH",$ls_director,"C");
		}
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","TELEFONO DIRECTOR RRHH",$ls_telfdirector,"C");
		}
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","CEDULA DIRECTOR RRHH",$ls_ceddirector,"C");
		}
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","CORREO DIRECTOR RRHH",$ls_cordirector,"C");
		}
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","CARGO DIRECTOR RRHH",$ls_cargodirector,"C");
		}
		if($lb_valido)
		{
			$lb_valido=$this->uf_insert_config("SNO","NOMINA","RIF DIRECTOR RRHH",$rif,"C");
		}
		//-----------------------------------------------------------------------------------------------------
		...
		{ 
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz� la configuraci�n de las n�mina.";
	...
	function uf_reparar_subnominas($aa_seguridad)
    {
		...
		//	      Returns: lb_valido True si se ejecuto correctamente la funci�n y false si hubo error
		//	  Description: Funci�n que le inserta subn�minas a todas aquellas n�minas que no tengan sub n�minas asociadas
		//	   Creado Por: Ing. Yesenia Moreno
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_reparar_subnominas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$this->io_sql->rollback();
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion ="Se realiz� el proceso de reparar subn�minas.";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////				
			if($lb_valido)
			{	
				$this->io_mensajes->message("El proceso de reparar subn�minas se realiz� correctamente.");
				$this->io_sql->commit();
			}
			else
			{
	...
	function uf_reparar_conceptopersonal($aa_seguridad)
    {
		...
		//	    Arguments: aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto correctamente la funci�n y false si hubo error
		//	  Description: Funci�n que le inserta los conceptos y constantes a personal que no se le haya asociado alguno
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 05/04/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_reparar_conceptopersonal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$this->io_sql->rollback();
		}
		else
		{
			...
			else
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="PROCESS";
				$ls_descripcion ="Se realiz� el proceso de reparar concepto-personal.";
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////				
				if($lb_valido)
				{	
					$this->io_mensajes->message("El proceso de reparar concepto-personal se realiz� correctamente.");
					$this->io_sql->commit();
				}
				else
				{
	...
	function uf_recalcular_sueldointegral($aa_seguridad)
    {
		...
		//	    Arguments: aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto correctamente la funci�n y false si hubo error
		//	  Description: Funci�n que le actualiza el sueldo integral del personal en las tablas de hist�ricos
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 05/04/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_recalcular_sueldointegral ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$this->io_sql->rollback();
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion ="Se realiz� el proceso de recalcular sueldo integral.";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////				
			if($lb_valido)
			{	
				$this->io_mensajes->message("El proceso de recalcular sueldo se realiz� correctamente.");
				$this->io_sql->commit();
			}
			else
			{
	...
	function uf_mantenimiento_historicos($aa_seguridad)
    {
		...
		//	    Arguments: aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto correctamente la funci�n y false si hubo error
		//	  Description: Funci�n que elimina de la tabla de resumen hist�rico los registros que no est�n en la tabla de salida
		//				   hist�ricos
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 05/04/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_mantenimiento_historicos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$this->io_sql->rollback();
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion ="Se realiz� el proceso de mantenimiento hist�ricos.";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////				
			if($lb_valido)
			{	
				$this->io_mensajes->message("El proceso de mantenimiento hist�ricos se realiz� correctamente.");
				$this->io_sql->commit();
			}
			else
			{
	...
	function uf_mantenimiento_repararacumuladoconceptos($aa_seguridad)
    {
		...
		//	    Arguments: aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto correctamente la funci�n y false si hubo error
		//	  Description: Funci�n que actualiza el valor acumulado de los conceptos
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 05/04/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_mantenimiento_repararacumuladoconceptos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$this->io_sql->begin_transaction();
			if($lb_valido)
			{
				...
				{
					$lb_valido=false;
					$this->io_mensajes->message("CLASE->SNO M�TODO->uf_mantenimiento_repararacumuladoconceptos 2 ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
				}
			}
			if($lb_valido)
			{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion ="Se realiz� el proceso de Reparar Acumulado de Conceptos.";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////				
			if($lb_valido)
			{	
				$this->io_mensajes->message("El proceso de Reparar Acumulado de Conceptos se realiz� correctamente.");
				$this->io_sql->commit();
			}
			else
			{
	...
	//------------------------------------------------------------------------------------------------------------------------------------
   	...
   	{ 	
		...
		//	    Arguments:
		//	      Returns: lb_valido True si se ejecuto correctamente la funci�n y false si hubo error
		//	  Description: Funci�n que busca el numero de seguro social en sigesp_empresa
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_numero_IVSS ERROR->".
	...
	function uf_crear_sessionnomina_encargaduria($as_codnom)
    {
		...
		// Fecha Creaci�n: 26/12/2008 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		if($as_codnom!="")
		{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->SNO M�TODO->uf_crear_sessionnomina_encargaduria ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("Favor verifique los datos de la n�mina de encargadur�a. No se pueden cargar los datos."); 
				print "<script language=JavaScript>";
				print "location.href='sigespwindow_blank.php'";
				print "</script>";		
			}
	...
	function uf_buscar_personal($as_codnom,$codperi,&$as_valor,$ai_inicio,$ai_registros,&$ai_totpag, &$as_codperdes, &$as_codperhas,$as_codban)
	{
		...
		//    Description: 
		//	   Creado Por: Ing. Mar�a Beatriz Unda
		// Fecha Creaci�n: 08/01/2009								Fecha �ltima Modificaci�n :  		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_orden="";
		$ls_pag="";			
