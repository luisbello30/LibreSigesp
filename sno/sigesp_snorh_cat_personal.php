	...
	function uf_print($as_codper, $as_cedper, $as_nomper, $as_apeper, $as_codnom, $as_tipo)
   	{
		...
		//				   as_cedper  // Cï¿½dula de Pesonal
		//				   as_nomper  // Nombre de Personal
		//				   as_apeper // Apellido de Personal
		//				   as_codnom // cï¿½digo de nï¿½mina a la que pertenece
		//				   as_tipo  // Tipo de Llamada del catï¿½logo
		...
		else
		{
			while(!$rs_data->EOF)
			{
				...
				$ls_numpririf = substr($ls_rifper,2,8);//Nï¿½mero Principal del RIF, 8 Dï¿½gitos (0-9).
				$ls_numterrif = substr($ls_rifper,11,1);//Nï¿½mero Terminal  del RIF, 1 Dï¿½gitos (0-9).
				 ...
				 anoperobr, carantper,tipperrif,numpririf,numterrif)
{
	...
	opener.document.form1.btnnominas.disabled=false;
	opener.document.form1.btnpernomina.disabled=false;
	...
	if(opener.document.form1.btnguarderia!=null){opener.document.form1.btnguarderia.disabled=false;}
