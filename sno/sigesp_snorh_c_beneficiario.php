...
class sigesp_snorh_c_beneficiario
{
	...
	{	
		...
		// Fecha Creaci�n: 07/11/2007 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_select_beneficiario($as_codper,$as_codben,$as_tipben)
	{
		...
		if($rs_data===false)
		{
        	$this->io_mensajes->message("CLASE->Beneficiario M�TODO->uf_select_beneficiario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
									...
									$as_nomcheben,$ai_porpagben,$ai_monpagben,$as_codban,$as_ctaban,$as_forpagben,$as_nacben,$as_tipcueben,$as_nexben,$as_cedaut,$as_numexpben,$aa_seguridad)
	{
		...
		{
 			$lb_valido=false;
        	$this->io_mensajes->message("CLASE->Beneficiario M�TODO->uf_insert_beneficiario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
									...
									$as_nomcheben,$ai_porpagben,$ai_monpagben,$as_codban,$as_ctaban,$as_forpagben,$as_nacben,$as_tipcueben,$as_nexben,$as_cedaut, $as_numexpben,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
        	$this->io_mensajes->message("CLASE->Beneficiario M�TODO->uf_update_beneficiario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
	...
	{	
		...
		//                 as_nexben  // parentesco del beneficiario con el trabajador
		//				   as_cedaut  // cedula del autorizado
		//                 as_numexpben // n�mero del expediente del beneficiario
		//				   aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto el update � False si hubo error en el update
		//	  Description: Funcion que actualiza el beneficiario
		//	   Creado Por: Ing. Yesenia Moreno
	...
	function uf_delete_beneficiario($as_codper,$as_codben,$as_tipben,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
        	$this->io_mensajes->message("CLASE->beneficiario M�TODO->uf_delete_beneficiario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
	...
	function uf_select_sueldo_beneficiario($as_cedben)
	{
		...
		//		   Access: private
		//	    Arguments: as_cedben  // C�dula del Beneficiario		
		// 	      Returns: lb_existe True si existe � False si no existe
		//	  Description: Funcion que devuelve el sueldo del beneficiario
		//	   Creado Por: Ing. Mar�a Beatriz Unda
		// Fecha Creaci�n: 16/10/2008				Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$li_sueldo=0;
		...
		if($rs_data===false)
		{
        	$this->io_mensajes->message("CLASE->Beneficiario M�TODO->uf_select_sueldo_beneficiario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_porcentaje_beneficiario($as_cedben)
	{
		...
		$ls_sql="SELECT porpagben ".
		        "  FROM sno_beneficiario ".
				" WHERE codemp='".$this->ls_codemp."'".				
				"   AND cedaut='".$as_cedben."'";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
        	$this->io_mensajes->message("CLASE->Beneficiario M�TODO->uf_select_porcentaje_beneficiario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
