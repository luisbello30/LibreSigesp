...
class sigesp_sno_c_prestamocuotas
{
							 ...
							 $aa_seguridad)
	{
		...
		//				    ai_numcuo  // N�mero de Cuota
		//				    as_percob  // Per�odo donde se va a cobrar la cuota
		//				    ad_feciniper  // Fecha Inicio del Per�odo donde se va a cobrar la cuota
		//				    ad_fecfinper  // Fecha Fin del Per�odo donde se va a cobrar la cuota
		...
		{
 			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_insert_cuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		...
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la cuota ".$ai_numcuo." del Prestamo nro ".$ai_numpre." de tipo ".$as_codtippre." del personal ".
    ...
    function uf_delete_cuota($as_codper,$as_codtippre,$ai_numpre,$ai_numcuo,$as_tipo,$aa_seguridad)
    {
		...
		//				   as_codtippre  // C�digo del tipo de Prestamo
		//				   ai_numpre  // N�mero Correlativo del Prestamo
		//				   ai_numcuo  // N�mero de la cuota
		//				   as_tipo  // Tipo de eliminaci�n de las cuotas si son todas las cuotas � solo las no canceladas
		//				   aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si realiz� el delete � False si no lo elimin�
		//	  Description: Funci�n que elimina las cuotas asociadas a un prestamo
		//	   Creado Por: Ing. Yesenia Moreno
		...
		if($as_tipo=="1")//Se eliminan solo la cuota actual
		{
       	...
       	if ($li_row===false)
       	{
        	$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_delete_cuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
       	}
       	else
       	{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				...
				if ($as_tipo=="1")//Se eliminan solo la cuota actual
				{
					$ls_descripcion ="Elimin� la cuota ".$ai_numcuo." No cancelada del prestamo ".$ai_numpre." de tipo ".$as_codtippre." del personal ".
									 "".$as_codper." asociado a la n�mina ".$this->ls_codnom;
				}
				else //Se eliminan todas las cuotas
				{
					$ls_descripcion ="Elimin� las cuotas No canceladas del prestamo ".$ai_numpre." de tipo ".$as_codtippre." del personal ".
									 "".$as_codper." asociado a la n�mina ".$this->ls_codnom;
				}
	...
	function uf_obtener_cuota($as_codper,$as_codtippre,$ai_numpre,$as_tipo,&$as_percob,&$ad_feciniper,&$ad_fecfinper,&$ai_numcuo)
	{
		...
		//				   as_codtippre  // C�digo del tipo de Prestamo
		//				   ai_numpre  // N�mero Correlativo del Prestamo
		//				   as_tipo  // Tipo de Llamada al m�todo
		//	    		   as_percob  // Periodo a cobrar
		//	    		   ad_feciniper  // Fecha de Inicio del Per�odo
		//	    		   ad_fecfinper  // Fecha Fin del Periodo
		//	    		   ai_numcuo  // N�mero de Cuota
		//	      Returns: lb_valido True si se obtiene el n�mero de cuota sin problema False si hubo error
		//	  Description: Funcion que obtiene el n�mero de cuota dado el periodo y las fechas desde y hasta
		//	   Creado Por: Ing. Yesenia Moreno
		...
		switch($as_tipo)
		{
			...
			case "2": // se filtra por n�mero de cuota
				$ls_sql="SELECT numcuo, percob, feciniper, fecfinper ".
						"  FROM sno_prestamosperiodo ".
						" WHERE codemp='".$this->ls_codemp."'".
						"   AND codnom='".$this->ls_codnom."'".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_obtener_cuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_obtener_ultimacuota($as_codper,$as_codtippre,$ai_numpre,&$ai_numcuo,&$as_percob,&$ad_feciniper,&$ad_fecfinper)
	{
		...
		//	    		   ad_feciniper  // Fecha de Inicio del Per�odo
		//	    		   ad_fecfinper  // Fecha Fin del Periodo
		//	      Returns: lb_valido True si se obtiene la �ltima cuota del prestamo sin problema False si hubo error
		//	  Description: Funcion que obtiene la informaci�n de la �ltima del prestamo
		//	   Creado Por: Ing. Yesenia Moreno
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_obtener_ultimacuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
    ...
    function uf_update_cuota($as_codper,$as_codtippre,$ai_numpre,$ai_numcuo,$as_percob,$as_feciniper,$as_fecfinper,$ai_moncuopre,$as_tipo,$aa_seguridad)
    {
		...
		//				   as_codtippre  // C�digo del tipo de Prestamo
		//				   ai_numpre  // N�mero Correlativo del Prestamo
		//				   ai_numcuo  // N�mero de cuota del prestamo
		//				   as_percob  // Per�odo en que se va a cobrar esa cuota
		//				   as_feciniper  // Fecha desde del per�odo
		//				   as_fecfinper  // Fecha Hasta del Per�odo
		//				   ai_moncuopre  // Monto de la cuota del prestamo
		//				   as_tipo  // Tipo de actualizaci�n si es de los periodos a cobrar � si es el monto de la cuota
		//				   aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si realiz� el update � False si no lo actualiz�
		//	  Description: Funci�n que actualiza el periodo y las fechas del mismo de una cuota esto debido a una suspensi�n
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 10/02/2006 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 		$lb_valido=false;
		...
		switch($as_tipo)
		{
			case "1": // Actualizo el per�odo y las fechas de inicio y fin
		...
		$li_row=$this->io_sql->execute($ls_sql);
       	if ($li_row===false)
       	{
        	$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_update_cuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
       	}
       	else
       	{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			...
			{
				case "1": // Actualizo el per�odo y las fechas de inicio y fin
					$ls_descripcion ="Actualiz� el per�odo y las fechas de la cuota ".$ai_numcuo." del prestamo ".$ai_numpre." de tipo ".$as_codtippre." del personal ".
									 "".$as_codper." asociado a la n�mina ".$this->ls_codnom;
				break;

				case "2": // Actualizo el monto de la cuota
					$ls_descripcion ="Actualiz� el monto de la cuota ".$ai_numcuo." del prestamo ".$ai_numpre." de tipo ".$as_codtippre." del personal ".
									 "".$as_codper." asociado a la n�mina ".$this->ls_codnom;
					break;
			}
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
    ...
    function uf_existe_cuotapagada($as_codper,$as_codtippre,$ai_numpre)
    {
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 		$lb_existe=false;
       	...
       	if ($rs_data===false)
       	{
        	$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_existepago ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
       	}
       	else
       	{
			if ($row=$this->io_sql->fetch_row($rs_data))
    ...
    function uf_verificar_integridadcuota($as_codper,$as_codtippre,$ai_numpre)
    {
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 		$lb_valido=false;
       	...
       	if ($rs_data===false)
       	{
        	$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_verificar_integridadcuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
       	}
       	else
       	{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_verificarsueldo($as_codper,$ai_moncuo,$ai_sueper,$ai_numpre)
	{
		...
		//			       ai_numpre  // N�mero de Prestamos
		//	      Returns: lb_valido True si la cuota que va a pagar es menor que le 30% del sueldo � False si es mayor
		...
		if($li_validarprestamo=="1")
		{
			...
			if($rs_data===false)
			{
				$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_verificarsueldo ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
			else
			{
	...
	function uf_select_persona_prestamo_desincorpora_cierre($ad_desde_r,$ad_hasta_r)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Cierre Periodo M�TODO->uf_select_persona_prestamo_desincorpora_cierre ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	{  
		...
		//	  Description: Funcion que coloca como canceladas las cuotas del per�odo actual
...
/*NUEVA INCORPORACION PROXIMA
		...
		switch ($li_desincorpora)
			{
			case "0"; // No se Desincorpora de la n�mina 
				$ls_sql=$ls_sql." AND (sno_personalnomina.staper='1' OR sno_personalnomina.staper='9' OR sno_personalnomina.staper='2') AND sno_prestamosperiodo.codemp=sno_personalnomina.codemp AND sno_personalnomina.codnom=sno_prestamosperiodo.codnom AND sno_personalnomina.codper=sno_prestamosperiodo.codper";
			break;
		
			case "1"; // Se desincorpora de la n�mina
				$ls_sql=$ls_sql." AND (sno_personalnomina.staper='1' OR sno_personalnomina.staper='9') AND sno_prestamosperiodo.codemp=sno_personalnomina.codemp AND sno_personalnomina.codnom=sno_prestamosperiodo.codnom AND sno_personalnomina.codper=sno_prestamosperiodo.codper";
			break;
			}
	   ...
	   // echo $ls_sql;
	    ...
	    {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_cancelar_cuota ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	    }
	   return $lb_valido;	
...
/*NUEVA INCORPORACION PROXIMA*/
		...
		$ls_sql="UPDATE sno_hprestamosperiodo ".
				"	SET estcuo=".$li_estcuo."".
				" FROM sno_personalnomina".
				" WHERE sno_hprestamosperiodo.codemp='".$this->ls_codemp."'".
				"   AND sno_hprestamosperiodo.codnom='".$this->ls_codnom."'".
				//"   AND percob='".$ls_peractnom."'".
				"   AND sno_hprestamosperiodo.feciniper='".$ld_fecdesper."'".
				"   AND sno_hprestamosperiodo.fecfinper='".$ld_fechasper."'";

		switch ($li_desincorporar)
			{
			case "0"; // No se Desincorpora de la n�mina 
				$ls_sql=$ls_sql." AND (sno_personalnomina.staper='1' OR sno_personalnomina.staper='9' OR sno_personalnomina.staper='2') AND sno_hprestamosperiodo.codemp=sno_personalnomina.codemp AND sno_personalnomina.codnom=sno_hprestamosperiodo.codnom AND sno_personalnomina.codper=sno_hprestamosperiodo.codper";
			break;
		
			case "1"; // Se desincorpora de la n�mina
				$ls_sql=$ls_sql." AND (sno_personalnomina.staper='1' OR sno_personalnomina.staper='9') AND sno_hprestamosperiodo.codemp=sno_personalnomina.codemp AND sno_personalnomina.codnom=sno_hprestamosperiodo.codnom AND sno_personalnomina.codper=sno_hprestamosperiodo.codper";
			break;
			}
//echo $ls_sql;
	    $li_row=$this->io_sql->execute($ls_sql);
	...
	{  
		...
		//		   Access: public
		//	      Returns: lb_valido true si hizo el update correctamente false en caso contrario  
		//	  Description: Funcion que verifica que las cuotas sean cobradas en el c�lculo de la n�mina
		//	   Creado Por: Ing. Yesenia Moreno
	    ...
	    {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Prestamo Cuota M�TODO->uf_verificar_cuota_cobrada ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	    }
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				$ls_sql="SELECT sno_prestamos.codper ".
					...
					else
					{
						...
						{
							$this->io_mensajes->message("ERROR-> Se creo un Prestamo y la cuota para este periodo no fu� cobrada en el calculo de la n�mina. Debe Reversar la n�mina y volverla a calcular");
						}
					}
			}
			else
