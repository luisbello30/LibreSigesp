<?php
   ...
   function uf_print($as_codper, $as_cedper, $as_nomper, $as_apeper, $as_tipo, $ai_subnomina, $as_codnom)
   {
		...
		print "<td width=40>C�dula</td>";
		print "<td width=280>Nombre y Apellido</td>";
		print "<td width=60>Estatus</td>";
		print "<td width=60>Culminaci�n Contrato</td>";
		print "</tr>";
			...
			while(!$rs_data->EOF)
			{
				...
				switch ($as_tipo)
				{
					case "": // el llamado se hace desde sigesp_sno_d_personalnomina.php
						...
						print "'$ld_fecegrper','$ld_fecsusper','$ls_obsegrper','$ls_codger','".trim($ls_obsrecper)."');\">".$ls_codper."</a></td>";
                                                //print "'$ld_fecegrper','$ld_fecsusper','$ls_obsegrper','$ls_codger','DIRECTORA DE LA ESCUELA NACIONAL DE DANZA CARACAS');\">".$ls_codper."</a></td>";
				 ...
				 coddep, dendep, tippen, fecvid, prirem, segrem,salnorper, estencper,fecegrper,fecsusper,obsegrper, codger,obsrecper)
{
	...
	else
	{
		if ((tipnom!="3")&&(tipnom!="4"))
		{
		    ...
		    opener.document.form1.txtdescasicar.value=descasicar; //denomiancion de la asiganci�n del cargo
		}
...
function aceptarreppagnomhas(codper)
{
	...
	{
		alert("El rango que esta seleccionando es Inv�lido");
	}
}
