<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo)
	{
		...
		// Fecha Creaciï¿½n: 03/09/2007 
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_fun_nomina;
		
		$ls_descripcion="Generï¿½ el Reporte Consolidado ".$as_titulo.". Para ".$as_desnom.". ".$as_periodo;
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,&$io_pdf,&$io_encabezado)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_valido)
	{
           // echo "codnom: ".$ls_codnom."fechadesde: ".$ld_fecdesper."fecha hasta: ".$ld_fechasper."perdesd: ".$ls_codperdes."persona hasta: ".$ls_codperhas."coduniad: ".$ls_coduniadm.
	//												  "concepto cero ".$ls_conceptocero."concepto 2: ".$ls_conceptop2."concepto reporte: ".$ls_conceptoreporte."subnomdes: ".$ls_subnomdes."subnomhasta: ".$ls_subnomhas."orden: ".$ls_orden;
	...
	else // Imprimimos el reporte
	{
		...
		$periodos=$io_report->uf_fecha_periodos($ls_codperides,$ls_codperihas,$ls_codnom); // buscar fecha de los periodos
		$peri=count($io_report->DS->data_fec["codperi"]);
		$li_reg=1;
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
		     $ls_codper=$io_report->DS->data["codper"][$li_i];
		    $li_codcar=$io_report->DS->data["codcar"][$li_i];
		    $li_reg=1;
		    for($i=1;$i<=$peri;$i++)
		    {	
		      $fedes=$io_report->DS->data_fec["fecdesper"][$i];
		      $fehas=$io_report->DS->data_fec["fechasper"][$i];
		      $cod_peri=$io_report->DS->data_fec["codperi"][$i];
		      $periodos_cargos=$io_report->uf_periodo_cargo($cod_peri,$ls_codnom,$ls_codper,$li_codcar); // buscar periodos por cargo
		      $ls_quincena=3;
		      if($periodos_cargos==TRUE)
		      {
			    $lb_valido=$io_report->uf_recibopago_conceptopersonal($ls_codnom,$fedes,$fehas,$ls_codper,
			    ...
			    if($lb_valido)
			    {
			      ...
			      $ls_periodo="Periodo: <b>".$cod_peri."</b> del <b>".$fedes."</b> al <b>".$fehas."</b>";
			      $io_encabezado=$io_pdf->openObject(); 
			      uf_print_encabezado_pagina1($ls_titulo,$ls_desnom,$ls_periodo,&$io_pdf,&$io_encabezado); // Imprimimos el encabezado de la pï¿½gina
			  ...
			  /*  $lb_valido=$io_report->uf_recibopago_conceptopersonal($ls_codnom,$fedes,$fehas,$ls_codper,
				...
				unset($la_data_d);
				unset($la_data);
				if($i!=$peri || $li_i!=$li_totrow) $io_pdf->ezNewPage();	
			}//valido de concepto
			else
			{$lb_valido=TRUE;
			}
