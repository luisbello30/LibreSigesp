<?php
	...
	require_once("../../shared/ezpdf/class.ezpdf.php");
      ...
      //  var_dump($_SESSION["la_nomina"]["tiponomina"]); die();
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->write(4, 1, "C�dula",$lo_titulo);
		$lo_hoja->write(4, 2, "Apellidos y Nombres",$lo_titulo);
		$lo_hoja->write(4, 3, "Cargo",$lo_titulo);
		$lo_hoja->write(4, 4, "Unidad Adscripci�n",$lo_titulo);
		...
		$lo_hoja->write(4, 8, "Ubicaci�n F�sica",$lo_titulo);
		...
		$lo_hoja->write(4, 10, "Fecha de Ingreso a N�mina",$lo_titulo);
		$lo_hoja->write(4, 11, "Fecha de Culminaci�n de Contrato",$lo_titulo);
		$li_col=11;
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			$ls_apenomper=$io_report->rs_data->fields["apeper"]." ". $io_report->rs_data->fields["nomper"];
                        ...
                        if ($ls_codnom=='0102' || $ls_codnom=='0103' ) //solo para las nominas de reembolso. temporal
                        {
                            $ls_apenomper= str_ireplace('.', '', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('Ñ'), 'N', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('Á'), 'A', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('É'), 'E', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('È'), 'E', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('Í'), 'I', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('Ó'), 'O', $ls_apenomper);
                            $ls_apenomper= str_ireplace(utf8_decode('Ú'), 'U', $ls_apenomper);
                        }
			...
			$li_col++;
			$lo_hoja->set_column($li_col,$li_col,20);
			$lo_hoja->write(4, $li_col, "TOTAL ASIGNACI�N",$lo_titulo);
			$lo_hoja->write($li_row, $li_col, $li_totalasignacion, $lo_dataright);
			$li_col++;
			$lo_hoja->set_column($li_col,$li_col,20);
			$lo_hoja->write(4, $li_col, "TOTAL DEDUCCI�N",$lo_titulo);
