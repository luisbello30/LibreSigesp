<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		$ls_descripcion="Genero el Reporte ".$as_titulo." en Excel. Para ".$as_desnom.". ".$as_periodo;
	...
	else{//entra si el reporte se llama desde el modulo normal de nomina
	 ...
	 else
	{
	    ...
	    $ls_titulo="Reporte General de Pago";
	    $ls_periodo="Períodos Nro ".$ls_codperides."-".$ls_codperihas.", ".$ld_fecdesper." - ".$ld_fechasper."";
	    $ls_codnom=$_GET["codnom"];
	}
	...
	if($lb_valido)
	{
		...
		$lb_valido=$io_report->uf_pagonomina_personal_rango_peri($ls_codperdes,$ls_codperhas,$ls_conceptocero,$ls_conceptoreporte,$ls_conceptop2,
									 $ls_codubifis,$ls_codpai,$ls_codest,$ls_codmun,$ls_codpar,$ls_subnomdes,$ls_subnomhas,
									  $ls_orden,$ls_codnom,$ls_codperides,$ls_codperihas,$ls_coduniadm); // Cargar el DS con los datos de la cabecera del reporte
	}
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->write(4, 9, "Salario",$lo_titulo);
		$lo_hoja->write(4, 10, "Fecha de Ingreso a Nómina",$lo_titulo);
		$lo_hoja->write(4, 11, "Fecha de Culminación de Contrato",$lo_titulo);
		$li_col=11;
		// Buscamos los conceptos de asignaciï¿½n
		if(!$_GET["codperides"])
		    ...
		    $lb_valido=$io_report->uf_pagonomina_concepto_excel_rango_peri($ls_tituloconcepto," AND (sigcon='A' OR sigcon='B')",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
		if($lb_valido)
		{
		...
		// Buscamos los conceptos de Deducciï¿½n
		if(!$_GET["codperides"])
		    ...
		    $lb_valido=$io_report->uf_pagonomina_concepto_excel_rango_peri($ls_tituloconcepto," AND (sigcon='D' OR sigcon='E')",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
		...
		if(!$_GET["codperides"])
		    ...
		    $lb_valido=$io_report->uf_pagonomina_concepto_excel_rango_peri($ls_tituloconcepto," AND sigcon='P'",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
		if($lb_valido)
		{
			while(!$io_report->rs_data_detalle->EOF)
			{
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			$lo_hoja->write($li_row, 9, $ld_sueper, $lo_dataright);
			$lo_hoja->write($li_row, 10, $ld_fecingnom, $lo_datacenter);
			$lo_hoja->write($li_row, 11, $ld_feculcont, $lo_datacenter);
			$li_col=11;
			...
			if(!$_GET["codperides"])
			    ...
			    $lb_valido=$io_report->uf_pagonomina_conceptopersonal_excel_rango_peri($ls_codper,$ls_tituloconcepto," AND (".$ls_tabla.".tipsal='A' OR ".$ls_tabla.".tipsal='V1' OR ".$ls_tabla.".tipsal='W1')",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
			...
			if($lb_valido)
			{
				while(!$io_report->rs_data_detalle->EOF)
				{
					...
					$li_col=$io_report->DS_detalle2->getValue("columna",$li_find);//echo "  ";
			...
			if(!$_GET["codperides"])
			    ...
			    $lb_valido=$io_report->uf_pagonomina_conceptopersonal_excel_rango_peri($ls_codper,$ls_tituloconcepto,"AND (".$ls_tabla.".tipsal='D' OR ".$ls_tabla.".tipsal='V2' OR ".$ls_tabla.".tipsal='W2')",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
			...
			if(!$_GET["codperides"])
			    ...
			    $lb_valido=$io_report->uf_pagonomina_conceptopersonal_excel_rango_peri($ls_codper,$ls_tituloconcepto,"AND (".$ls_tabla.".tipsal='P1' OR ".$ls_tabla.".tipsal='V3' OR ".$ls_tabla.".tipsal='W3')",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
			...
			if(!$_GET["codperides"])
			    ...
			    $lb_valido=$io_report->uf_pagonomina_conceptopersonal_excel_rango_peri($ls_codper,$ls_tituloconcepto,"AND (".$ls_tabla.".tipsal='P2' OR ".$ls_tabla.".tipsal='V4' OR ".$ls_tabla.".tipsal='W4')",$ls_codnom,$ls_codperides,$ls_codperihas); // Obtenemos el detalle del reporte
