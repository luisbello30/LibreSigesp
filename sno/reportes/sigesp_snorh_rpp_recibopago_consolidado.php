<?php
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,300,9,$as_periodo); // Agregar el t�tulo
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,285,9,$as_desnom); // Agregar el t�tulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$li_racnom=$io_report->DS->data["racnom"][$li_i];
                        $as_codgra=$io_report->DS->data["codgra"][$li_i];
                        $as_codpas=$io_report->DS->data["codpas"][$li_i];
			...
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_cedper,$ls_nomper,$ls_descar,&$io_cabecera,$as_codgra,$as_codpas,$li_racnom,&$io_pdf); // Imprimimos la cabecera del registro
			}
			else
			{
				uf_print_cabecera2($ls_cedper,$ls_nomper,$ls_descar,&$io_cabecera,$as_codgra,$as_codpas,$li_racnom,&$io_pdf); // Imprimimos la cabecera del registro
			}
