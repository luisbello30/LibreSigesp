<?php
	...
	function uf_leer_archivo($as_archivo)
	{
		...
		//    Description: funci�n que lee un archivo de texto y lo mete en una cadena
		//	   Creado Por: Ing. Yesenia Moreno
				...
				if($lb_valido)
				{
					...
					while ((!$io_report->rs_detalle->EOF)&&($lb_valido))
					{
						...
						$ls_dia_letra=$io_numero_letra->letra();
                                                $ls_dia_letra=str_replace(' CON 00/100','',$ls_dia_letra);
                                                if ($ls_dia_letra == 'UNO')
                                                        $ls_dia_letra='UN';
                                               
						$ls_cuerpo=$ls_nuevocuerpo;
						$ls_cuerpo=str_replace("\$ls_ente",$ls_ente,$ls_cuerpo);
						$ls_cuerpo=str_replace("\$ls_dia",strtolower($ls_dia_letra).'('.$ls_dia_act.')',$ls_cuerpo);
						$ls_cuerpo=str_replace("\$ls_mes",strtolower($ls_mes_act),$ls_cuerpo);
                                                $io_numero_letra->setNumero($ls_ano_act);
						$ls_anio_letra=$io_numero_letra->letra();
                                                $ls_anio_letra=str_replace(' CON 00/100','',$ls_anio_letra);
						$ls_cuerpo=str_replace("\$ls_ano",strtolower($ls_anio_letra).'('.$ls_ano_act.')',$ls_cuerpo);
						...
						$ls_cuerpo=str_replace("\$ls_apellidos",$ls_apeper,$ls_cuerpo);
                				...
                				$ls_cuerpo=str_replace("\$ls_cedula",  $ls_nacper.'-'.number_format($ls_cedper,0,'','.'),$ls_cuerpo);
						...
						$ls_cuerpo=str_replace("\$li_horas_lab",$li_horper,$ls_cuerpo);
                                                $ls_sueintper=str_replace('BOLIVARES','',$ls_sueintper);
