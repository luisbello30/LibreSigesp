<?php
	...
	function uf_print_encabezado_pagina2($as_titulo,$as_desnom,$as_periodo,&$io_pdf)
	{
		...
		$li_tm=$io_pdf->getTextWidth(11,$as_titulo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,310,11,$as_titulo); // Agregar el t�tulo
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,300,9,$as_desnom); // Agregar el t�tulo
		$li_tm=$io_pdf->getTextWidth(9,$as_periodo);
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,285,9,$as_periodo); // Agregar el t�tulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,&$io_cabecera,$as_codgra,$as_codpas,$as_racnom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
                ...
                if($as_racnom!=0)
                {
                        ...
                        //$io_pdf->ezSetDy(-1);
                        $la_data=array(array('grado'=>'Grado:', 'paso'=>'Paso:'));
                        $la_columna=array('grado'=>'','paso'=>'');
                        $la_config=array('showHeadings'=>0, // Mostrar encabezados
                                                         'xPos'=>315,
                                                         'fontSize' => 8, // Tama�o de Letras
                                                         'showLines'=>0, // Mostrar L�neas
                                                         ...
                                                         'maxWidth'=>550, // Ancho M�ximo de la tabla
                                                         'xOrientation'=>'center', // Orientaci�n de la tabla
                                                         'cols'=>array('grado'=>array('justification'=>'left','width'=>225), // Justificaci�n y ancho de la columna
                                                                                   'paso'=>array('justification'=>'left','width'=>305))); // Justificaci�n y ancho de la columna
                        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
                        $io_pdf->addText(40,661,'10','______________________________________________________________________________________________');
                        $la_data=array(array('grado'=>$as_codgra, 'paso'=>$as_codpas));
                        $la_columna=array('grado'=>'','paso'=>'');
                        $la_config=array('showHeadings'=>0, // Mostrar encabezados
                                                         'xPos'=>315,
                                                         'fontSize' => 7, // Tama�o de Letras
		...
		$io_pdf->addText(40,645,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetY(645);
		$la_data=array(array('denomasig'=>'<b>DENOMINACION</b>', 'valorasig'=>'<b>ASIGNACION</b>', 'denomdedu'=>'<b>DENOMINACION</b>','valordedu'=>'<b>DEDUCCION</b>'));
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(40,633,'10','______________________________________________________________________________________________');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera2($as_cedper,$as_nomper,$as_descar,&$io_cabecera,$as_codgra,$as_codpas,$as_racnom,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezSetDy(-175);
		$la_data=array(array('nombre'=>'Apellidos y Nombres:', 'cedula'=>'Cedula:', 'cargo'=>'Cargo:' ));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
						 'fontSize' => 8, // Tama�o de Letras
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
                ...
                {
                        $io_pdf->saveState();
                        $io_pdf->addText(40,252,'10','______________________________________________________________________________________________');
                        //$io_pdf->ezSetDy(-1);
                        $la_data=array(array('grado'=>'Grado:', 'paso'=>'Paso:'));
                        $la_columna=array('grado'=>'','paso'=>'');
                        $la_config=array('showHeadings'=>0, // Mostrar encabezados
                                                         ...
                                                         'cols'=>array('grado'=>array('justification'=>'left','width'=>225), // Justificaci�n y ancho de la columna
                                                                                   'paso'=>array('justification'=>'left','width'=>305))); // Justificaci�n y ancho de la columna
                        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
                        $io_pdf->addText(40,242,'10','______________________________________________________________________________________________');
                        $la_data=array(array('grado'=>$as_codgra, 'paso'=>$as_codpas));
                        $la_columna=array('grado'=>'','paso'=>'');
                        $la_config=array('showHeadings'=>0, // Mostrar encabezados
                                                         'xPos'=>315,
		...
		$io_pdf->addText(40,224,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetY(226);
		$la_data=array(array('denomasig'=>'<b>DENOMINACION</b>', 'valorasig'=>'<b>ASIGNACION</b>', 'denomdedu'=>'<b>DENOMINACION</b>','valordedu'=>'<b>DEDUCCION</b>'));
		$la_columna=array('denomasig'=>'<b>DENOMINACION</b>',
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('denomasig'=>array('justification'=>'center','width'=>150), // Justificaci�n y ancho de la columna
						 			   ...
						 			   'valordedu'=>array('justification'=>'right','width'=>100))); // Justificaci�n y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(40,214,'10','______________________________________________________________________________________________');
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_cabecera,'all');
	}// end function uf_print_cabecera
	...
	if(strstr(strtoupper($_SESSION["la_nomina"]["desnom"]),'AGUINALDO'))
	    $ls_periodo="Periodo: <b>".$ls_peractnom."</b>";
	else
	...
	else // Imprimimos el reporte
	{
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			$li_total=$io_report->rs_data->fields["total"];
                        $as_codgra=$io_report->rs_data->fields["codgra"];
                        $as_codpas=$io_report->rs_data->fields["codpas"];
                        $as_racnom=$io_report->rs_data->fields["racnom"];
			$io_cabecera=$io_pdf->openObject(); // Creamos el objeto cabecera
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_cedper,$ls_nomper,$ls_descar,$io_cabecera,$as_codgra,$as_codpas,$as_racnom,$io_pdf); // Imprimimos la cabecera del registro
			}
			else
			{
				uf_print_cabecera2($ls_cedper,$ls_nomper,$ls_descar,$io_cabecera,$as_codgra,$as_codpas,$as_racnom,$io_pdf); // Imprimimos la cabecera del registro
			}
