...
class sigesp_snorh_c_fideiconfigurable
{
	...
	function uf_select_fideiconfigurable($ai_anocurfid, $as_codded, $as_codtipper)
	{
		...
		if($rs_data===false)
		{
        	$this->io_mensajes->message("CLASE->Fideiconfigurable M�TODO->uf_select_fideiconfigurable ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
	...
	function uf_insert_fideiconfigurable($ai_anocurfid, $as_codded, $as_codtipper, $ai_diabonvacfid, $ai_diabonfinfid, $as_cueprefid, $aa_seguridad)
	{
		...
		//                 ai_diabonfinfid //dias de bono fin de a�o fideicomiso		
		...
		{
 			$lb_valido=false;
        	$this->io_mensajes->message("CLASE->fideiconfigurable M�TODO->uf_insert_fideiconfigurable ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		...
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la Configuraci�n de Fideicomiso. A�o ".$ai_anocurfid." Dedicaci�n ".$as_codded." Tipo Personal ".$as_codtipper;
			...
			if($lb_valido)
			{	
				$this->io_mensajes->message("La configuraci�n del Fideicomiso fue registrada.");
				$this->io_sql->commit();
			}
			else
			{
	...
	function uf_update_fideiconfigurable($ai_anocurfid,$as_codded,$as_codtipper,$ai_diabonvacfid,$ai_diabonfinfid,$as_cueprefid,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
        	$this->io_mensajes->message("CLASE->fideiconfigurable M�TODO->uf_update_fideiconfigurable ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
			////////////////////////////////         SEGURIDAD               //////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz� la Configuraci�n de Fideicomiso. A�o ".$ai_anocurfid." Dedicaci�n ".$as_codded." Tipo Personal ".$as_codtipper;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////	
			if($lb_valido)
			{	
				$this->io_mensajes->message("La configuraci�n del Fideicomiso fue Actualizada.");
				$this->io_sql->commit();
			}
			else
			{
	...
	{		
		...
		switch ($as_existe)
		{
			case "FALSE":
				...
				{
					$this->io_mensajes->message("La configuraci�n del Fideicomiso ya existe, No la puede incluir.");
				}
				break;
			...
			case "TRUE":
				...
				{
					$this->io_mensajes->message("La configuraci�n del Fideicomiso no existe, No la puede actualizar.");
				}
				break;
		}
		return $lb_valido;
	...
	function uf_delete_fideiconfigurable($ai_anocurfid,$as_codded,$as_codtipper,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Fideiconfigurable M�TODO->uf_delete_fideiconfigurable ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="DELETE";
			$ls_descripcion ="Elimin� la Configuraci�n de Fideicomiso. A�o ".$ai_anocurfid." Dedicaci�n ".$as_codded." Tipo Personal ".$as_codtipper;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////	
			if($lb_valido)
			{	
				$this->io_mensajes->message("La configuraci�n del Fideicomiso fue Eliminada.");
				$this->io_sql->commit();
			}
			else
			{
	...
	function uf_load_dias_vacaagui($ai_anocurfid,$as_codded,$as_codtipper,&$ai_diavac,&$ai_diaagui)
	{
		...
		//				   as_codded     // codigo de dedicacion
		//                 as_codtipper  // codigo tipo personal
		//                 ai_diavac  // d�as de vacaciones
		//                 ai_diaagui  // d�as de aguinaldo 
		//	      Returns: lb_valido True si existe la configuraci�n � False si no existe
		//	  Description: Funcion que obtiene los d�as de vacaciones y de aguinaldo para esa dedicaci�n y tipo de personal
		//	   Creado Por: Ing. Yesenia Moreno
		...
		if($rs_data===false)
		{
        	$this->io_mensajes->message("CLASE->Fideiconfigurable M�TODO->uf_load_dias_vacaagui ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
			...
			{
				$lb_valido=false;
        		$this->io_mensajes->message("La configuraci�n del fideicomiso para el A�o->".$ai_anocurfid." Dedicaci�n->".$as_codded." Tipo de Personal->".$as_codtipper." No se ha realizado.Debe configurarla para hacer el proceso."); 
			}
			else
			{
