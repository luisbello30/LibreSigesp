...
class sigesp_scv_class_report
{
	...
	function uf_select_solicitudpago_beneficiario($as_codemp,$as_codsoldes,$as_codsolhas,$ad_fecdes,$ad_fechas,$ai_orden,$as_codsolvia)	
	{
		...
		$ls_sql="SELECT rpc_beneficiario.ced_bene AS cedper, rpc_beneficiario.nombene AS nomper, rpc_beneficiario.apebene AS apeper, '' AS desuniadm, ".
				...
				"       (SELECT COUNT(codper) FROM scv_dt_personal ".
				"		  WHERE ".
				"			 scv_dt_personal.codsolvia = scv_solicitudviatico.codsolvia ) AS acompanante,  ".
			    "       '' AS cargo".
				"  FROM scv_dt_personal,  scv_solicitudviatico, scv_misiones, rpc_beneficiario ".
				" WHERE scv_solicitudviatico.codemp='".$as_codemp."' ".
				$ls_criterio.
				...
				"   AND scv_dt_personal.codemp = rpc_beneficiario.codemp ".
				"   AND scv_dt_personal.codper = rpc_beneficiario.ced_bene ".
				" GROUP BY rpc_beneficiario.ced_bene, rpc_beneficiario.nombene,rpc_beneficiario.apebene,rpc_beneficiario.telbene, 
				  scv_solicitudviatico.fecsalvia, scv_solicitudviatico.fecregvia,scv_solicitudviatico.fecsolvia,
				  scv_solicitudviatico.numdiavia,scv_misiones.denmis,scv_solicitudviatico.codsolvia,scv_solicitudviatico.codsolvia,
				  scv_solicitudviatico.solviaext,scv_solicitudviatico.obssolvia ";
	...
	function uf_select_solicitudpago_asignaciones($as_codemp,$as_codsolvia)	
	{
		...
		$ls_sql= "SELECT scv_dt_asignaciones.codemp, scv_dt_asignaciones.codsolvia, scv_dt_asignaciones.codasi,".
				 ...
				 "                 AND scv_dt_asignaciones.codasi=scv_tarifakms.codtar) END) AS denasi, ".
				 "       (CASE scv_dt_asignaciones.proasi ".
				 "        WHEN 'TVS' THEN (SELECT (CASE scv_tarifas.nacext WHEN '0' THEN (scv_tarifas.monbol+scv_tarifas.monpas+scv_tarifas.monhos+scv_tarifas.monali+scv_tarifas.monmov) 
							      ELSE (scv_tarifas.mondol) END) 
							    FROM scv_tarifas WHERE scv_dt_asignaciones.codemp=scv_tarifas.codemp AND scv_dt_asignaciones.codasi=scv_tarifas.codtar)".
	...
	function uf_select_listadosolicitudes($ad_fecregdes,$ad_fecreghas,$as_coduniadm,$as_orden)
	{
		...
		$ls_sql="SELECT scv_solicitudviatico.codsolvia,scv_solicitudviatico.fecsalvia,MAX(scv_solicitudviatico.monsolvia) AS monto,scv_solicitudviatico.fecregvia,scv_rutas.desrut,scv_dt_personal.codper,".
