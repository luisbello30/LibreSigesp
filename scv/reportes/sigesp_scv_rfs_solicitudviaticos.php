<?php
	...
	function uf_print_encabezado_pagina($as_codsolvia,&$io_encabezado,&$io_pdf)
	{
		...
		$io_pdf->addText(468,727,11,"No DE CONTROL"); // Agregar NRO DE CONTROL
							  ...
							  $as_telefono,$ai_sueper,$as_denestpro1,$as_denestpro2,$as_desrut,$ai_solviaext,$as_obssolvia,&$io_pdf)
	{
		...
		$la_columna=array('nombre'=>'<b>2. Nombre del Funcionario</b>','cedula'=>utf8_decode('<b>3. Cédula de Identidad No</b>'),
						  'cargo'=>utf8_decode('<b>4. Denominación del Cargo</b>'),'unidad'=>'<b>5. Sueldo</b>','categoria'=>'<b>6. Telefono Oficina / Celular</b>');
		...
		$la_data=array(array('titulo'=>utf8_decode('<b>7. No de Cuenta Nómina</b>'),'corriente'=>$ls_corriente,'titulocorriente'=>'Corriente',
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>utf8_decode('<b>DATOS DE LA MISIÓN</b>')));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tama�o de Letras
						 'showLines'=>1, // Mostrar L�neas
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('estpro1'=>utf8_decode('<b>9. Proyecto / Acción Centralizada:</b> ').$as_denestpro1,'estpro2'=>utf8_decode('<b>10. Acción Específica:</b> ').$as_denestpro2));
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('desde'=>'<b>13. Tiempo Estimado        Desde:</b> '.$ad_fecsalvia,'hasta'=>'<b>Hasta:</b> '.$ad_fecregvia,'dias'=>utf8_decode('<b>14. Total Días:</b> ').$ai_numdiavia));
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('mision'=>utf8_decode('<b>15. Conceptos de la Misión:</b> ').$as_denmis));
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('desde'=>utf8_decode('<b>16. Tipo de Viático:</b>            Nacional    ').$ls_extno.'               Exterior    '.$ls_extsi.' ','dias'=>'<b>17. Pernocta:   </b>              Si '.$ls_pernoctasi.'            No '.$ls_pernoctano.' '));
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('desde'=>array('justification'=>'left','width'=>270),
									   'dias'=>array('justification'=>'left','width'=>270))); // Ancho M�ximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
		unset($la_columna);
		unset($la_config);
/*		$la_data[1]=array('desde'=>'<b>18. Vehiculo Oficial:</b>           Si      No','hasta'=>'<b>19. Pasaje Terrestre</b>            Si      No','dias'=>'<b>20. Vehiculo Propio</b>          Si      No');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('desde'=>array('justification'=>'left','width'=>180),
									   ...
									   'dias'=>array('justification'=>'left','width'=>180))); // Ancho M�ximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
		unset($la_columna);
		unset($la_config);*/
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>utf8_decode('21. Pasaje Aéreo                       Si         No                                  Ruta:')));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tama�o de Letras
						 'showLines'=>1, // Mostrar L�neas
...
/*		unset($la_data);
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'22. C�digo:                              Nacional  4.03.09.01.00     <b>'.$ls_extno.'</b>                       Extranjero   4.03.09.02.00     <b>'.$ls_extsi.'</b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tama�o de Letras
						 'showLines'=>1, // Mostrar L�neas
	...
	function uf_print_detalle_asignaciones($la_data,$ai_total,$as_totalletras,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>utf8_decode('<b>Total Misión Bs.:</b>              '),'total'=>$ai_total));
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('elaborado'=>'<b>24. Solicitado Por:</b>','revisado'=>utf8_decode('<b>25. Responsable del Proyecto o Acción Centralizada:</b>'),'autorizado'=>'<b>26. Oficina de Administraci�n y Servicios:</b>'),
