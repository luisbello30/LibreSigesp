...
function ue_inicializar()
{
	...
	function onInicializar(respuesta)
	{
		...
		{  
			...
			var cajas = new Array('hidid_art','txtporant','cmbestsue','cmbincbon','txtsc_cta_ps','txtemp_fijo_ps','txtemp_fijo_vac','txtemp_fijo_agu','txtobr_fijo_ps','txtobr_fijo_vac','txtobr_fijo_agu','txtemp_cont_ps','txtemp_cont_vac','txtemp_cont_agu','txtemp_esp_ps','txtemp_esp_vac','txtemp_esp_agu','txtemp_max_ps','txtemp_max_vac','txtemp_max_agu');
			...
			habilitar("txtporant,cmbestsue,cmbincbon,txtsc_cta_ps,txtemp_fijo_ps,txtemp_fijo_vac,txtemp_fijo_agu,txtobr_fijo_ps,txtobr_fijo_vac,txtobr_fijo_agu,txtemp_cont_ps,txtemp_cont_vac,txtemp_cont_agu,txtemp_esp_ps,txtemp_esp_vac,txtemp_esp_agu,txtemp_max_ps,txtemp_max_vac,txtemp_max_agu");
			$('txtporant').focus();
		}
		else
		{
			alert("No existen datos en Configuraci�n.");	
			var cajas = new Array('hidid_art','txtporant','cmbestsue','cmbincbon','txtsc_cta_ps','txtemp_fijo_ps','txtemp_fijo_vac','txtemp_fijo_agu','txtobr_fijo_ps','txtobr_fijo_vac','txtobr_fijo_agu','txtemp_cont_ps','txtemp_cont_vac','txtemp_cont_agu','txtemp_esp_ps','txtemp_esp_vac','txtemp_esp_agu','txtemp_max_ps','txtemp_max_vac','txtemp_max_agu');
			for (i=0; i<cajas.length; i++)
			{	
				$(cajas[i]).value = "";
...
function ue_guardar()
{
	...
	if(lb_valido)
	{
		...
		{
			alert("No tiene permiso para actualizar informaci�n.");
		}
		else
		{
			...
			var configuracion =
			{
				...
				"sig_cuenta_emp_esp_agu":$F('txtemp_esp_agu'),
                                "sig_cuenta_emp_max_ps":$F('txtemp_max_ps'),
                                "sig_cuenta_emp_max_vac":$F('txtemp_max_vac'),
                                "sig_cuenta_emp_max_agui":$F('txtemp_max_agu')
...
{   
  ...
  if ($('hidctas').value=='12')
  {
    ...
    if ($('hidctas').value=='13')
  {
	  var cajas = new Array('txtemp_max_ps','txtdenemp_max_ps');
	  ...
	  deshabilitar("txtemp_max_ps, txtdenemp_max_ps");
          
  }
    if ($('hidctas').value=='14')
  {
	  var cajas = new Array('txtemp_max_vac','txtdenemp_max_vac');
	  for (i=0; i<cajas.length; i++)
	  {
		  $(cajas[i]).value = arr_datos[i];
	  }
	  deshabilitar("txtemp_max_vac, txtdenemp_max_vac");
  }
  if ($('hidctas').value=='15')
  {
	  var cajas = new Array('txtemp_max_agu','txtdenemp_max_agu');
	  for (i=0; i<cajas.length; i++)
	  {
		  $(cajas[i]).value = arr_datos[i];
	  }
	  deshabilitar("txtemp_max_agu, txtemp_esp_agu");
  }
  //$('hidguardar').value = "modificar";
}
