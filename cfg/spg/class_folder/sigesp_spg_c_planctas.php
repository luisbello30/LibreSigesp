...
function uf_procesar_cuentas($as_cuenta_spg,$as_denominacion_cta,$aa_estpro,$as_cuenta_scg,$aa_security)
{
		...
		if($this->int_spg->uf_spg_select_cuenta($this->dat["codemp"], $aa_estpro, $ls_spg_cuenta, &$ls_status, &$ls_denominacion, $ls_scg_cuenta ))
		{
				...
				if($lb_valido)
				{
					...
					$ls_denctapu="";
					$ls_ctaplan="";
					$this->is_msg_error="Denominaci�n Actualizada";
					$disabled="";
	...
	function uf_load_casamiento_contable($ls_empresa)
	{
		...
		//         Access: public 
		//      Argumento: ls_empresa  // c�digo de la empresa
		//	      Returns: Retorna un Booleano
		//    Description: 
		//	   Creado Por: Ing. Gloriely Fr�itez
		// Fecha Creaci�n: 02/09/08 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->msg->message("M�TODO->uf_load_casamiento_contable ERROR->".$this->fun->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{ 
...
function uf_load_plan_cuenta_ingreso($as_codemp)
{
  ...
  return $lb_valido;
	...
	function uf_spg_delete_cuentafuentefinanciamiento($as_codemp, $aa_estprog, $as_spg_cuenta)
	{
		...
		if($li_rows===false)
		{
			$this->msg->message("M�TODO->uf_spg_delete_cuentafuentefinanciamiento ERROR->".$this->fun->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		return $lb_valido;
