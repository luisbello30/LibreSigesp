...
class sigesp_sep_class_report
{
								   ...
								   $ai_aprobada,$ai_pagada)
	{
		...
		//	  			   as_codprobendes  // C�digo del proveedor/beneficiario de inicio del parametro de Busqueda
		//	  			   as_codprobenhas  // C�digo del proveedor/beneficiario de fin del parametro de Busqueda
		...
		//	  			   as_codusudes    // Indica si se desea filtrar por el c�digo de usuario
		//	  			   as_codusuhas    // Indica si se desea filtrar por el c�digo de usuario
		...
		//    Description: funci�n que busca la informaci�n de las solicitudes 
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 11/03/2007									Fecha �ltima Modificaci�n :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_criterio="";
		$ls_orden="";
	...
	function uf_select_solicitud($as_numsol)
	{
		...
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: funci�n que busca la informaci�n de la una solicitud de ejecucion presupuestaria 
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 12/03/2007									Fecha �ltima Modificaci�n :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		switch ($_SESSION["ls_gestor"])
		{
		...
		$ls_sql="SELECT sep_solicitud.codemp, sep_solicitud.numsol, sep_solicitud.codtipsol, sep_solicitud.coduniadm,".
				...
				"                                                  ELSE 'NINGUNO'".
				"         END) AS rif,
                                          (CASE WHEN sep_solicitud.tipo_destino='B' THEN (SELECT rpc_beneficiario.nacben||'-'||rpc_beneficiario.ced_bene".
				"                                                      FROM rpc_beneficiario".
				"                                                     WHERE sep_solicitud.codemp=rpc_beneficiario.codemp".
				"                                                       AND sep_solicitud.ced_bene=rpc_beneficiario.ced_bene)".
				
				"                                                  ELSE 'NINGUNO'".
				"         END) AS cedula_benepro,".
				"       (SELECT denfuefin".
	...
	function uf_select_dt_solicitud($as_numsol)
	{
		...
		//	    Arguments: as_numsoldes     // Numero de solicitud de inicio del parametro de Busqueda
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: funci�n que obtiene la informacion de detalle (bienes, servicios o conceptos) de una solicitud
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
	   ...
	   switch($_SESSION["ls_gestor"])
	   {
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_dt_solicitud ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_dt_cargos($as_numsol)
	{
		...
		//	    Arguments: as_numsol     // Numero de solicitud 
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: funci�n que obtiene la informacion de detalle de los cargos (bienes, servicios o conceptos) de una solicitud
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 17/03/2007									Fecha �ltima Modificaci�n :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_dt_cargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_dt_spgcuentas($as_numsol)
	{
		...
		$ls_sql="SELECT substr(sep_cuentagasto.codestpro1,$li_longestpro1,$len_ep1) AS codestpro1, ".
				"substr(sep_cuentagasto.codestpro2,$li_longestpro2,$len_ep2) AS codestpro2, ". 
				"substr(sep_cuentagasto.codestpro3,$li_longestpro3,$len_ep3) AS codestpro3, ". 
				"substr(sep_cuentagasto.codestpro4,$li_longestpro4,$len_ep4) AS codestpro4, ". 
				"substr(sep_cuentagasto.codestpro5,$li_longestpro5,$len_ep5) As codestpro5, ". 
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_dt_spgcuentas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_dt_unidad($as_codart)
	{
		...
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: funci�n que obtiene las unidades de medida de un articulo
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 17/03/2007									Fecha �ltima Modificaci�n :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_dt_unidad ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		else
		{
	...
	function uf_sep_select_unidad_medida($as_codart,&$as_codunimed)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
		 ...
		 {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_sep_select_unidad_medida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		 }		
		 else
		 {
	...
	function uf_sep_select_denominacion_unidad_medida($as_codart,$as_codunimed,&$as_denunimed)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
		 ...
		 {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_sep_select_denominacion_unidad_medida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		 }		
		 else
		 {
			 if($row=$this->io_sql->fetch_row($rs))
   ...
   //---------------------------------------------------------------------------------------------------------------------------------	
	                              ...
	                              &$ad_monto_disponible)
	{
		...
		$ls_sql =" SELECT (asignado-(comprometido+precomprometido)+aumento-disminucion) as disponible".
		 ...
		 {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_disponible ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		 }		
		 else
		 {
			 if($row=$this->io_sql->fetch_row($rs))
   ...
   //---------------------------------------------------------------------------------------------------------------------------------	
	...
	function uf_select_ubicacionsolicitudes($as_numsoldes,$as_numsolhas,$as_tipproben,$as_codprobendes,$as_codprobenhas,&$lb_valido)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_ubicacionsolicitudes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $rs_data;
	}// end function uf_select_solicitud
	...
	function uf_load_sep_ubicacionprecompromiso($as_numsol,&$lb_valido)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_load_sep_ubicacionprecompromiso ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $rs_data;
	}// end function uf_select_cuenta_gasto
