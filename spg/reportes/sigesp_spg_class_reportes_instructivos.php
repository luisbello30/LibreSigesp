	 ...
	 {		
		...
		if($li_numrows>=0)
		{
		     while($row=$this->io_sql->fetch_row($rs_data))
			 {
			   ...
			   $ld_disponibilidad=$ld_asignado+$ld_aumento_acumulado-$ld_disminucion_acumulado-$ld_comprometer;
	 ...
	 $ls_sql = " SELECT DT.fecha, DT.monto, OP.aumento, OP.disminucion, ".
	  ...
	  else
	  {
		while($row=$this->io_sql->fetch_row($rs_ejecutado))
		{
		  ...
		  if(($li_comprometer)||($li_precomprometer))
		  { 
...
//-----------------------------------------------------------------------------------------------------------------------------------
    ...
    {////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 if($lb_valido)
	 {
		$lb_valido=$this->uf_spg_reportes_ingresos_actividadespropias2($adt_fecdes,$adt_fechas,$adts_datastore,$as_mesdes,$as_meshas);
	 }
	 if($lb_valido)
	 {
		$lb_valido=$this->uf_spg_reportes_otrosingresos2($adt_fecdes,$adt_fechas,$adts_datastore,$as_mesdes,$as_meshas);
	 }
     if($lb_valido)
     {
	 ...
	 if($lb_valido)
	 {
		$lb_valido=$this->uf_spg_reportes_gastos_corrientes2($adt_fecdes,$adt_fechas,$adts_datastore,$as_mesdes,$as_meshas);
	 } */
	 if($lb_valido)
	 {
    ...
    {////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación : 18/06/2008                       Fecha última Modificacion :      Hora :
	  ...
	  $ls_sql=" SELECT spi_cuenta, max(denominacion) as denominacion,                                    ".
			  ...
			  "        spi_cuenta = '305000000".$ls_ceros."' OR spi_cuenta = '305010000".$ls_ceros."' OR ".
			  "        spi_cuenta = '305020000".$ls_ceros."' OR spi_cuenta = '305030000".$ls_ceros."'  
			  		   OR spi_cuenta = '305040000".$ls_ceros."'OR spi_cuenta ='305050000".$ls_ceros."'
			   		   GROUP BY spi_cuenta
			          ORDER BY spi_cuenta ";
	  $rs_data=$this->io_sql->select($ls_sql);
	  if($rs_data===false)
...
//---------------------------------------------------------/////////////////////////////////////////////////////////////////    
    ...
    { /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  //    Fecha Creación :    28/06/2008                       Fecha última Modificacion :      Hora :
	    ...
	    $ls_ceros=$this->io_funciones->uf_cerosderecha("",$li_len);
		$ls_sql = "SELECT denominacion FROM sigesp_plan_unico_re WHERE sig_cuenta='".$ls_cuenta_buscar."' ";    //$as_spi_cuenta.$ls_ceros
		$rs_data=$this->io_sql->select($ls_sql);                       
		if($rs_data===false)
		{
			$lb_valido=false;
