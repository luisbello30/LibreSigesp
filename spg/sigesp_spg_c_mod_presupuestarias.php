...
function uf_generar_num_cmp($as_codemp,$as_procede)
{
	 ...
	 $ls_sql="SELECT max(comprobante) as comprobante ".
					...
					"					 AND (comprobante not like '%�%' AND comprobante not like '%�%')".
...
/**********************************************************************************************************************************/
	...
	//	Description: M�todo que inserta el registro comprobante (informaci�n cabezera )en la tabla SIGESP_Cmp. Usado en el m�dulo de comprobante contable
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->is_msg_error = "Error en m�todo uf_sigesp_insert_comprobante ";
		}
		else
		{
		   $this->is_log_transacciones="Inserto comprobante N�".$as_comprobante." de procedencia ".$as_procede." con fecha ".$as_fecha; 
		}
		return $lb_valido;
...
/**********************************************************************************************************************************/
    ...
    {		
		...
		//Description:  M�todo que elimina el registro comprobante (informaci�n cabezera ) en la tabla SIGESP_Cmp
		/////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($li_result===false)
		{
			$this->is_msg_error = "Error en m�todo uf_sigesp_update_comprobante ".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		else
		{
...
/**********************************************************************************************************************************/
    ...
    //	            $as_cuenta->cuenta contable ,$as_procede_doc->procede documento,$as_documento->n� documento
...
/**********************************************************************************************************************************/
 ...
 {	 
	...
	//Description:  inserta la informaci�n del saldo de la cuenta correspondiente.
	////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
...
function uf_valida_datos_cmp($as_comprobante,$ad_fecha,$as_procedencia,$as_desccomp,$as_cod_prov,$as_ced_bene,$as_tipo)
{
	...
	{
		$this->io_msg->message("Debe registrar la c�dula del beneficiario1.");
		return false;
	}
	if((trim($as_ced_bene)=="")&&($as_tipo=="B"))
	{
		$this->io_msg->message("Debe registrar la c�dula del beneficiario.2");
		return false;	
	}
	...
	if($ls_mensaje!="")
	{
		...
		if($lb_valido)
		{
			...
			if($lb_valido)
			{
				...
				{
				 $lb_valido =  false;
				 $this->io_msg->message("El detalle ya se encuentra registrado pero con operaci�n distinta, verifique por favor");
				}
			}
		}
		else
...
/**********************************************************************************************************************************/
   function uf_insert_movimiento_spg($estprog,$as_cuenta,$as_procede_doc,$as_documento,$as_operacion,$as_descripcion,$ad_monto_actual,$as_codfuefin="--")
   {
	   ...
	   {
		  $lb_valido=false;
		  $this->is_msg_error = "Error de SQL m�todo->uf_spg_insert_movimiento class->xspg ::".$this->io_function->uf_convertirmsg($this->io_sql->message);
...
/**********************************************************************************************************************************/
	...
	/////////////////////////////////////////////////////////////////////////////	
	    ...
	    if($rs_data===false)
	    {
   	 	   $this->is_msg_error="Error de SQL m�todo->uf_spg_obtener_orden_movimiento class->xspg ::".$this->io_function->uf_convertirmsg($this->io_sql->message);
		   print $this->io_sql->message;
		   return false;
	    }
...
/**********************************************************************************************************************************/
	...
	//              $as_operacion->operacion debe haber,$adec_monto->mnto movimiento
	//	  Returns:  Boolean
	//Description:  Este m�todo registra un movimiento contable (M�todo Principal MAIN )
	////////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_desproc="";	
		$li_orden=0;
		$this->is_codemp      = $as_codemp;
...
/**********************************************************************************************************************************/
	...
	//	  Returns:  boolean
	//Description:  Este m�todo verifica si existe o no el movimiento contable
	////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_existe = false;
		$ld_fecha=$this->io_function->uf_convertirdatetobd($this->id_fecha);
		...
		if($rs_mov===false)	{  $this->is_msg_error = "Error en el m�todo uf_scg_select_movimiento ".$this->io_function->uf_convertirmsg($this->io_sql->message);	}
		else
		{
...
/**********************************************************************************************************************************/
	...
	// 	   Access:  public
	//	  Returns:  boolean
	//Description:  Este m�todo elimina el movimineto contable
	////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido= true;
		$ls_fecha = $this->io_function->uf_convertirdatetobd($as_fecha);
		...
		if($li_result===false)
		{
			$this->is_msg_error = "Error en m�todo uf_scg_delete_movimiento ".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
	    return $lb_valido;
...
/**********************************************************************************************************************************/
	...
	// 	   Access:  public
	//  Arguments:  $as_cuenta->cuenta contable ,$as_procede_doc->procede documento,$as_documento->n� documento
	//              $as_operacion->operacion debe haber,$adec_monto->mnto movimiento
	//	  Returns:  Boolean
	//Description:  Este m�todo registra un movimiento final contable enla tabla movimiento  (DEPENDE DEL PROCESAR)
	////////////////////////////////////////////////////////////////////////////////////////////////////
	function uf_scg_insert_movimiento( $as_cuenta, $as_procede_doc, $as_documento, $as_debhab, $as_descripcion, $adec_monto )
	{
		...
		if($li_result===false)
		{
		   ...
		   {
			   $this->is_msg_error = "Error en m�todo uf_scg_insert_movimiento, Fallo alguna clave foranea";
		   }
		   else
		   {
		   		$this->is_msg_error = "Error en m�todo uf_scg_insert_movimiento ".$this->io_function->uf_convertirmsg($this->io_sql->message);
		   }
		   //print $this->io_sql->message;
		   $lb_valido=false;
		}
...
/**********************************************************************************************************************************/
	...
	function uf_spg_select_movimiento($estprog,$as_cuenta,$as_procede_doc,$as_documento,$as_operacion,&$adec_monto,&$ai_orden)
	{
		...
		if($rs_data===false)
		{
   	 	    $this->is_msg_error="Error de SQL m�todo->uf_spg_select_movimiento class->xspg ::".$this->io_function->uf_convertirmsg($this->io_sql->message);
		    return false;
		}
		else
		{
...
/**********************************************************************************************************************************/
	...
	function uf_spg_comprobante_select()
	{
	    ...
	    if($rs_data===false)
	    {
   	 	   $this->is_msg_error="Error de SQL m�todo->uf_spg_comprobante_select class->xspg ::".$this->io_function->uf_convertirmsg($this->io_sql->message);
  	       return false;
	    }
...
/**********************************************************************************************************************************/
    ...
    //      Access: public
    //     Returns: retorna valido
    // Description: Este m�todo actualiza si existe el comprobante SIGESP_cmp
    /////////////////////////////////////////////////////////////////////////////////////////////////////
	function uf_spg_comprobante_update($li_montoanterior, $li_montoactual)
	{
	   ...
	   if($li_exec===false)
	   {
 	      $this->is_msg_error="Error de SQL m�todo->uf_spg_comprobante_update class->xspg ::".$this->io_function->uf_convertirmsg($this->io_sql->message);
		  $lb_valido=false;
	   }	   
	   return $lb_valido;
	} // function uf_spg_comprobante_update()
/**********************************************************************************************************************************/
	...
	function  uf_spg_comprobante_insert($ai_monto, $ai_tipocomp)
	{
		...
		if($li_exec===false)
		{
 	       $this->is_msg_error="Error de SQL m�todo->uf_spg_comprobante_insert class->class_sigesp_int_spg ::".$this->io_function->uf_convertirmsg($this->io_sql->message);
		   //print $this->io_sql->message;
		   $lb_valido=false;
		}
	return $lb_valido;
...
function uf_spg_valida_datos_movimiento($as_cuenta,$as_descripcion,$as_documento,$adec_monto)
{
	...
	{
		$this->is_msg_error = "Registre la Descripci�n del Movimiento." ;
		return false;
	}
...
/**********************************************************************************************************************************/
	...
	// 	   Access:  public
	//	  Returns:  integer
	//Description:  Este m�todo genera un numero de orden secuencial de los movimiento 
	////////////////////////////////////////////////////////////////////////////////////////////////////
	function uf_scg_obtener_orden_movimiento()
	{
		$li_orden=0;
		...
		if($rs_saldos==false)
		{
		  $this->is_msg_error = "Error en el m�todo uf_scg_obtener_orden_movimiento ".$this->io_sql->message;
		  $lb_valido=false;
		}
		else
		{
...
/**********************************************************************************************************************************/
	...
	function uf_spg_delete_movimiento($estprog, $as_cuenta, $as_procede_doc, $as_documento, $as_operacion)
	{
	   ...
	   $ls_sql = " DELETE FROM spg_dtmp_cmp ".
				 ...
				 "        estcla='".$estprog[5]."' AND     ".     // CAMBIO DEL TAMA�O DE ESTRUCTURA PROGRAMATICA
...
/**********************************************************************************************************************************/
 ...
 //---------------------------------------------------------------------------------------------------------------------------------
	...
	//	Arguments:  $as_codemp-> empresa,$as_procedencia->procedencia,$as_comprobante->comprobante,$as_fecha
	//	  Returns:	booleano lb_existe
	//Description:  M�todo que verifica si existe o no el comprobante con fechas distintas
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_load_fuentes_financiamiento($as_codemp)
{
  ...
  if ($rs_data===false)
  {
	   $this->is_msg_error="Error.CLASS->sigesp_spg_c_mod_presupuestarias.php.-M�todo->uf_load_fuentes_financiamiento ".$this->io_function->uf_convertirmsg($this->io_sql->message);
	   $this->io_msg->message($this->is_msg_error);
	   return false;
  }
  return $rs_data;
...
function uf_load_unidades_administradoras($as_codemp)
{
  ...
  if ($rs_datos===false)
  {
	   $this->is_msg_error="Error.CLASS->sigesp_spg_c_mod_presupuestarias.php.-M�todo->uf_load_unidades_administradoras ".$this->io_function->uf_convertirmsg($this->io_sql->message);
	   $this->io_msg->message($this->is_msg_error);
	   return false;
  }
  return $rs_datos;
...
function uf_update_bsf_sigespcmpmd($ad_monto,$as_codemp,$as_procede,$as_comprobante,$ad_fecha,$aa_seguridad)
{
	...
	// Fecha Creaci�n :  24/09/2007                 Fecha �ltima Modificacion :        
	/////////////////////////////////////////////////////////////////////////////////////////
	$lb_valido=true;
                                  ...
                                  $as_spg_cuenta,$as_procede_doc,$as_documento,$as_operacion,$aa_seguridad)
{
	...
	$ls_estcla=$as_codestpro[5];  // CAMBIO TAMA�O DE LA ESTRUCTURA PROGRAMATICA
...
function uf_convertir_sigespcmpmd($as_codemp,$aa_seguridad)
{
	...
	if($rs_data===false)
	{ 
		$this->io_mensajes->message("CLASE->sigesp_rcm_c_cfg M�TODO->SELECT->uf_convertir_sigespcmpmd ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
		$lb_valido=false;
	}
	else
	{
...
function uf_get_nomestructura($as_codemp)
{
  ...
  if ($rs_data===false)
  {
	   $this->is_msg_error="Error.CLASS->sigesp_spg_c_mod_presupuestarias.php.-M�todo->uf_load_nomestructura ".$this->io_function->uf_convertirmsg($this->io_sql->message);
	   $this->io_msg->message($this->is_msg_error);
	   return false;
  }
  else
...
function uf_select_comprobantes_spg($as_codcmpdes,$as_codcmphas,$ad_fecha,$ad_fecaprsol,$as_concepto,$as_procede,$ai_estatus,&$ao_object,&$ai_totrows)
	{
		...
		//				   ai_estatus  // Estatus de Contabilizaci�n
		...
		{
			$lb_valido=false;
            $this->io_msg->message("CLASE->Modificaciones_Presupuestarias M�TODO->uf_select_comprobantes_spg ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
		}
		else
		{
	...
	//	   Access:  public
	//	  Returns:	rs_data
	//Description:  M�todo que devuelve los nombres de las Base de Datos para la Consolidaci�n
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//	         Returns:  $lb_valido.
//	     Description:  Funci�n que se encarga de obtener los valores de la validacion
//                     para los Traspasos Presupuestarios 
//     Elaborado Por:  Ing. Arnaldo Su�rez.
// Fecha de Creaci�n:  10/09/2008       Fecha �ltima Actualizaci�n:
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
...
//////-------------------------------------------------------------------------------------------------------------------------------------
	...
	// Fecha de Creaci�n:  21/11/2008       Fecha �ltima Actualizaci�n:
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_valor=0;
...
//-------------------------------------------------------------------------------------------------------------------------------------
 ...
 function uf_sigesp_existe_tipmodpre($as_codtipmodpre)
	...
	//	          Access:  public
	//	       Arguments: 
	//       $as_codtipmodpre :  C�digo del Tipo de Modificacion Presupuestaria
	//	         Returns:  $lb_valido.
	//	     Description:	                     
	//     Elaborado Por:  Ing. Arnaldo Su�rez
	// Fecha de Creaci�n:  30/12/2008       Fecha �ltima Actualizaci�n:
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_existe=false;
		$ls_sql=" Select * ".
		        "   from spg_tipomodificacion   ".
...
///--------------------------------------------------------------------------------------------------------------------------------------
 ...
 {  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//	          Access:  public
	//	       Arguments: $as_codtipmodpre : C�digo del Tipo de Modificacion Presupuestaria
	//                    $as_dentipmodpre : Denominaci�n del Tipo de Modificacion Presupuestaria   
	...
	//     Elaborado Por:  Ing. Arnaldo Su�rez
	// Fecha de Creaci�n:  06/01/2009       Fecha �ltima Actualizaci�n:
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_existe=false;
...
///--------------------------------------------------------------------------------------------------------------------------------------
 ...
 function uf_load_fuentes_financiamiento_estructura($as_estpro,$as_codigo="%%",$as_denominacion="%%")
 {
  ...
  if ($rs_data===false)
  {
	   $this->is_msg_error="Error.CLASS->sigesp_spg_c_mod_presupuestarias.php.-M�todo->uf_load_fuentes_financiamiento_estructura ".$this->io_function->uf_convertirmsg($this->io_sql->message);
	   $this->io_msg->message($this->is_msg_error);
	   return false;
  }
  return $rs_data;
...
//--------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 {  
		...
		{  
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la distribucion mensual del comprobante ".$as_comprobante."  de procede ".$as_procede." Asociado a la empresa ".$this->li_codemp;
...
//--------------------------------------------------------------------------------------------------------------------------------------
									...
									$as_codestpro4,$as_codestpro5,$as_estcla,$as_cuentaspg,$as_operacion,$aa_seguridad)
	{
		...
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="DELETE";
			$ls_descripcion ="Elimin� la distribucion mensual del comprobante ".$as_comprobante."  de procede ".$as_procede." Asociado a la empresa ".$this->li_codemp;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
