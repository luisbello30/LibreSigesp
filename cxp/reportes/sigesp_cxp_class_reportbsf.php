...
class sigesp_cxp_class_reportbsf
{
	...
	function uf_select_report_libcompra($ld_fecdesde,$ld_fechasta,$rs_data)
	{
		...
		$ls_sql= "SELECT RD.numrecdoc,--RD.tipproben,
			RD.cod_pro,RD.ced_bene,MAX(RD.montotdocaux) AS montotdoc,MAX(RD.mondeddocaux) AS mondeddoc,RD.codtipdoc,MAX(RD.fecemidoc) AS fecemidoc,MAX(RD.numref) AS numref,SUM(COALESCE(RDDED.monret,0)) as monret,MAX(DED.iva) AS iva
				 FROM sigesp_deducciones DED,cxp_rd RD
				 LEFT OUTER JOIN cxp_rd_deducciones RDDED
