...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_solicitud($as_numsol)
	{
		...
		$ls_sql="SELECT cxp_solicitudes.numsol, cxp_solicitudes.cod_pro, cxp_solicitudes.ced_bene, cxp_solicitudes.codfuefin,".
				...
				"                                         AND rpc_beneficiario.ced_bene=cxp_solicitudes.ced_bene) ".
				"                       ELSE '-' END ) AS rifpro, ".
                                "       (CASE cxp_solicitudes.tipproben WHEN 'P' THEN (SELECT rpc_proveedor.nacpro ".
				"                                        FROM rpc_proveedor ".
				"                                       WHERE rpc_proveedor.codemp=cxp_solicitudes.codemp ".
				"                                         AND rpc_proveedor.cod_pro=cxp_solicitudes.cod_pro) ".
	...
	function uf_select_compromiso_afectado($as_numsol)
	{
		...
		if ($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_compromiso_afectado ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_detalleconrecdoc_scg($as_numsol)
	{
		...
		if ($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_detalleconrecdoc_scg ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_proveedores($as_tipproben,$as_codproben)
	{
		...
		//                 as_codprobendes  // Codigo de Proveedor/Beneficiario Desde
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: Funci�n que busca la informaci�n de los proveedores.
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_proveedores ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		else
		{
	...
	function uf_select_informacionpagosprevios($as_tipproben,$as_codpro,$as_cedbene,$ad_fecregdes,$ad_fecreghas,&$ad_totpagosprevios)
	{
		...
		if ($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_informacionpagosprevios ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
									 ...
									 $ai_contabilizada,$ai_anulada,$ai_propago,$ai_pagada)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_solicitudesf1 ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_retencionesislr_cxp($as_numsol)
	{
	   ...
	   /*$ls_sql="SELECT cxp_rd.numrecdoc AS numdoc, cxp_rd.numref, cxp_rd.fecemidoc, cxp_rd.tipproben, rpc_proveedor.nitpro AS nit, ".
	    ...
	    $ls_sql="SELECT cxp_rd.numrecdoc AS numdoc, cxp_rd.numref, cxp_rd.fecemidoc, cxp_rd.fecregdoc, cxp_rd.tipproben, ".
			   ...
			   "       cxp_dt_cmp_islr.numcmpislr,".
			   "	   (SELECT rpc_tipo_organizacion.dentipoorg".
	...
	/*function uf_select_retenciones($as_codded,$as_coddedhas,$as_tipded="T")
	{
		...
		//	    		   as_coddedhas     // C�digo de Deduccion final del intervalo
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: Funci�n que se encarga de traer las deducciones en el intervalo indicado
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 20/06/2008									Fecha �ltima Modificaci�n :  20/06/2008
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$this->DS=new class_datastore();
		switch($as_tipded)
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	/*function uf_retencionesespecifico($as_codded,$as_coddedhas,$as_tipproben,$as_codprobenhas,$as_codprobendes,$ad_fecdes,$ad_fechas,$as_tipper="",$as_codope="")
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 10/07/2007									Fecha �ltima Modificaci�n :  20/06/2008
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_gestor = $_SESSION["ls_gestor"];
		$lb_valido=true;
		$ls_criterio="";
		...
		switch ($ls_gestor)
		{
			...
			case 'ANYWHERE':
			   ...
			   break;
                ...
                if($as_codope !="")
                {
                    $re_sql     = " cxp_sol_banco.codope,";
                    $re_sql2    = " ,cxp_sol_banco";
                    $re_sql3    = " ,cxp_sol_banco.codope ";
                    $re_sql4    = " AND cxp_sol_banco.codemp = cxp_solicitudes.codemp
                                    AND cxp_sol_banco.numsol = cxp_solicitudes.numsol
                                    AND cxp_sol_banco.codope = '".$as_codope."'";
                }
		$ls_sql="SELECT cxp_solicitudes.numsol, MAX(rpc_proveedor.nitpro) AS nitpro,".$re_sql." MAX(cxp_solicitudes.tipproben) AS tipproben, MAX(cxp_solicitudes.fecemisol) AS fecemisol, ".
				"       MAX(rtrim(cxp_solicitudes.consol)) AS concepto, MAX(cxp_solicitudes.monsol) AS monsol, MAX(cxp_solicitudes.estprosol) AS estprosol, ".
				...
				"  FROM cxp_solicitudes,cxp_dt_solicitudes,cxp_rd_deducciones,rpc_proveedor,rpc_beneficiario,cxp_rd ".$re_sql2." ".
			   "  WHERE cxp_solicitudes.codemp = '".$this->ls_codemp."' ".
			   "    AND (cxp_solicitudes.estprosol = 'E' OR cxp_solicitudes.estprosol='C' OR cxp_solicitudes.estprosol='S' OR cxp_solicitudes.estprosol='P') ".
			   "    AND cxp_solicitudes.fecemisol >= '".$ad_fecdes."' ".
			   "    AND cxp_solicitudes.fecemisol <= '".$ad_fechas."' ".
			   $ls_criterio.
				...
				"   AND cxp_dt_solicitudes.codtipdoc = cxp_rd.codtipdoc ".
				"   AND cxp_dt_solicitudes.cod_pro = cxp_rd.cod_pro ".
				"   AND cxp_dt_solicitudes.ced_bene = cxp_rd.ced_bene ".$re_sql4." ".
				" GROUP BY cxp_solicitudes.numsol ".$re_sql3." ".
				" ORDER BY cxp_solicitudes.numsol";
              //echo $ls_sql."<hr>";die();
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
	...
	function uf_retencionesespecifico_detalle($as_codded,$as_numsol)
	{
		...
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: Funci�n que se encarga de extraer todas aquellas recepciones de las solicitudes de pago
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 17/02/2010									Fecha �ltima Modificaci�n :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_retencionesespecifico_detalle ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $lb_valido;
									  		...
									  		$ad_fechas,$as_tipper="")
	{
		...
		if($this->rs_detalle===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_retencionesespecificomanual ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $lb_valido;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
        ...
        function uf_retencionesislr_proveedor($as_numcom)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesiva_proveedor
		//         Access: public
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
	    $ls_sql="SELECT numcom, codret, fecrep, perfiscal, codsujret, nomsujret, dirsujret, estcmpret, ".
			 	"        (SELECT telpro ".
				" 		    FROM rpc_proveedor ".
				"		   WHERE rpc_proveedor.codemp=scb_cmp_ret.codemp ".
				"			 AND rpc_proveedor.cod_pro=scb_cmp_ret.codsujret) AS telpro,
                                        (SELECT telbene ".
				" 		    FROM rpc_beneficiario".
				"		   WHERE rpc_beneficiario.codemp=scb_cmp_ret.codemp ".
				"			 AND rpc_beneficiario.ced_bene=scb_cmp_ret.codsujret) AS telbene,
                                        
                                    (SELECT cod_pro FROM rpc_proveedor WHERE rpc_proveedor.codemp=scb_cmp_ret.codemp AND rpc_proveedor.cod_pro=scb_cmp_ret.codsujret) AS cod_pro,

                                    (SELECT rifben FROM rpc_beneficiario WHERE rpc_beneficiario.codemp=scb_cmp_ret.codemp AND rpc_beneficiario.ced_bene=scb_cmp_ret.codsujret) AS rifben, ".

                                 "(SELECT rifpro FROM rpc_proveedor WHERE rpc_proveedor.codemp=scb_cmp_ret.codemp AND rpc_proveedor.cod_pro=scb_cmp_ret.codsujret) AS rifpro,".

                                 "   (SELECT ced_bene FROM rpc_beneficiario WHERE rpc_beneficiario.codemp=scb_cmp_ret.codemp AND rpc_beneficiario.ced_bene=scb_cmp_ret.codsujret) AS ced_bene ".
				"  FROM scb_cmp_ret ".
				" WHERE codemp='".$this->ls_codemp."'".
				"   AND numcom = '".$as_numcom."'".
	...
	function uf_retencionesislr_detalle($as_numcom)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesiva_detalle
		//         Access: public
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT max(codret) as codret, codded, max(numcom) as numcom, max(numope) as numope, max(fecfac) as fecfac, ".
				"		 max(numfac) as numfac, max(numcon) as numcon, max(numnd) as numnd, max(numnc) as numnc, ".
				"		 max(tiptrans) as tiptrans, SUM(totcmp_sin_iva) as totcmp_sin_iva , max(totcmp_con_iva) as totcmp_con_iva, ".
				"		 SUM(basimp) as basimp, porimp, SUM(totimp) as totimp, SUM(iva_ret) as iva_ret, max(desope) as desope, ".
				"		 max(numsop) as numsop, max(codban) as codban, max(ctaban) as ctaban, max(numdoc) as numdoc, max(codope) as codope ".
				...
				"   AND numcom='".$as_numcom."' ".
				"	AND codret='0000000002' ".
				" GROUP BY codemp, numfac, porimp, numnd,numnc,codded".
				" ORDER BY numope ";
		$rs_data=$this->io_sql->select($ls_sql);
               // echo $ls_sql."<hr>"; //die();
		if($rs_data===false)
	...
	function uf_retencionesaporte_proveedor($as_numcom)
	{
		...
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 21/10/08									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
	    $ls_sql="SELECT numcom, codret, fecrep, perfiscal, codsujret, nomsujret, rif, dirsujret, estcmpret, ".
			 	"        (SELECT telpro ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_retencionesaporte_proveedor ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_retencionesunoxmil_detalles($as_numcom)
	{
		...
		/*$ls_sql="SELECT max(scb_dt_cmp_ret.codret) as codret, ".
				...
				"	AND cxp_dt_solicitudes.numsol=cxp_solicitudes.numsol ".
				"  GROUP BY scb_dt_cmp_ret.codemp, scb_dt_cmp_ret.numfac, scb_dt_cmp_ret.porimp,scb_dt_cmp_ret.basimp,scb_dt_cmp_ret.totimp,scb_dt_cmp_ret.iva_ret,scb_dt_cmp_ret.totcmp_con_iva ";*/
		$rs_data=$this->io_sql->select($ls_sql);//echo $ls_sql.'<hr>';
		//print_r($rs_data);
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_retencionesmunicipales_detalles ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	...
	function uf_retencionesmunicipales_detalle_solpago($as_numcom)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_retencionesmunicipales_detalle_solpago ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_retencion1x1000_detalle_solpago($as_numcom)
	{
		...
		//    Description: Funci�n que busca la informaci�n de los comprobantes municipales
		//	   Creado Por: 
		// Fecha Creaci�n: 11/11/2009									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT max(scb_dt_cmp_ret.codret) as codret,
		                max(scb_dt_cmp_ret.numsop) as numsop,
	...
	function uf_select_report_libcompra($ld_fecdesde,$ld_fechasta,$rs_data)
	{
		...
		// Fecha Creaci�n: 11/03/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/*$ls_sql="SELECT RD.numrecdoc,RD.tipproben,RD.cod_pro,MAX(RD.montotdoc) AS montotdoc,".
...
//				"       SUM(COALESCE(RDDED.monret,0)) as monret,MAX(DED.iva) AS iva".
				...
				"   AND estlibcom=1 and RD.tipproben='P'".
				"   AND fecregdoc between '".$ld_fecdesde."' AND '".$ld_fechasta."'".
				//"   AND DED.codded=RDDED.codded".
				"   AND RD.estprodoc='C'".
				" GROUP BY RD.numrecdoc,RD.tipproben,RD.cod_pro,RD.codtipdoc".
				" ORDER BY MAX(fecemidoc)";//print $ls_sql;die();*/
...
//				mysql_data_seek( $rs_data,0);//Devuelvo el puntero al comienzo
	...
	function uf_select_informacionpagoscxp($as_tipproben,$as_cedbene,$as_codpro,$ad_fecregdes,$ad_fecreghas,$as_numsol)
	{
		...
		$ls_sql=" SELECT cxp_solicitudes.numsol,cxp_sol_banco.numdoc,cxp_sol_banco.monto,cxp_sol_banco.codope,scb_movbco.fecmov,scb_movbco.conmov, cxp_sol_banco.estmov, ".
				...
				"    AND cxp_solicitudes.ced_bene='".$as_cedbene."'".
				"    AND cxp_solicitudes.cod_pro='".$as_codpro."'".
				"    AND (cxp_historico_solicitud.estprodoc='S' OR cxp_historico_solicitud.estprodoc='P')".
	...
	function uf_select_notacargos($ls_codemp,$ls_numrecdoc,$ls_codtipdoc,$ls_codpro,$ls_cedben,$ls_codope,$ls_numnota,$ls_numsol,$ldec_porcar)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_notacargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_nrocomprobante($as_fac,$as_nrocon,$as_fecha)
	{
		...
		//    Description: Funci�n que busca el numero de  comprobantes ISRL
		//	   Creado Por: Ing. Jennifer Rivero
		// Fecha Creaci�n: 09/01/2008									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_nrocom=0;
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_nrocomprobante ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_iva_retendio_ISLR($as_fac,$as_nrocon,$as_fecha)
	{
		...
		//      Arguments: as_fecha     // fecha de la factura
		//	      Returns: lb_valido True si encontro un numero de comprobante
		//    Description: Funci�n que busca el iva retenido
		//	   Creado Por: Ing. Jennifer Rivero
		// Fecha Creaci�n: 09/01/2008									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_iva_retendio_ISLR ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
    ...
    function uf_fecha_ISLR($as_fac,$as_nrocon,$as_fecha)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_fecha_ISLR ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_det_deducciones($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		//                 as_codtipdoc  // Codigo de Tipo de Documento
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: Funci�n que busca las retenciones de IVA.
		...
		if ($rs===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_det_deducciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_det_deducciones_solpag($as_numsol)
	{
		...
		if ($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_det_deducciones_solpag ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_datos_cheque_retencion($as_numsol,&$as_nummov,&$ad_fecmov,&$as_monto)
	{
		...
		//    Description: Funci�n que busca los datos del cheque de la retencion del impuesto de 1 x 100
...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_beneficiarios_libro_islr($as_mes,$as_anio,&$rs_data)
	{
		...
		//	    		   as_anio   // a�o del comprobante
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: Funci�n que busca la informaci�n de los comprobantes de impuesto sobre la renta
		//	   Creado Por: Ing. Mar�a Beatriz Unda
		// Fecha Creaci�n: 26/11/2008									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_criterio="";
		$ls_criterio2="";
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_beneficiarios_libro_islr ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_solicitudesubicar($as_tipproben,$as_codprobendes,$as_codprobenhas,$as_numsoldes,$as_numsolhas,&$lb_valido)
	{
		...
		//                 as_numsolhas     // Numero de Solicitud Hasta
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: funci�n que busca la informaci�n de las solicitudes de pago en los intervalos indicados
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 03/06/2007									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_solicitudesubicar ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $rs_data;
	...
	function uf_load_ubicacionsol($as_numsol,&$lb_valido)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_load_ubicacionsol ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		return $rs_data;
								   			 ...
								   			 $ai_registrada,$ai_anulada,$ai_procesada,$as_orden,$as_numrecdoc)
	{
		...
		//                 ai_anulada       // Estatus de la Recepcion Anulada
		//                 ai_procesada     // Estatus de la Recepcion Procesada
		//                 as_numrecdoc     // Nro de Recepci�n de documentos
		//                 ai_orden         // Orden de los Datos en el Reporte Numero/Fecha
		//	      Returns: lb_valido True si se creo el Data stored correctamente � False si no se creo
		//    Description: funci�n que busca la informaci�n de las ubicaciones de recepciones de documentos en los intervalos indicados
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 31/03/2009									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_criterio="";
		...
		if($this->rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_select_ubicacion_recepciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			//$lb_valido=false;
		}
		else
		{
	...
	function uf_sep_select_usuario($as_codusu,&$as_nomusu)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		 ...
		 {
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_sep_select_usuario ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		 }
		 else
		 {
			 if($row=$this->io_sql->fetch_row($rs))
	...
	function uf_load_beneficiario_alterno($as_numsol)
	{
		...
		//                 $as_numdoc = N�mero del Documento.
		//                 $as_codope = C�digo de la Operaci�n.
		...
		//                 que manejen la ayuda econ�mica.
		//     Creado por: Ing. N�stor Falc�n.
		// Fecha Creaci�n: 26/08/2008
		///////////////////////////////////////////////////////////////////////////////////////////////
		$ls_nombenalt = "";
		...
		for($li_i=$ls_mesdesaux;$li_i<=$ls_meshasaux;$li_i++)
		{
			...
			while(!$rs_datac->EOF)
			{
				...
				if ($ls_procedencia=='CXP')
				{
					...
					{
                                                //echo $ls_numrecdoc." <-aqui <hr>";
						$ls_cadena=$ls_cadena."La factura ".$ls_numrecdoc." no se pudo agregar ya que la deducci�n no posee Concepto de Retenci�n asociado. \r\n";
						$correcto=false;
					}
				}
				if ($ls_procedencia=='SNO')
				{
					...
					if ((trim($ls_codconret)==""))
					{
						$ls_cadena=$ls_cadena."El personal  ".$ls_codper." no se pudo agregar ya que la deducci�n no posee Concepto de Retenci�n asociado. \r\n";
						$correcto=false;
					}
					if (($li_porded==0))
					{
						$ls_cadena=$ls_cadena."El personal  ".$ls_codper." no se pudo agregar ya que el porcentaje de deducci�n es cero. \r\n";
						$correcto=false;
					}
				}
				if($correcto)
			...
			if($lb_valido)
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="PROCESS";
				$ls_descripcion ="Genero el xml de Declaraci�n de sueldos y otras remuneraciones para el periodo ".
								 $as_mesdes." a ".$as_meshas." del a�o ".$as_year." en el Archivo ".$ls_archivo.
	...
	function uf_declaracion_xml_cabecera($as_fecemidocdes,$as_fecemidochas,$as_periodo,$as_year)
	{
		...
		//	   Creado Por: Ing. Yesenia Moreno / Ing. Luis Lang
		// Fecha Creaci�n: 05/06/2009									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_criterio="";
		$rs_data="";
		if($as_fecemidocdes!="")
		...
		$ls_sql="SELECT '' AS codper, rpc_proveedor.rifpro, rpc_beneficiario.rifben,cxp_rd.numrecdoc, cxp_rd.numref,".
				...
				"   AND substr(cast(fecdesper as char(10)),1,4) = '".$as_year."' ".
				"   AND substr(cast(fecdesper as char(10)),1,4) = '".$as_periodo."' ".
...
// echo $ls_sql; die();
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_declaracion_xml_cabecera ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		return $rs_data;
	}// end function uf_arc_cabecera
	...
	function uf_obtener_clasificador()
	{
		...
		{
			$this->io_mensajes->message("CLASE->Report M�TODO->uf_obtener_clasificador ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		else
		{
