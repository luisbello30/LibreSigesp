<?php
	...
	function uf_print_estructura1()
   	{
		...
		else
		{
			print "<table width=500 border=0 cellpadding=1 cellspacing=1 class=fondo-tabla align=center>";
			print "<tr class=titulo-celda>";
			print "<td style='cursor:pointer' title='Ordenar por C�digo' align='center' onClick=ue_orden('codestpro1')>C�digo </td>";
			print "<td style='cursor:pointer' title='Ordenar por Denominaci�n' align='center' onClick=ue_orden('denestpro1')>Denominaci�n</td>";
	...
	function uf_print_estructura2()
   	{
		...
		else
		{
			...
			print "<td width=150 style='cursor:pointer' title='Ordenar por C�digo' align='center' onClick=ue_orden('codestpro2')>C�digo</td>";
			print "<td width=200 style='cursor:pointer' title='Ordenar por Denominaci�n' align='center' onClick=ue_orden('denestpro2')>Denominaci�n</td>";
	...
	function uf_print_estructura3()
   	{
		...
		else
		{
			...
			print "<td width=100 style='cursor:pointer' title='Ordenar por C�digo' align='center' onClick=ue_orden('spg_ep3.codestpro3')>C�digo</td>";
			print "<td width=150 style='cursor:pointer' title='Ordenar por Denominaci�n' align='center' onClick=ue_orden('spg_ep3.denestpro3')>Denominaci�n</td>";
			print "<td width=50 style='cursor:pointer' title='Ordenar por Estatus' align='center' onClick=ue_orden('estcla')>Estatus</td>";
			print "</tr>";
			while($row=$io_sql->fetch_row($rs_data))
			{
	...
	function uf_print_estructura4()
   	{
		...
		else
		{
			...
			print "<td width=80 style='cursor:pointer' title='Ordenar por C�digo' align='center' onClick=ue_orden('codestpro4')>C�digo</td>";
			print "<td width=130 style='cursor:pointer' title='Ordenar por Denominaci�n' align='center' onClick=ue_orden('denestpro4')>Denominaci�n</td>";
			print "<td width=50 style='cursor:pointer' title='Ordenar por Estatus' align='center' onClick=ue_orden('estcla')>Estatus</td>";
			print "</tr>";
			while($row=$io_sql->fetch_row($rs_data))
			{
	...
	function uf_print_estructura5()
   	{
		...
		else
		{
			...
			print "<td width=70 style='cursor:pointer' title='Ordenar por C�digo' align='center' onClick=ue_orden('spg_ep5.codestpro5')>C�digo</td>";
			print "<td width=100 style='cursor:pointer' title='Ordenar por Denominaci�n' align='center' onClick=ue_orden('spg_ep5.denestpro5')>Denominaci�n</td>";
			print "<td width=50 style='cursor:pointer' title='Ordenar por Estatus' align='center' onClick=ue_orden('estcla')>Estatus</td>";
			print "</tr>";
			while($row=$io_sql->fetch_row($rs_data))
			{
		...
		else
		{
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				$li_estaposol=$row["retaposol"];
                                $li_unopormil = $row["estretmil"];
				$ls_activo=""; 
				$li_monobjret=0;
				$li_monret="0,00";
				if(($li_islr=='1')||($li_estretmun=='1')||($li_estaposol=='1')||($li_unopormil=='1'))
				{
				...
				{
                                       // echo $ls_codded.'uno por mi1<br>';
					$li_monobjret=$li_cargos;
				}
                              // echo $li_monobjret.'<br>';
				...
				if ($ls_tipo!='CMPRETIVA' && $ls_tipo!='CMPRETMUN' && $ls_tipo!='CMPRETAPO' && $ls_tipo!='CMPRETMIL')
				{
	...
	function uf_print_recepcion()
   	{
        ...
        $ls_sql="SELECT trim(numrecdoc) as numrecdoc, cxp_rd.codtipdoc, trim(ced_bene) as ced_bene, cod_pro, codcla, dencondoc, 
				...
				"       coduniadm,codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,estcla,estact,numordpagmin,codtipfon,repcajchi,".
		...
		else
		{
			...
			print "<tr class=titulo-celda>";
			print "<td style='cursor:pointer' title='Ordenar por Recepci�n' align='center' onClick=ue_orden('numrecdoc')>Nro Recepci�n</td>";
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($ls_tipo)
				{
					case "":
						...
						print "'".$ls_codestpro5."','".$ls_estcla."','".$ls_estact."','".$ls_numordpagmin."','".$ls_codtipfon."','".$ls_repcajchi."');\">".$ls_numrecdoc."</a></td>";
					...
					case "SOLICITUDPAGO":
						...
						print "<tr class=celdas-blancas>";
						print "<td align='center'><a href=\"javascript: ue_aceptar_solicitud('".$ls_numrecdoc."','".$ls_dencondoc."','".$ls_codtipdoc."','".$ls_dentipdoc."','".$li_montotdoc."','".$ls_cedbene."','".$ls_codpro."','".$ls_codfuefin."','".$ls_denfuefin."');\">".$ls_numrecdoc."</a></td>";
						print "<td with='200'>".$ls_nombre."</td>";
						print "<td with='100' align='center'>".$ld_fecregdoc."</td>";
						print "<td with='100' align='center'>".$ls_estatus."</td>";
			...
			if ($lb_valido)
			{
				...
				if($li_totrow>0)
				{
					...
					while((!$io_recepcion->rs_compromisos->EOF)&&($lb_valido))
					{
						...
						$li_monimpcon=0;
						$ls_numcomanu="";
						if(($ls_varcomrd=="1")&&($ls_procede=="SOCCOC"||$ls_procede=="SOCCOS"))//no
						{
						...
						if($ls_valcompromiso)
						{
							...
							if($lb_valido)
							{	//$li_total es el monto aprobado en la SEP
								//$li_monto_causado
								//$li_monto_totanulado
								//$li_monto_recepcion
	//							$li_disponible=($li_total+$li_monto_ajuste)-$li_monto_causado+$li_monto_anulado-$li_monto_recepcion-$li_monto_cargo;
								 //  $li_monto_recepcion ES MAYOR A 0 CUANDO SE TIENE UNA RECEPCION DE DOCUMENTO YA RELACIONADA CON LA SEP
								//print" DISPONIBLE->".$li_disponible."=(".$li_total." +".$li_monimpcon."+".$li_monto_ajuste." )-(".$li_monto_causado." +".$li_monto_totanulado." )-".$li_monto_recepcion."<br><br>"; 
	 					...
	 					//print" DISPONIBLE->".$li_disponible." TOTAL->".$li_total." AJUSTE->".$li_monto_ajuste." CAUSADO->".$li_monto_causado." Anulado->".$li_monto_anulado." Recepcion->".$li_monto_recepcion."<br><br>";
								...
								if($li_disponible>0)
								{
									$lb_valido=$io_recepcion->uf_load_acumulado_solicitudes($ls_numdoc,$ls_codtipdoc,$ls_codprov,$ls_cedbene,&$li_monto_solicitud);
									      //$li_monto_solicitud es el monto de la solicitud de pago asociada a la sep
									if($lb_valido)
									{
	...
	function uf_print_solicitudespago()
	{
		...
		$ls_sql="SELECT cxp_solicitudes.numsol, cxp_solicitudes.cod_pro, cxp_solicitudes.ced_bene, cxp_solicitudes.codfuefin,".
				...
				"       cxp_solicitudes.codtipfon,cxp_solicitudes.repcajchi,".
		...
		else
		{
			...
			print "<tr class=titulo-celda>";
			print "<td width=60  style='cursor:pointer' title='Ordenar por Numero de Solicitud'       align='center' onClick=ue_orden('numsol')>N�mero de Solicitud</td>";
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($ls_tipo)
				{
					case "":
						...
						print "'$ls_estatus','$ls_tipo_destino','$li_i','$ls_rifproben','$ls_numordpagmin','$ls_codtipfon','$ls_repcajchi');\">".$ls_numsol."</a></td>";
	...
	function uf_print_retencionesislr()
	{
		...
		unset($io_funciones);
		unset($ls_codemp);*/
                //busco en los comprobantes de retecnion de i.s.l.r en la tabla de scb_cmp_ret, ya que los mismos se deben generar por el modulo de
                //de cuentas por pagar igual que los otros comprobantes
               global $io_grid;
		...
		$ls_sql="SELECT DISTINCT scb_cmp_ret.numcom, scb_cmp_ret.fecrep, scb_cmp_ret.nomsujret ".
				...
				"   AND scb_cmp_ret.codret = '0000000002' ".
		...
		$rs_data=$io_sql->select($ls_sql);//ECHO $ls_sql;DIE;
		if($rs_data===false)
		{
...
/*		$ls_codprobendes=$_POST['codprobendes'];
	...
	function uf_print_retencionesaporte()
	{
		...
		//		   Access: private
		//	    Arguments: 
		//	  Description: Funci�n que obtiene e imprime los resultados de la busqueda de retenciones de aporte social
		//	   Creado Por: Ing. Yesenia Moreno/ Ing. Luis Lang
		// Fecha Creaci�n: 12/07/2007 								Fecha �ltima Modificaci�n : 
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_grid;
	...
	function uf_print_ordenministerio()
   	{
		...
		else
		{
			 ...
			 while (!$rs_data->EOF)
			{
				...
				$ld_porrepfon    = $rs_data->fields["porrepfon"];//Porcentaje de Reposici�n.
				...
				$ld_monmaxmov = 0;
				$ld_monmaxmov = ($ld_monordpagmin-$ld_totmoncon);//Monto M�ximo de la Operaci�n.(Original - Consumido).
