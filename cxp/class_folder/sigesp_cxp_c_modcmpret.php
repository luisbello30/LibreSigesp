...
class sigesp_cxp_c_modcmpret
 {
	...
	{	
		...
		// Fecha Creaci�n: 18/09/2007 								Fecha �ltima Modificaci�n : 21/09/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_load_dt_cmpret($as_numcom,$as_codret)
	{
		...
		//		   Access: public
		//		 Argument: as_numcom // N�mero del Comprobante
		//		           as_codret // Codigo de la Retencion
		//	  Description: Funci�n que busca los Comprobantes de Retencion
		//	   Creado Por: Ing. Gerardo Cordero
		// Fecha Creaci�n: 21/09/2007								Fecha �ltima Modificaci�n : 21/09/2007
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Modificar Comprobate M�TODO->uf_load_dt_cmpret ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
...
//-------------------------------------------------------------------------------------------------------------------------
	function uf_insert_dt_cmpret($as_numcom,$as_codret, $ai_totrowrecepciones,$as_probene,$as_codigo, $aa_seguridad)
	{
		...
		//		   Access: private
		//	    Arguments: as_numcom            // N�mero del Comprobante 
		//				   as_codret            // C�digo de la retencion
		...
		for($li_i=1;($li_i<$ai_totrowrecepciones)&&($lb_valido);$li_i++)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_insert_dt_cmpret ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}
			else
			{
				/////////////////////////////////           SEGURIDAD             /////////////////////////////////	
				$ls_evento="INSERT";
				$ls_descripcion ="Insert� el Detalle ".$ls_numope." del comprobate ".$as_numcom.
											 ...
											 $as_numdoc,$as_codope)
	{
		...
		if($li_result===false)
		{	
			$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_guardar_detallecmp_consolida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_delete_dt_cmpret($as_numcom,$as_codret, $aa_seguridad)
	{
		...
		//				   as_codret            // Codigo de la retencion
		//				   aa_seguridad         // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto el insert � False si hubo error 
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Modificar Comprobate M�TODO->uf_delete_dt_cmpret ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="DELETE";
			$ls_descripcion ="Elimin� los Detalle del comprobate ".$as_numcom." Asociado a la empresa ".$this->ls_codemp;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////	
 ...
 function uf_buscar_ultimo($as_numcom,$as_codret)
	{
		...
		//		   Access: public
		//		 Argument: as_numcom // N�mero de comprobante
		//				   as_codret // Codigo de la retencion
		//	      Returns: lb_valido True si se ejecuto � False si hubo error 
		//	  Description: Funci�n que busca el ultimo comprobante
		//	   Creado Por: Ing. Gerardo Cordero
		...
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false){
			$this->io_mensajes->message("CLASE->Modificar Comprobate M�TODO->uf_buscar_ultimo ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else{
...
//----------------------------------------------------------------------------------------------------------------
   ...
   function uf_delete_cmpret($as_numcom,$as_codret,$aa_seguridad)
	{
		...
		{	
			...
			else
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� el comprobate ".$as_numcom." Asociado a la empresa ".$this->ls_codemp;
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////	
  ...
  function uf_liberar_rd($as_codded,$as_probene,$as_codprobene,$ai_totrowrecepciones)
    {
		...
		for($li_i=1;($li_i<$ai_totrowrecepciones)&&($lb_valido);$li_i++)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_liberar_rd ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}
		}
		return $lb_valido;
  ...
  function uf_liberar_recepciones($as_codded,$as_numcom,$as_probene,$as_codprobene)
  {
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_liberar_recepciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
  ...
  function uf_liberar_recepciones_consolida($as_codded,$as_numcom,$as_probene,$as_codprobene,&$as_hostname)
  {
		...
		if($as_hostname!="")
		{
				...
				if($rs_data===false)
				{
					$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_liberar_recepciones_consolida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
					return false;
				}
				else
				{
					while($row=$this->io_sql->fetch_row($rs_data))
					{
						...
						{
							$lb_valido=false;
							$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_liberar_recepciones_consolida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sqlconsolida->message)); 
						}
					}
				}
		}
	...
	function uf_obtener_bdorigen($as_numcom,$as_codret)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Modificar Comprobate M�TODO->uf_obtener_bdorigen ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_actualizar_estcmp($as_numrecdoc,$as_codprobene,$as_codded,$as_tipo)
	{
		...
		if($li_result===false)
		{	
				$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_actualizar_estcmp ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		return $lb_valido;
    }	  
	...
	function uf_delete_dt_cmpret_consolida($as_numcom,$as_codret, $aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Modificar Comprobate M�TODO->uf_delete_dt_cmpret_consolida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		return $lb_valido;
	...
	function uf_anular_cmpret_consolida($as_codret,$as_numcom,$as_probene,$as_codigo)
    {
		...
		if(($lb_valido)&&($as_hostname!=""))
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Modificar Comprobante M�TODO->uf_anular_cmpret_consolida ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sqlconsolida->message)); 
			}
		}
		return $lb_valido;
	}// end function uf_anular_cmpret
