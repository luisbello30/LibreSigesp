...
class sigesp_cxp_c_recepcion
 {
	...
	function uf_insert_recepcion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$as_codcla,$as_dencondoc,$ad_fecemidoc,$ad_fecregdoc,
								 ...
								 $as_estcla,$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$lb_valido=true;
		 $ls_sql="INSERT INTO cxp_rd (codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, codcla, dencondoc, fecemidoc, fecregdoc, ".
				...
				" codestpro4, codestpro5, estcla, estact, numordpagmin, codtipfon,repcajchi)  VALUES ('".$this->ls_codemp."','".trim($as_numrecdoc)."','".$as_codtipdoc."', ".
				...
				"'".$as_codestpro4."','".$as_codestpro5."','".$as_estcla."','".$as_estact."','".$as_numordpagmin."','".$as_codtipfon."','".$as_repcajchi."')";	
		...
		else
		{
			...
			if($lb_valido)
			{	
				$lb_valido=$this->uf_insert_deducciones($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_seguridad,$as_codrecdoc);
			}			
			if($lb_valido)
			{	
								  ...
								  $aa_seguridad)
	{
		...
		for($li_i=1;($li_i<$ai_totrowspg)&&($lb_valido);$li_i++)
		{
			...
			$li_moncue=$_POST["txtspgmonto".$li_i];//die();
...
//die ();
			...
			{
				 $lb_valido=$this->uf_load_monto_anulados_cuenta($ls_numcomanu,$ls_procede,$as_tipodes,$as_codpro,$as_cedbene,
...
*/
			if($lb_valido)
			{
				$li_comprometido=$li_monto_compromiso+(($li_monto_ajuste)-($li_monto_causado+$li_monto_anulado)-$li_monto_recepcion);
						//  5893+               (()                -(2473.85+                           ) - 2473.85          )
				//$li_comprometido=$li_monto_compromiso+(($li_monto_ajuste)-$li_monto_causado+$li_monto_anulado-$li_monto_recepcion);
				// echo $li_monto_compromiso."+((".$li_monto_ajuste.")-(".$li_monto_causado."+".$li_monto_anulado.")-".$li_monto_recepcion.")<hr>";//die();
				//$li_comprometido=$li_monto_compromiso+(($li_monto_ajuste)-$li_monto_causado+$li_monto_anulado-$li_monto_recepcion-$li_monto_cargo);
				//echo $li_comprometido."-".$li_moncue;die();
				// echo "compromiso: ".$li_monto_compromiso."monto causar".$li_moncue."<br>";
				if($li_monto_compromiso>0)
				{
					    //echo $li_comprometido." - ".$li_moncue;
				...
				//echo "disponible: ".$li_disponible;
				//echo $ls_cuenta." - ".$li_disponible."<hr>";
				if($li_disponible>=0)
				{
				//print "a";
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_deducciones($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$aa_seguridad,$as_codrecdoc)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if(array_key_exists("deducciones",$_SESSION))
		{
			...
			$li_totrow=$this->io_ds_deducciones->getRowCount('codded');
                      ...
                      // var_dump($_SESSION["deducciones"]); die();
			for($li_fila=1;$li_fila<=$li_totrow;$li_fila++)
			{
	...
	function uf_update_recepcion($as_numrecdoc,$as_codtipdoc,$as_cedbene,$as_codpro,$as_codcla,$as_dencondoc,$ad_fecemidoc,$ad_fecregdoc,
								 ...
								 $as_codestpro3,$as_codestpro4,$as_codestpro5,$as_estcla,$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_guardar($as_existe,$as_numrecdoc,$as_tipodestino,$as_codprovben,$as_codtipdoc,$ad_fecregdoc,$ad_fecvendoc,
						...
						$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$aa_seguridad)
	{		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar
		//		   Access: public (sigesp_cxp_p_recepcion.php)
			...
			case "FALSE":
				if(!($lb_encontrado))
				{
					...
					$lb_valido=$this->uf_insert_recepcion($as_numrecdoc,$as_codtipdoc,$ls_cedbene,$ls_codpro,$as_codcla,$as_dencondoc,
														  ...
														  $as_codestpro5,$as_estcla,$as_estact,$as_numordpagmin,$as_codtipfon,$as_repcajchi,$aa_seguridad);
				}
				else
				{
			...
			case "TRUE":
				if($lb_encontrado)
				{
					...
					$lb_valido=$this->uf_update_recepcion($as_numrecdoc,$as_codtipdoc,$ls_cedbene,$ls_codpro,$as_codcla,$as_dencondoc,
														  ...
														  $as_numordpagmin,$as_codtipfon,$as_repcajchi,$aa_seguridad);
				}
				else
				{
	...
	function uf_load_monto_anulados($as_comprobante,$as_procedencia,$as_tipodestino,$as_codpro,$as_cedbene,&$ai_monto) 
	{
		...
		$lb_valido=true; 
		$ls_operacion1="GC";
		$as_procedencia1='CXPSOP';
		$as_procedencia2='CXPRCD';
		$ls_operacion2="CP";
		$ls_sql="SELECT SUM(spg_dt_cmp.monto) AS monto ".
				"  FROM spg_dt_cmp ".
				" INNER JOIN sigesp_cmp ".
				...
				"   AND (spg_dt_cmp.operacion='".$ls_operacion1."' OR spg_dt_cmp.operacion='".$ls_operacion2."')".
				"   AND spg_dt_cmp.documento='".$as_comprobante."'".
				"   AND (spg_dt_cmp.procede_doc='".$as_procedencia1."' OR spg_dt_cmp.procede_doc='".$as_procedencia2."')";
		$rs_data=$this->io_sql->select($ls_sql);
		if ($rs_data===false)
		{
			$lb_valido=false; 
										   ...
										   $as_programatica,$as_estcla,$as_spguenta,&$ai_monto) 
	{
		...
		$ls_sql="SELECT SUM(spg_dt_cmp.monto) AS monto ".
				...
				"	AND TRIM(sigesp_cmp.cod_pro)='".$as_codpro."'".
				"   AND TRIM(sigesp_cmp.ced_bene)='".$as_cedbene."' ".
				"   AND (spg_dt_cmp.operacion='".$ls_operacion1."' OR spg_dt_cmp.operacion='".$ls_operacion2."') AND (";
			    for($i=1;$i<=count($as_comprobante);$i++)
			    {
				  $ls_sql.=" spg_dt_cmp.documento='".$as_comprobante[$i]."'";
				  if($i<count($as_comprobante))
				      $ls_sql.=" OR ";
			    }
		$ls_sql.=" ) AND (spg_dt_cmp.procede_doc='".$as_procedencia1."' OR spg_dt_cmp.procede_doc='".$as_procedencia2."')".
  		     	"   AND spg_dt_cmp.codestpro1='".$ls_codestpro1."' ".
				"   AND spg_dt_cmp.codestpro2='".$ls_codestpro2."' ".
			 	"   AND spg_dt_cmp.codestpro3='".$ls_codestpro3."' ".
				"   AND spg_dt_cmp.codestpro4='".$ls_codestpro4."' ".
	...
	{    
		...
		/* $ls_sql="SELECT sum(cxp_rd_spg.monto) AS monto ".
			 ...
			 $ls_sql="SELECT sum(cxp_rd_spg.monto) AS monto ".
				...
				"   AND cxp_rd.codtipdoc=cxp_rd_spg.codtipdoc ".
				"   AND trim(cxp_rd.numrecdoc)=trim(cxp_rd_spg.numrecdoc) ".
				"   AND cxp_rd.estprodoc='R' AND cxp_rd_spg.estcla='R'".//condiciones para tomar solo las recepciones que no esten anuladas
				" WHERE cxp_rd_spg.codemp='".$this->ls_codemp."' ".
				"   AND cxp_rd_spg.procede_doc='".$as_procedencia."' ".
				"	AND cxp_rd_spg.numdoccom='".$as_comprobante."' ";
		$rs_data=$this->io_sql->select($ls_sql);
	...
	{    
		...
		/*$ls_sql="SELECT sum(cxp_rd_spg.monto) AS monto ".
			 ...
			 $ls_sql="SELECT sum(cxp_rd_spg.monto) AS monto ".
			    ...
			    //trae todas las recepciones con estatus de registradas R. cxp_rd.estprodoc ='R' antes estaba con cxp_rd.estprodoc ='A'
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$lb_valido=false; 
