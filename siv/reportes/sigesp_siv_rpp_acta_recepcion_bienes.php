<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_cabezera($as_nomresalm,$as_nompro,$as_nombre,$as_nomrepemp,$as_nomreppro,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_encabezado=$io_pdf->openObject();
		$io_pdf->saveState();
	    $la_data=array(array('name'=>'			Entre el <b>'.trim($as_nombre).'</b> representado en este acto por el ciudadano <b>'.trim($as_nomrepemp).'</b> por una parte, y por la otra <b>" LA EMPRESA " '.$as_nompro.'</b> representada en este acto por el ciudadano  <b>'.strtoupper(trim($as_nomreppro)).'</b> , se ha procedido a efectuar la recepcion de los siguientes bienes  </b>'));
		
		$la_columnas=array('name'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						'fontSize' => 12, // Tamaï¿½o de Letras
						 'lineCol'=>array(0.9,0.9,0.9), // Mostrar Lï¿½neas
						 ...
						 'xPos'=>'centre', // Orientaciï¿½n de la tabla
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle($la_data,$as_estpro,$as_numordcom,$as_nombre,$as_montotart,$as_ciuemp,$adt_fecrec,&$io_pdf)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->ezTable($la_data,$la_columnas,'',$la_config);
		uf_print_pie_detalle($as_estpro,$as_numordcom,$as_nombre,$as_montotart,$as_ciuemp,$adt_fecrec,$io_pdf); 
	...
	function uf_print_pie_detalle($as_estpro,$as_numordcom,$as_nombre,$as_montotart,$as_ciuemp,$adt_fecrec,&$io_pdf)
	{
	    ...
	    $la_data=array(array('name'=>'
			Los cuales fueron recibidos  '.$adt_fecrec.' según <b>'.$ls_tipo_orden.' No. '.$as_numordcom.'</b> Acta levantada al efecto, en la forma siguiente:  
			El represenante del  <b>"'.trim($as_nombre).'"</b> luego de haber constatado que   <b>"LA EMPRESA"</b>  ejecuto el suministro señalado en la referida  '.$ls_tipo_orden.', declarada  <b>ACEPTADA</b>  la compra . 
			El monto correspondiente a este suministro es de  ('.$ls_titulo.'  '.number_format($as_montotart,2,",",".").' ) <b>'.$ls_monto_letras.'</b> 
			En prueba de conformidad se firma, en la ciudad de '.trim($as_ciuemp).', a los '.$li_dia.' dias del mes de '.$ls_mes.' de '.$li_agno.'. '));
	...
	function uf_print_pie_cabecera($as_nombre,&$io_pdf)
	{
       ...
       /* $io_pdf->addText(30,180,12,"POR LA EMPRESA");   
        ...
        $io_pdf->addText(110,180,12,"POR EL ".trim($as_nombre));   
        $io_pdf->addText(220,165,12,"ENTREGA CONFORME");   
        $io_pdf->addText(200,145,12,"Firma:");   
        $io_pdf->addText(200,130,12,"Nombre:");   
        $io_pdf->addText(200,115,12,"Cargo:");   
        $io_pdf->addText(200,100,12,"C:I. No.:");   
		$io_pdf->line(240,145,380,145);	//Horizontal		
		$io_pdf->line(245,130,380,130);	//Horizontal		
		$io_pdf->line(240,115,380,115);	//Horizontal		
		$io_pdf->line(245,100,380,100);	//Horizontal		
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -------------------------------------
	    ...
	    $ls_nomrepemp=$_GET["txtnompersol"];
	    $ls_nomreppro=$_GET["txtnomrespro"];
	    $ls_fecordcom=$_GET["txtfecordcom"];
    ...
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
	 ...
	 else // Imprimimos el reporte
	 {
		...
		uf_cabezera($ls_nomresalm,$ls_nompro,$ls_nombre,$ls_nomrepemp,$ls_nomreppro,$io_pdf);
		uf_cabezera_detalle($io_pdf);
		uf_print_pie_cabecera($ls_nombre,$io_pdf);
		uf_print_detalle($la_data,$ls_estpro,$ls_numordcom,$ls_nombre,$ls_montotart,$ls_ciuemp,$ls_fecordcom,$io_pdf); // Imprimimos el detalle
		unset($la_data);
