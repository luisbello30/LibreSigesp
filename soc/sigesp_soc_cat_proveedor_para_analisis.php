<?php
	...
	for($i=0;$i<count($ls_coditem);$i++){
	    $io_analisis->uf_proveedores_item($_GET["tipsolcot"],$ls_numsolcot,$ls_coditem[$i],$la_proveedores);//Se obtienen los proveedores que cotizaron el item seleccionado
	    $la_proveedorestodos[$i+1]=$la_proveedores;
	}
...
<form name="formulario" method="post" action="">
  ...
  <p><?php  
		$li_totrow=count($la_proveedorestodos[$li_item]);
		if($li_totrow>=1)
		{
			...
			{
				$ls_codpro=$la_proveedorestodos[$li_item][$li_i]["cod_pro"];
				if(array_search($ls_codpro,$la_provcotizaciones))
				{
					print "<tr class=celdas-blancas>";				
					$ls_nompro=        $la_proveedorestodos[$li_item][$li_i]["nompro"];
					$ls_numcot=        $la_proveedorestodos[$li_item][$li_i]["numcot"];
				    ...
				    for($j=1;$j<=count($la_proveedorestodos);$j++){
					$ls_cantidad.=number_format($la_proveedorestodos[$j][$li_i]["cantidad"],2,",",".");
					$ls_preciounitario.=number_format($la_proveedorestodos[$j][$li_i]["preciounitario"],2,",",".");
					$ls_moniva.=        number_format($la_proveedorestodos[$j][$li_i]["moniva"],2,",",".");
					$ls_montototal.=    number_format($la_proveedorestodos[$j][$li_i]["montototal"],2,",",".");
					if($j<count($la_proveedorestodos))
					{
					    $ls_cantidad.='//';
					    $ls_preciounitario.='//';
					    $ls_moniva.=        '//';
					...
					switch($la_proveedorestodos[$li_item][$li_i]["calidad"])
					{
						case "E":
...
{	
	...
	//---------------------------------------------------------------------------------
	parametros="";
	as_cantidad=as_cantidad.split('//');
	as_preciounitario=as_preciounitario.split('//');
	as_moniva=as_moniva.split('//');
	as_montototal=as_montototal.split('//');
	for(j=1;j<=total;j++)
	{ 
		...
		{*/
			...
			canselec=as_cantidad[j-1];
			preuniselec=as_preciounitario[j-1];
			ivaselec=as_moniva[j-1];
			monselec=as_montototal[j-1];
			numcot=as_numcot;
			codpro=as_codpro;
		/*}
		else
