...
switch ($li_diasem){
  ...
  break;
   ...
   function uf_limpiarvariables()
   {
		...
		// Fecha Creaci�n: 11/04/2007			Fecha �ltima Modificaci�n : 22/08/08.
		//////////////////////////////////////////////////////////////////////////////
   		global $io_fundb,$io_fun_compra,$ls_parametros,$ls_uniejeaso,$ls_estcla,$ls_distipmat,$ls_distipact,$ls_tipbiesol;
		global $ls_chkmat,$ls_chkact,$ls_numsolcot,$ls_codemp,$ls_operacion,$ls_fecregsolcot,$ls_tipsolcot,$ls_cedpersol;
		...
		$ls_tipbiesol = "-";//Tipo de Bienes Incluidos en la Solicitud de Cotizaci�n Materiales y/o Suministros � Activos.
...
switch($ls_operacion){
  case 'GUARDAR':
            uf_load_variables();
			$lb_valido=$io_soc->uf_guardar($ls_existe,$ls_numsolcot,$ls_tipsolcot,$ls_obssolcot,$ls_consolcot,$ls_uniejeaso,$ls_fecregsolcot,
...
?>
    <table width="800" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
      <tr class="titulo-ventana">
        ...
        <input name="existe" type="text" id="existe" value="<?php echo $ls_existe ?>" />
	...
	<input name="txtfecregsolcot" type="text" id="txtfecregsolcot"  style="text-align:center" value="<?php print $ls_fecregsolcot ?>" size="15" maxlength="10" datepicker="true" <?php echo $ls_disabled; ?> onKeyDown="javascript:ue_formato_fecha(this,'/',patron,true,event);" onBlur="javascript: ue_validar_formatofecha(this);" >
...
<!--         <input name="txtfecregsolcot" type="text" class="sin-borde" id="txtfecregsolcot" value="<?php echo $ld_fecregsolcot ?>" size="15" maxlength="10" style="text-align:left" readonly /></td> -->
      ...
      <tr>
        ...
        <td height="13" colspan="3"><label>
          <textarea name="txtconsolcot" cols="122" id="txtconsolcot" <?php echo $ls_discmb; ?> onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn�opqrstuvwxyz����� '+'�!:;_�#@/?�%&$*-,.+(){}[]='); "><?php echo $ls_consolcot ?></textarea>
        </label></td>
      </tr>
      <tr>
        <td height="13" style="text-align:right">Unidades Asociadas </td>
        <td height="13" colspan="3"><textarea readonly name="txtuniejeaso" cols="122" id="txtuniejeaso" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn�opqrstuvwxyz����� '+'�!:;_�#@/?�%&$*-,.+(){}[]='); "><?php echo $ls_uniejeaso ?></textarea>        </td>
      </tr>
      <tr>
        <td height="22" style="text-align:right">Observaci&oacute;n</td>
        <td height="22" colspan="3"><label>
          <input name="txtobssolcot" type="text" id="txtobssolcot" style="text-align:left" tabindex="2" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn�opqrstuvwxyz����� '+'�!:;_�#@/?�%&$*-,.+(){}[]='); " value="<?php echo $ls_obssolcot ?>" size="125" <?php echo $ls_discmb; ?>/>
        </label></td>
      </tr>
      <tr>
        <td height="13">&nbsp;</td>
			     ...
			     if (ls_tipsolcot!='-')
			        {
				      ...
				      if (lb_valido)
					     {
						   lb_valido = ue_validarcampo(ls_numsolcot,"El N�mero de Solicitud no puede estar vac�o !!!.",f.txtnumsolcot);
					     }
					  if (lb_valido)
						 {
						  lb_valido = ue_validarcampo(ls_consolcot,"El Concepto de la Solicitud no puede estar vac�o !!!.",f.txtconsolcot);
						 }
					  if (lb_valido)
						 {
						   lb_valido = ue_validarcampo(ls_fecregsolcot,"La Fecha no puede estar vac�a !!!.",f.txtfecregsolcot);
						 }
				      if (lb_valido)
					     {
						   lb_valido = ue_validarcampo(ls_cedpersol,"La C�dula del Personal solicitante no puede estar vac�a !!!.",f.txtcedper);
					     }
				      if (lb_valido)
					     {
						   lb_valido = ue_validarcampo(ls_codunieje,"La Unidad Ejecutora no puede estar vac�a !!!.",f.txtcodunieje);
					     }
				      if (lb_valido)
				         {
					       if (li_totrowsep>1)
						   ...
						   if (ls_tipsolcot=='B' && lb_valido)//Solicitud de Cotizaci�n de Tipo Bienes.
						      {
							    if (li_rowbienes>1)
							       ...
							       {
								     alert("La Solicitud de Cotizaci�n debe tener al menos un Material/Suministro � Activo incorporado !!!");
								     lb_valido = false;
							       }
						      }
					       else
...
function ue_imprimir_proveedor(codproveedor)
{
	li_imprimir = f.imprimir.value;
	lb_existe   = f.existe.value;
	if(li_imprimir==1)
	{
		if(lb_existe=="TRUE")
		{
			...
			if (ls_tipsolcot!='-')
			   {
			     ls_formato   = f.formato.value;
			     window.open("reportes/"+ls_formato+"?numsolcot="+ls_numsolcot+"&tipsolcot="+ls_tipsolcot+"&fecsolcot="+ls_fecsolcot+"&codprov="+codproveedor,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
			   }
		    else
			   {
			     alert("Debe seleccionar un Tipo de Solicitud v�lida (Bienes/Servicios) !!!");
