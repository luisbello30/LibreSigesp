...
class sigesp_soc_c_aceptacion_orden_servicio
{
  function sigesp_soc_c_aceptacion_orden_servicio($as_path)
  {
	...
	function uf_load_ordenes_servicios($as_tipope)
	{
		...
		elseif($as_tipope=='R')
		   {
		     $ls_sql="SELECT soc_ordencompra.numordcom,                   										   ".
					 ...
					 "   AND soc_ordencompra.codemp=rpc_proveedor.codemp  										   ".
					 "   AND soc_ordencompra.cod_pro=rpc_proveedor.cod_pro										   ".
					 "   AND soc_ordencompra.numordcom NOT IN(SELECT CASE WHEN numdoccom IS NULL THEN '-----' ELSE numdoccom END FROM cxp_rd_spg join cxp_rd on 
						  cxp_rd.numrecdoc=cxp_rd_spg.numrecdoc and cxp_rd_spg.codemp=cxp_rd.codemp 
					    WHERE cxp_rd_spg.procede_doc='SOCCOS' AND soc_ordencompra.codemp=cxp_rd_spg.codemp AND soc_ordencompra.cod_pro=cxp_rd_spg.cod_pro AND soc_ordencompra.numordcom=cxp_rd_spg.numdoccom  and cxp_rd.estprodoc!='A')";
		   }
		//echo $ls_sql;
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->sigesp_soc_c_aceptacion_orden_servicio.M�TODO->uf_soc_load_ordenes ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		return $rs_data;
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		 ...
		 for ($i=1;$i<=$ai_totrows;$i++)
			   ...
			   if (array_key_exists("chk".$i,$_POST))
			      {
		            ...
		            if ($rs_data===false)
		               {
			             $this->io_msg->message("CLASE->sigesp_soc_c_aceptacion_orden_servicio.M�TODO->uf_aceptar_orden_servicio->ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			             $lb_valido=false;
		                 break; 
					   }
					else
					   {
						 ///////////////////////////////         SEGURIDAD               /////////////////////////////		
						 $ls_evento="UPDATE";
						 $ls_descripcion ="Actualiz� estatus de aprobacion en ".$ls_estordser." la orden de servicio numero ".$ls_numordcom." Asociada a la Empresa ".$this->ls_codemp;
		  ...
		  if ($lb_valido)
		     {	
			   $this->io_sql->commit();
			   $this->io_mensajes->message("Operaci�n fue realizada con �xito !!!");
	  	       $this->io_sql->close();
			 }
		  else
		     {
		  	   $this->io_sql->rollback();
			   $this->io_mensajes->message("Ocurrio un Error en la operaci�n !!!"); 
   	  	       $this->io_sql->close();
			 }
