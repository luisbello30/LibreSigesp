...
class sigesp_soc_c_generar_orden_analisis
{
  function sigesp_soc_c_generar_orden_analisis($as_path)
  {
	...
	//	  Description: Funci�n que recorre el grid de los analisis de cotizacion y genera las respectivas ordenes de compra
	//	   Creado Por: Ing. Laura Cabr�
	// Fecha Creaci�n: 09/08/2007								Fecha �ltima Modificaci�n : 
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	  $lb_valido = false;
			...
			if (array_key_exists("chk".$i,$_POST))
			   {
					...
					for($li_i=0;$li_i<$li_totalganadores;$li_i++)
					{
						...
						if ($ls_tipsolcot=='B')
						   {
						     $ls_procede = 'SOCCOC';
						     $ls_numini  = 'numordcom';//N�mero Inicial.
						   }
						elseif($ls_tipsolcot=='S')
						   {
						     $ls_procede = 'SOCCOS';
							 $ls_numini  = 'numordser';//N�mero Inicial.
						   }
						...
						if ($ls_numordcom==false)
						   {
						     $this->io_mensajes->message("Este documento est� configurado para el manejo de Prefijos, y en este momento Ud. No tiene acceso a ninguno. Por favor dir�jase al Administrador del Sistema");
	...
	//	   Creado Por: Ing. Laura Cabr�
	// Fecha Creaci�n: 20/06/2007 								Fecha �ltima Modificaci�n : 
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$as_forpag="-";
	$as_diaplacom=0;
	...
	if($lb_valido)
	{
		...
		if($rs_data===false)
		{
			...
			else
			{
				...
				$this->io_mensajes->message("ffffff***CLASE->sigesp_soc_c_generar_orden_analisis.php; M�TODO->uf_insert_orden_compra ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			}			
		...
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la Orden de Compra ".$ls_numordcom." tipo ".$as_tipsolcot." de fecha".$ls_fecordcom;
	...
	function uf_insert_bienes($as_numordcom,$aa_seguridad,$as_codpro,$ls_numcot,$aa_items_cotizacion,$ai_totrow_cotizacion,$as_tipo_proveedor)
	{
		...
		for($li_i=1;($li_i<=$ai_totrow_cotizacion)&&($lb_valido);$li_i++)
		{
			...
			{
				$lb_valido=false;
			    $this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php;M�TODO->uf_insert_bienes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
								...
								
	{
		...
		for($li_i=1;($li_i<=$ai_totrow_cotizacion)&&($lb_valido);$li_i++)
		{
			...
			{
				$lb_valido=false;
			    $this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php;M�TODO->uf_insert_servicios ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	...
	function uf_insert_cuentas_presupuestarias($as_numordcom,$as_estcondat,$aa_items,$li_totrow,$aa_items_cotizacion,$ai_totrow_cotizacion,$aa_seguridad,$as_tipo_proveedor,$as_numcot,$ab_vienesep, $as_codpro)
	{
		...
		// Fecha Creaci�n: 17/03/2007 								Fecha �ltima Modificaci�n : 21/06/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if(count($this->io_dscuentas->data)>0)
		{
			...
			for ($li_fila=1;$li_fila<=$li_total;$li_fila++)
				...
				{
					$lb_valido=false;
					$this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php; M�TODO->uf_insert_cargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="INSERT";
					$ls_descripcion ="Insert� la Cuenta ".$ls_cuenta." de programatica ".$ls_codpro." a la orden de compra ".$as_numordcom. " Asociado a la empresa ".$this->ls_codemp;
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
    ...
    function uf_insert_cuentas_cargos($as_numordcom,$as_estcondat,$aa_items,$li_totrow,$aa_seguridad,$ab_vienesep)
	{
		...
		//	   Creado Por: Ing. Yesenia Moreno/ Ing. Luis Lang
		// Modificado Por: Ing. Yozelin Barrgan, Ing Laura Cabre
		// Fecha Creaci�n: 24/06/2007 								Fecha �ltima Modificaci�n : 01/06/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if ($ls_tipafeiva=='P')
		   {
				for($li_i=1;($li_i<=$li_totrow)&&($lb_valido);$li_i++)
				{
					...
					if($lb_valido)
					{
						...
						{
							$lb_valido=false;
							$this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php; M�TODO->uf_insert_cuentas_cargos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
						}
						else
						{
						    $lb_valido=true;
							/////////////////////////////////         SEGURIDAD               /////////////////////////////		
							$ls_evento="INSERT";
							$ls_descripcion ="Insert� la Cuenta ".$ls_spg_cuenta." de programatica ".$ls_codpro."Tipo = ".$ls_estcla." al cargo ".$ls_codcargo." de la orden de compra  ".$as_numordcom. " Asociado a la empresa ".$this->ls_codemp;
							$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
															$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
															$aa_seguridad["ventanas"],$ls_descripcion);
							/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			  ...
			  for ($li_i=1;($li_i<=$li_totrow)&&($lb_valido);$li_i++)
				 ...
				 /*  $ls_codestpro1 = $this->io_dscargos->getValue("codestpro1",$li_i);
				   ...
				   if ($rs_data===false)
				      {
					    $lb_valido = false;
						$this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php(Iva Contable);M�TODO->uf_insert_cuentas_cargos (Iva Contable);ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
					  } 
				   else
				      {
					     $lb_valido = true;
						 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
						 $ls_evento="INSERT";
						 $ls_descripcion ="Insert� la Cuenta Contable ".$ls_codctascg." al cargo ".$ls_codcargo." de la orden de compra  ".$as_numordcom. " de tipo ".$as_estcondat." Asociado a la empresa ".$this->ls_codemp;
						 $lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
						 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_validar_cuentas($as_numordcom,&$as_estcom,$as_estcondat)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php; M�TODO->uf_validar_cuentas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_cargos_sep($as_coditem,$as_numsep,$ls_tipsolcot)
	{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 22/06/2007								Fecha �ltima Modificaci�n : 13/11/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_cargos=array();
		$lb_valido=false;
		if($ls_tipsolcot=="B")
	...
	function uf_select_cuentacontable($as_codestpro1,$as_codestpro2,$as_codestpro3,$as_codestpro4,$as_codestpro5,$as_spgcuenta,$as_estcla,&$as_sccuenta)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->sigesp_soc_c_generar_orden_analisis.php; M�TODO->uf_select_cuentacontable ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
...
//---------------------------------------------------------------------------------------------------------------------------------------
	function uf_select_bienes_servicios($as_coditem,$as_tipo,$as_codpro,$as_numcot)
	{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 21/06/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$aa_datos=array();
		$lb_valido=false;
		if($as_tipo=="B")
...
//---------------------------------------------------------------------------------------------------------------------------------------
	function  uf_select_items_cotizacion($as_numcot,$as_codpro,$as_numanacot,$as_tipsolcot,&$aa_items,&$li_i)
	{
		...
		else
		{
				$ls_sql="SELECT DISTINCT d.codser as codigo, a.denser as denominacion, p.nompro, dt.canser as cantidad, dt.monuniser as precio, dt.moniva,dt.montotser as monto,
								...
								soc_dtsc_servicios.codestpro4,soc_dtsc_servicios.codestpro5,soc_dtsc_servicios.estcla
	...
	function uf_select_solicitud($as_numanacot,&$as_concepto,&$as_unidad,&$as_uniejeaso,&$as_tipbiesolcot)
	{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 31/10/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$as_concepto = array();
		$lb_valido   = true;
	                                       ...
	                                       &$as_codestpro1,&$as_codestpro2,&$as_codestpro3,&$as_codestpro4,&$as_codestpro5,&$as_estcla)
	{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			  Fecha: 30/10/2007								Fecha �ltima Modificaci�n : 11/11/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$aa_unidades = array();
		$lb_valido=true;
		$as_concepto="";
	...
	function uf_insert_enlace_sep($as_numordcom,$as_estcondat,$as_estcom,$aa_unidades,$aa_seguridad)
	{
		...
		// Fecha Creaci�n: 12/05/2007 								Fecha �ltima Modificaci�n : 30/10/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		for($li_fila=0;$li_fila<$li_total;$li_fila++)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Generar Orden Analisis M�TODO->uf_insert_enlace_sep ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			    echo $this->io_sql->message;
			}
			else
			{
				if($lb_valido)
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
					$ls_evento="INSERT";
					$ls_descripcion ="Insert� el enlace de la sep ".$ls_numsol." a la Orden de Compra  ".$as_numordcom." tipo ".$as_estcondat." Asociado a la empresa ".$this->ls_codemp;
					$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_viene_de_sep($as_numcot,$as_codpro, &$ab_viene_sep)
	{
		...
		//	   Creado Por: Ing. Laura Cabr�
		// 			Fecha: 11/11/2007								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$ab_viene_sep = false;
		$lb_valido=true;
		if($lb_valido)
