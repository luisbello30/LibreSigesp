	...
	function uf_print_sep($as_tipo)
   	{
		...
		echo "<tr class=titulo-celda>";
		echo "<td width=100 style='cursor:pointer' title='Ordenar por N�mero'            align='center' onClick=ue_orden('numsol')>Nro. Solicitud</td>";
	...
	function uf_print_analisis()
   	{
		...
		else
		{
				...
				while($row=$io_sql->fetch_row($rs_data))
				{
					...
					print "<tr class=celdas-blancas>";
					print "<td align='center'><a href=\"javascript:ue_aceptar('$ls_numanacot','$ls_fecanacot','$ls_obsana','$ls_numsolcot','".$row["tipsolcot"]."','$ls_estatus','$ls_estasitec','$ls_estesp','$ls_garanacot');\">".$ls_numanacot."</a></td>";
