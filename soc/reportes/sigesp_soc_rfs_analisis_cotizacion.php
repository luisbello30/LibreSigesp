<?PHP
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------------------------------------------------------
	function uf_print_pie_pagina(&$io_pdf,$nomusu,$apeusu)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_data[1]["1"]=strtoupper($nomusu." ".$apeusu);
		$la_data[0]["2"]="<b>Revisado: Jefe de Compra</b>";
		$la_data[1]["2"]="\n";
	...
	if($lb_valido)
	{
		...
		if($lb_valido)
		{
			...
			if($lb_valido)
			{
				...
				$la_ganadores=$io_class_report->uf_select_cotizacion_analisis($ls_numanacot,$ls_tipsolcot);//die();
				uf_print_ganadores($ls_numanacot,$ls_tipsolcot,$la_ganadores,$io_pdf);
				$io_pdf->set_margenes(0,0,0,0);
				uf_print_pie_pagina($io_pdf,$la_cotizaciones[1]["nomusu"],$la_cotizaciones[1]["apeusu"]);
				$io_pdf->ezStream();
				unset($io_pdf);
			}
		}
