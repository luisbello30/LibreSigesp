<?php
	...
	function uf_print_pie_cabecera($ld_total,&$io_pdf)
	{
		...
		$la_data=array(array('total'=>'<b>Total Pasivo + Capital '.$ls_bolivares.'</b>','totalgen'=>$ld_total));
	...
	if($lb_valido)
	{
		...
		else// Imprimimos el reporte
		{
			...
			for($li_i=1;$li_i<=$li_tot;$li_i++)
			{
				...
				//echo $ls_sc_cuenta." - Orden: ".$ls_orden." - ".$li_nro_reg."<br>";
				...
				for($li_i=1;$li_i<=$li_tot_acree;$li_i++)
				{
				 ...
				 else
				 {
					...
					if($ls_nivel==7)
					{
						...
						$la_data_acreedoras[$pos]=array('cuenta'=>$as_cuenta,'denominacion'=>'            '.$ls_denominacion,'saldo'=>$ld_saldo);					  
					...
					if($ls_nivel==3)
					{
						  $la_data_acreedoras[$pos]=array('cuenta'=>'<b>'.$as_cuenta.'</b>','denominacion'=>'<b>        '.$ls_denominacion.'</b>','saldo'=>$ld_saldo);
						 
					}
					if($ls_nivel==2)
					{
						  $la_data_acreedoras[$pos]=array('cuenta'=>'','denominacion'=>'','saldo'=>'');
						  $pos++;
						  $la_data_acreedoras[$pos]=array('cuenta'=>'<b>'.$as_cuenta.'</b>','denominacion'=>'    '.'<b>'.$ls_denominacion.'</b>','saldo'=>'<b>'.$ld_saldo.'</b>');
					}
					if($ls_nivel==1)
					{
						 ...
						 if ($cont>1)
						 {
							$la_data_acreedoras[$pos]=array('cuenta'=>'','denominacion'=>'','saldo'=>'');
							$pos++;
							$la_data_acreedoras[$pos]=array('cuenta'=>'<b><i>'.$as_cuenta.'</b></i>','denominacion'=>'<b><i>'.$ls_denominacion.'</b></i>','saldo'=>'<b><i>'.$ld_saldo.'</b></i>');
						 }
						 else
						 {
							$la_data_acreedoras[$pos]=array('cuenta'=>'<b><i>'.$as_cuenta.'</b></i>','denominacion'=>'<b><i>'.$ls_denominacion.'</b></i>','saldo'=>'<b><i>'.$ld_saldo.'</b></i>');
