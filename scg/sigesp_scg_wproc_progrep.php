...
//----------------------------------------------------------------------------------------------------------------------			
  ...
  {	
	...
	if ($ls_opcion=="N")
	{
		...
		$ld_saldo_aprobado=str_replace(',','.',$ld_saldo_aprobado);
		$ds_prorep->updateRow("saldo_aprobado",$ld_saldo_aprobado,$i);		
	...
	{   
		...
		for($li=1;$li<=$total;$li++)
		{
		  ...
		  $ds_prorep->updateRow("saldo_aprobado",0,$pos);
		...
		while($li_nivel_cta>1 && !empty($ls_next_cta))
		{
			...
			for($li_s=1;$li_s<=$li_totrows;$li_s++)
			{
				...
				if($li_posicion>0)
				{
					...
					if($li_nivel_aux==$li_new_nivel)
					{
						...
						$ld_total_saldo_aprobado = $ld_total_saldo_aprobado + $ds_prorep->getValue("saldo_aprobado",$li_posicion);
			...
			//$ds_prorep->updateRow("saldo_aprobado",$ld_total_saldo_aprobado,$pos);
    ...
    <tr class="celdas-amarillas">
      ...
      <?php
			 ...
			 print"<td>".number_format($ds_prorep->getValue("diciembre",$i),2,",",".")."</td>";
			 print"<td>".number_format($ds_prorep->getValue("saldo_real_ant",$i),2,",",".")."</td>";
			 print"<td>".number_format($ds_prorep->getValue("saldo_aprobado",$i),2,",",".")."</td>";
			 print"<td>".number_format($ds_prorep->getValue("saldo_mod",$i),2,",",".")."</td>";
		     print"</tr>";
		 }//for
...
{	
	...
	for ($i=1; $i<= $li_num; $i++)
	{
		...
		$la_datos[25] = $ds_prorep->getValue("saldo_aprobado",$i);
