...
if(array_key_exists("operacion",$_POST))
{
	...
	for($li_row=1;$li_row<=$li_filascuentas;$li_row++)
	{
		...
		$la_objectcuentas[$li_row][4]="<input name=txtmoncue".$li_row." type=text id=txtmoncue".$li_row." class=sin-borde size=20 style= text-align:right value='".$ls_moncue."' onKeyPress=return(currencyFormat(this,'.',',',event)) onBlur=javascript:ue_validardispo()>";
...
//	$io_valuacion->uf_select_newcodigo($ls_codcon,&$ls_codval);
	...
	if($ai_rowscuentas>0)
	{
		while(!$rs_datacuentas->EOF)
		{
			...
			$la_objectcuentas[$li_row][2]="<input name=txtestcla".$li_row." type=text id=txtestcla".$li_row." class=sin-borde style= text-align:center size=20 value='".$ls_estcla."' readonly>";
			$la_objectcuentas[$li_row][3]="<input name=txtnomcue".$li_row." type=text id=txtnomcue".$li_row." class=sin-borde style= text-align:left size=10 value='".$ls_spgcuenta."' readonly >";
			$la_objectcuentas[$li_row][4]="<input name=txtmoncue".$li_row." type=text id=txtmoncue".$li_row." class=sin-borde size=20 style= text-align:right value='0,00' onKeyPress=return(currencyFormat(this,'.',',',event)) onBlur=javascript:ue_validardispo()>";
			$la_objectcuentas[$li_row][5]="";
   ...
   /************************CARGANDO CUENTAS**************************************************/ 
	...
	if($ai_rowscuentas>0)
	{
		while(!$rs_datacuentas->EOF)
		{
			...
			$la_objectcuentas[$li_row][2]="<input name=txtestcla".$li_row." type=text id=txtestcla".$li_row." class=sin-borde style= text-align:center size=20 value='".$ls_estcla."' readonly>";
			$la_objectcuentas[$li_row][3]="<input name=txtnomcue".$li_row." type=text id=txtnomcue".$li_row." class=sin-borde style= text-align:left size=10 value='".$ls_spgcuenta."' readonly >";
			$la_objectcuentas[$li_row][4]="<input name=txtmoncue".$li_row." type=text id=txtmoncue".$li_row." class=sin-borde size=20 style= text-align:right value='".$li_monto."' onKeyPress=return(currencyFormat(this,'.',',',event)) onBlur=javascript:ue_validardispo()>";
			$la_objectcuentas[$li_row][5]="";
...
function ue_cargarvaluacion(ls_codval,ls_codasi,ls_codcon,ls_fecha,ls_fecinival,ls_fecfinval,ls_obsval,ls_amoval,ls_obsamoval,ls_amoantval,ls_amototval,ls_amoresval,ls_basimpval,ls_montotval,ls_subtotpar,ls_totreten,ls_subtot,ls_nomestval,ls_estapr,ls_moncont)
{
    f=document.form1;
