...
class class_sigesp_scbmov_integracion
{
	...
	function class_sigesp_scbmov_integracion()
	{
		...
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 01/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
											   ...
											   $aa_seguridad)
	{
		...
		//	    Arguments: as_codban  // C�digo de Banco
		...
		//				   ai_feccondep  // Fecha de contabilizaci�n de los dep�sitos
		...
		// Verifico si el comprobante existe y de ser as� lo cargo en el datastored
		...
		if ($lb_valido)
		{   
			// Procesa la Programaci�n de pagos si viene de cuentas por pagar	
			$lb_valido=$this->uf_procesar_programacion_pagos($ldt_fecha);
		}
		if (!$lb_valido)
		{   
			$this->io_msg->message("ERROR-> En programaci�n de pago"); 
			$this->io_sigesp_int->uf_sql_transaction($lb_valido);
		    return false;		   		   
		}
	    ...
	    if($lb_valido)
	    {
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Contabiliz� el Documento <b>".$as_numdoc."</b>, Banco <b>".$as_codban."</b>, ".
							"Cuenta Banco <b>".$as_ctaban."</b>, Fecha de Contabilizaci�n <b>".$ldt_fecha."</b>";
	...
	function uf_obtener_data_movimento_banco()
	{
		...
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 02/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_existe=false;		
		...
		if($rs_data===false)
		{
            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_obtener_data_movimento_banco ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
	...
	function uf_obtener_codigo_contable_cuentabanco(&$as_scg_cuenta)
	{
		...
		//		   Access: private
		//	    Arguments: as_scg_cuenta // N�mero de cuenta contable 
		//	      Returns: lb_valido True si se encontro el movimiento � false si no se encontro
		//	  Description: Funcion que obtiene el c�digo contable de la cuenta del banco
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 02/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_existe=false;		
		...
		if($rs_data===false)
		{
            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_obtener_codigo_contable_cuentabanco ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
			...
			{
	            $this->io_msg->message("ERROR-> La cuenta ".$this->is_ctaban." del banco".$this->is_codban." no posee c�digo contable.");			
			}
			$this->io_sql->free_result($rs_data);
		}
		return $lb_existe;
	...
	function uf_procesar_gastos($as_descripcion)
	{
		...
		if($rs_data===false)
		{   
            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_gastos ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
	...
	function uf_procesar_contable($as_descripcion,$as_procede)
	{
		...
		if($rs_data===false)
		{   
            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_contable ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
	...
	function uf_procesar_ingresos($as_descripcion)
	{
		...
		//		   Access: private
		//	    Arguments: as_descripcion // Descripci�n
		//	      Returns: lb_valido True si se encontro el movimiento � false si no se encontro
		//	  Description: Este metodo ejecuta el nucleo integrador del m�dulo de banco
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 03/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT * ".
                "  FROM scb_movbco_spi ".
		...
		if($rs_data===false)
		{   
            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_ingresos ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{    
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		    {
				...
				///---informaci�n de la estructuras presupuestarias relacionadas a las cuentas de ingresos
	...
	function uf_procesar_programacion_pagos($adt_fecha)
	{
		...
		//	      Returns: lb_valido True si se encontro el movimiento � false si no se encontro
		//	  Description: Funcion que actualiza la informaci�n de las programaciones de pago de una solicitud 
		//                  y verifica su informaci�n de pago total o parcial
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 02/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $lb_valido=true;
		...
		if(($this->ii_datasource==4)&&(!$this->ii_compromiso_previo))
		{
			...
			if($rs_data===false)
			{   
	            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_programacion_pagos 1 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
				return false;
			}
			else
			{                 
				while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
				{
					...
					{   
		            	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_programacion_pagos 2 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
						return false;
					}
					else
					{    
	...
	function uf_update_estatus_solicitud_cancelado($as_numsol,$as_estatus)
	{
		...
		//	      Returns: lb_valido True si se encontro el movimiento � false si no se encontro
		//	  Description: M�todo que actualiza en cancelado la solicitud de pago, cuando la sumatoria de los cheques sean 
		...
		if($li_row===false)
		{
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_update_estatus_solicitud_cancelado ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_update_estatus_programacion_pago($as_numsol,$as_estatus)
	{
		...
		//	    		   as_estatus // estatus de la Solicitud
		//	      Returns: lb_valido True si se encontro el movimiento � false si no se encontro
		//	  Description: M�todo que actualiza en estatus cancelado la programacion de pago
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 03/11/2006
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_valido=true;
		...
		if($li_row===false)
		{
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_update_estatus_programacion_pago ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
										...
										$as_procede,$as_comprobante,$adt_fecmov,$adt_fecha)
	{
		...
		if (!$lb_existe)
		{
            $this->io_msg->message("ERROR -> El movimiento no existe Banco=".$as_codban." Cuenta=".$as_ctaban." documento=".$as_numdoc." operaci�n=".$as_codope." estatus=".$as_estmov);		
			return false;
		}
		...
		if ($lb_existe)
		{
            $this->io_msg->message("ERROR -> El movimiento ya existe Banco=".$as_codban." Cuenta=".$as_ctaban." documento=".$as_numdoc." operaci�n=".$as_codope." estatus=".$as_estmov_new);		
			return false;			
		}
		...
		$ls_sql = "INSERT INTO scb_movbco (codemp,codban,ctaban,numdoc,codope,estmov,cod_pro,ced_bene,".
				  ...
				  "						   fechaconta, fechaanula,nrocontrolop, estant,docant,monamo,numconint,tranoreglib) ".
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_create_movimiento_banco ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		//------------------------se agrega el insert para scb_movbco_anticipo----------------------------------------------
		   $ls_sql=" INSERT INTO scb_movbco_anticipo(codemp, codban, ctaban, numdoc, codope, estmov, codamo, monamo, ".
			...
			if($li_row===false)
			{   
				$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_create_movimiento_banco 
	...
	function uf_select_movimiento_banco($as_codemp,$as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov)
	{
		...
		if($rs_data===false)
		{ 
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_select_movimiento_banco ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{
    ...
    function uf_delete_movimiento_banco($as_codemp,$as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov)
	{
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 1 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		} 
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 2 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 3 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 4 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
        // transferencia al nuevo registro de banco detalle contables
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 5 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		...
		if($li_row===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 6 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		...
		//---------------------------------para los movimientos de anticipos----------------------------------------------------
			...
			if($li_row===false)
			{   
				$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_delete_movimiento_banco 8 ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
				return false;
			}
		//---------------------------------------------------------------------------------------------------------------------
	...
	function uf_reversar_contabilizacion_banco($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$ad_fechaconta,$aa_seguridad)
	{
		...
		//	    		   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se encontro reverso el movimiento � false si no se reverso
		...
		if ($ls_valor==0)
		{
			...
			if($lb_valido)
			{
				...
				// Vuelvo a colocar la programaci�n de pagos activa
				$lb_valido = $this->uf_restaura_programacion_pago('N');
				if (!$lb_valido)
				{   
					$this->io_msg->message("ERROR -> Al restaurar la programaci�n de Pago"); 
					$this->io_sigesp_int->uf_sql_transaction($lb_valido);
					return false;		   		   
				}
			...
			if($lb_valido)
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="PROCESS";
				$ls_descripcion="Revers� la contabilizaci�n del Documento <b>".$as_numdoc."</b>, Banco <b>".$as_codban."</b>, ".
		...
		{
			$this->io_msg->message("No se puede Reversar la contabilizaci�n el Docuemnto $as_numdoc, es de tipo anticipo y posee Amortizaciones asociadas ");
			$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_restaura_programacion_pago($as_estnue)
	{
		...
		//		   Access: private
		//	    Arguments: as_estnue // Nuevo estatus de la programaci�n de pago
		//	      Returns: lb_valido True si se encontro reverso el movimiento � false si no se reverso
		//	  Description: Este metodo restaura la programaci�n del pago de un cheque
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 03/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_restaura_programacion_pago ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
										 ...
										 $ad_fechaconta,$as_conanu,$aa_seguridad)
	{
		...
		//	    		   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se encontro reverso el movimiento � false si no se reverso
		//	  Description: Proceso de anulaci�n banco en cualquiera de sus operaciones
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 03/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$this->is_codemp=$this->io_codemp;
		...
		if ($ls_valor==0)
		{
			...
			{
			   $this->io_msg->message("ERROR-> La Fecha de Anulaci�n es menor que la fecha del Documento.");
			   return false;
			}
			...
			if($lb_valido)
			{
				...
				// M�todo que restaura la programaci�n
			...
			if($lb_valido)
			{
				...
				$ls_descripcion="Anulo el Documento <b>".$as_numdoc."</b>, Banco <b>".$as_codban."</b>, ".
								"Cuenta Banco <b>".$as_ctaban."</b>, Fecha de Anulaci�n <b>".$ldt_fecha."</b>";
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
    ...
    function uf_configuro_source_class()
    {
		...
		//		   Access: private
		//	      Returns: 
		//	  Description: M�todo que configura y establece las propiedades de la clase
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 03/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// Fuente de Generaci�n (Banco o Cheque)
		if(($this->ii_datasource==1)||($this->ii_datasource==4))
		{
		...
		// Fuente de Generaci�n (Colocaciones)
	...
	function uf_reversar_anulado($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$ad_fechaconta,$ad_fechaanula,$aa_seguridad)
	{
		...
		//	    		   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se encontro reverso el movimiento � false si no se reverso
		//	  Description: Proceso de elimina el la anulaci�n de un cheque, el cual elimina tanto el anulado como el original 
		//                  y coloca el numero de solicitud en esttaus de programaci�n de pago (S)
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 03/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// inicio transacci�n SQL
        $this->io_sigesp_int->uf_int_init_transaction_begin();
										   ...
										   $ad_fechaanula,$aa_seguridad)
	{
		...
		//	    		   ad_fechaanula // Fecha de Anulaci�n
		//	      Returns: lb_valido True si se elimin� el movimiento � false si no se elimin�
		//	  Description: Proceso de elimina el documento con estatus anulado
		//	   Creado Por: Ing. Wilmer Brice�o
		// Modificado Por: Ing. Yesenia Moreno								Fecha �ltima Modificaci�n : 06/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$this->is_codemp=$this->io_codemp;
	    $this->is_codban=$as_codban;
	    $this->is_ctaban=$as_ctaban;
	...
	function uf_procesar_contabilizacion_orden_pago($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$adt_fecha,$aa_seguridad)
	{
		...
		//	    		   as_codope // C�digo de Operaci�n
		//	    		   as_estmov // estatus del Movimiento
		//	    		   adt_fecha // Fecha de Contablizaci�n
		//	    		   aa_seguridad // Arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se encontro reverso el movimiento � false si no se reverso
	    ...
	    if($lb_valido)
	    {
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Contabiliz� la Orden de Pago Directa Documento <b>".$as_numdoc."</b>, Banco <b>".$as_codban."</b>, ".
							"Cuenta Banco <b>".$as_ctaban."</b>, Fecha de Contabilizaci�n <b>".$ldt_fecha."</b>";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
	...
	function uf_procesar_gastos_pago_directo($as_descripcion)
	{
		...
		if($rs_data===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_gastos_pago_directo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
	...
	function uf_procesar_contable_pago_directo($as_descripcion)
	{
		...
		if($rs_data===false)
		{   
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_procesar_contable_pago_directo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
															...
															$aa_seguridad)
	{
	    ...
	    if($lb_valido)
	    {
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="PROCESS";
			$ls_descripcion="Revers� la Orden de Pago Directa Documento <b>".$as_numdoc."</b>, Banco <b>".$as_codban."</b>, ".
							"Cuenta Banco <b>".$as_ctaban."</b> ";
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
												  ...
												  $ad_fechaconta,$ad_fechaanula,$as_conanu)
	{
		...
		//                 as_conanu     // concepto de Anulacion
		//	      Returns: lb_valido True si se ejecuto la contabilizaci�n correctamente
		//	  Description: M�todo que actualiza las fechas de contabilizaci�n y de anulaci�n del documento
		...
		if($li_row===false)
		{
            $this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_update_fecha_contabilizado_scbmov ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		return $lb_valido;
...
//------------------------------------------------------------------------------------------------------------------------------------
   ...
   {		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//	    Arguments: 
		//	      Returns: lb_valido True si se ejecuto la contabilizaci�n correctamente
		//	  Description: Elimina la amortizaci�n que realizo el cheque
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->Error en select scb_movbco ".
		...
		return	$lb_valido;
   }//fin de la funci�n  uf_eliminar_amortizacion //-------------------------------------------------------------------------------------------------------------------------------------
 //---------------------------------------------------------------------------------------------------------------------------------------
    function uf_buscar_amortizaciones($as_codemp,$as_docant)
	{
		...
		if($rs_data===false)
		{ 
           	$this->io_msg->message("CLASE->Integraci�n SCB M�TODO->uf_buscar_amortizaciones ERROR->".
			                       $this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
