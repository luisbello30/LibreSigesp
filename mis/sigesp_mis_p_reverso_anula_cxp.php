<?php
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_tipo_operacion,$io_ddlb,$ls_numdoc,$ls_fecdoc;
                ...
                $lo_title[1]="Todos    <input type='checkbox' name='todos' id='todos' onclick='uf_select_all(document.forms[0])' />";
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if($lb_valido)
					{
						$in_class->io_msg->message("La Solicitud de Pago  ".$ls_numsol." fue Reversada la Anulaci�n.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Anulaci�n de la Solicitud de Pago ".$ls_numsol);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_orden_pago_reversar_anulacion($ls_numsol,$ls_fecemi,$ls_fecapr,$ls_codigo,$ls_tipo,
...
function uf_select_all(form)
{



     var formulario = eval(form)
    if (form.txtnumsol1.value != "")
    {
        ...
        if ( formulario.elements[i].type == "checkbox" )
         {
                ...
                formulario.elements[i].checked = formulario.todos.checked
