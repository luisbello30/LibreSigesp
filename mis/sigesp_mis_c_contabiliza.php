...
/*SE COMENTAN LAS SIGUIENTES LINEAS, BEDIDO A QUE DESPUES DE ANULAR O ELIMINAR UNA SOP RELACIONADA CON LA RDC, ESTA ULTIMA NO SE LIBERABA PARA SER MODIFICADA 
(REVERSAR CONTABILIZACION Y APROBACION), LA UNICA MANERA DE HACERLO ERA ELIMINARLA Y VOLVERLA A HACER, YA QUE SE ENCONTRABA UN DETALLE DE SOP ATADO A LA 
RCD A PESAR DE QUE LA SOP ESTUVIESE ANULADA O ELIMINADA*/
		/*switch($as_proceso)
		{
			case "REVERSAR":
	...
	function uf_select_orden_pago_contabilizar($as_numsol,$as_fecemi,$as_fecapr,$as_codigo,$as_tipo,&$ao_object,&$ai_totrows,$as_estatus)
	{
		...
		if(!empty($as_numsol))
		{
			$ls_criterio=$ls_criterio." AND a.numsol like '%".$as_numsol."%'";
		}
		if(!empty($as_fecemi))
		{
			$as_fecemi=$this->io_function->uf_convertirdatetobd($as_fecemi);
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_orden_pago_reversar_anulacion($as_numsol,$as_fecemi,$as_fecapr,$as_codigo,$as_tipo,&$ao_object,&$ai_totrows,$as_estatus)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_orden_pago_contabilizar
		//		   Access: public (sigesp_mis_p_contibiliza_cxp.php)
		...
		$ls_criterio=$ls_criterio."AND b.numrecdoc NOT IN (SELECT c.numrecdoc FROM cxp_dt_solicitudes AS c JOIN cxp_solicitudes AS d ON c.numsol=d.numsol 
					  AND c.numrecdoc= b.numrecdoc AND d.estprosol!='".$as_estatus."') 
					  AND (a.numsol IN (SELECT MAX(d.numsol) FROM cxp_dt_solicitudes AS c LEFT JOIN cxp_solicitudes AS d 
					      ON c.numsol=d.numsol WHERE c.numrecdoc= b.numrecdoc ) or (SELECT COUNT(c.numrecdoc) 
					      FROM cxp_dt_solicitudes AS c LEFT JOIN cxp_solicitudes AS d ON c.numsol=d.numsol WHERE 
					      c.numrecdoc= b.numrecdoc AND d.numsol=a.numsol)=0)";

		$ls_sql="SELECT distinct a.numsol, a.fecemisol, a.consol, a.fechaconta, a.fechaanula  ".
                "  FROM cxp_solicitudes AS a LEFT JOIN cxp_dt_solicitudes AS b ON a.numsol=b.numsol".
				" WHERE a.codemp='".$ls_codemp."' ".
				"   AND a.estprosol='".$as_estatus."'  ".
				"   AND a.estaprosol = 1 ".
				    $ls_criterio;
	...
	function uf_select_banco_contabilizar($as_numdoc,$as_fecdoc,$as_tipo_operacion,&$ao_object,&$ai_totrows,$as_estatus)
	{
		...
		$ls_sql="SELECT codban, ctaban, estmov, numdoc, fecmov, conmov, codope, fechaconta, fechaanula, numconint ".
                "  FROM scb_movbco ".
				" WHERE codemp='".$ls_codemp."' ".
