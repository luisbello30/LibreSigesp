		...
		for ($li_i=1;$li_i<=$li_totalRows;$li_i++)				
		    {
			  if (array_key_exists("chk".$li_i,$_POST))
			     { 
				   ...
				   $lb_valido  = true;
                                   $lb_valido=$io_carord->uf_procesar_movbanco($ls_codban,$ls_cuenta_banco,$ls_numdoc,$ls_mov_operacion,$ld_fecha,$ls_desmov,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_desproben,$ldec_montomov,$ldec_monobjret,$ldec_montoret,$ls_chevau,$ls_estmov,0,0,'T','SCBCOR','',$ls_tipo,$ls_numcarord,"--",$ls_numordpagmin,$ls_codtipfon,$ls_numcontint);
				   ...
				   if ($ls_codproben!=$ls_probentemp)
				      {
						...
						$arr_movbco["mov_document"] = $ls_numdoc;
					    $arr_movbco["objret"]   	= $ldec_monobjret;
					    $lb_valido=$io_carord->uf_procesar_movbanco($ls_codban,$ls_cuenta_banco,$ls_numdoc,$ls_mov_operacion,$ld_fecha,$ls_desmov,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_desproben,$ldec_monto,$ldec_monobjret,$ldec_montoret,$ls_chevau,$ls_estmov,0,0,'T','SCBCOR','',$ls_tipo,$ls_numcarord,$ls_codfuefin,$ls_numordpagmin,$ls_codtipfon,$ls_numcontint);
				      }	
				   else
				      {
					    $io_carord->uf_select_monto_actual($ls_codemp,$ls_codban,$ls_cuenta_banco,$ls_numdoc,'ND',$ld_fecha,&$ldec_montoactual,&$ldec_monobjret_actual,&$ldec_monret_actual);
