...
class sigesp_scb_report
{
	...
	function uf_cargar_chq_voucher($ls_numdoc,$ls_voucher,$ls_codban,$ls_ctaban,$ls_codope)
	{
		...
		switch ($_SESSION["ls_gestor"])
		{
			case "MYSQLT":
				...
				break;
			case "POSTGRES":
				$ls_cadena="rpc_beneficiario.apebene||', '||rpc_beneficiario.nombene||' '||rpc_beneficiario.ced_bene";
				break;
			case "INFORMIX":
		...
		if($ls_voucher=="")
		{
		   $ls_sql="SELECT scb_movbco.*, scb_ctabanco.dencta,nomban,scb_ctabanco.codtipcta,".
	...
	function uf_cargar_chq_voucher_anulado($as_numdoc,$as_voucher,$as_codban,$as_ctaban,$as_codope)
	{
		...
		// Fecha Creaci�n: 26/03/2008									Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_codemp=$_SESSION["la_empresa"]["codemp"];
		if($as_voucher=="")
		{
		...
		if ($as_tipproben=='P')
		   {
		     $ls_campo  = "cod_pro";
		     $ls_tabla  = "rpc_proveedor";
		     $ls_straux = ", max(rpc_proveedor.nompro) as nompro, max(rpc_proveedor.rifpro) as rifpro, max(rpc_proveedor.ctaban) as ctapro";
		...
		else
		   {
		     $ls_campo  = "ced_bene";
		     $ls_tabla  = "rpc_beneficiario";
		     $ls_straux = ", max(rpc_beneficiario.nombene) as nombene,max(rpc_beneficiario.apebene) as apebene, max(rpc_beneficiario.ctaban) as ctabene ";
			...
			if ($as_tipproben=='B')
			   {
			     while ($row=$this->SQL->fetch_row($rs_data))
			           {
				         $li++;
				         $la_data[$li]=array('cedbene'=>$row["ced_bene"],'nombene'=>$row["nombene"].$row["apebene"],'ctaban'=>$row["ctabene"],'monto'=>number_format($row["monto"],2,",","."));
			           }
			   }
			elseif($as_tipproben=='P')
			   {
			     while ($row=$this->SQL->fetch_row($rs_data))
					   {
					     $li++;
						 $la_data[$li]=array('cod_pro'=>$row["rifpro"],'nompro'=>$row["nompro"],'ctaban'=>$row["ctapro"],'monto'=>number_format($row["monto"],2,",","."));
					   }
			   }
			$this->SQL->free_result($rs_data);
		}
...
//		   Access: public
//		 Argument: $as_codban = C�digo del Banco.
//                 $as_ctaban = N�mero de la Cuenta Bancaria.
//                 $as_numdoc = N�mero del Movimiento Bancario.
//                 $as_codope = C�digo de la Operaci�n CH=Cheque,DP=Dep�sito,ND=Nota de D�bito,NC= Nota de Cr�dito,RE=Retiro.
//                 $as_estmov = Estatus del Movimiento.
//	  Description: Funci�n que extrae la sumatoria de las retenciones de impuesto sobre la renta aplicadas desde Banco.
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $lb_valido = false;
	   $this->io_msg->message("CLASS->sigesp_scb_report.php;M�TODO->uf_load_retenciones_scb;ERROR->".$this->fun->uf_convertirmsg($this->SQL->message));
...
//                 $as_codope = C�digo de la Operaci�n CH=Cheque,DP=Dep�sito,ND=Nota de D�bito,NC= Nota de Cr�dito,RE=Retiro.
//                 $as_estmov = Estatus del Movimiento.
//	  Description: Funci�n que extrae la sumatoria de las retenciones de IVA e ISLR aplicadas desde Cuentas Por Pagar, dependiendo
//                 del tipo de retenci�n indicada por el par�metro $as_tipret.
//	   Creado Por: Ing. N�stor Falc�n.
// Fecha Creaci�n: 16/06/2008
////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $lb_valido = false;
	   $this->io_msg->message("CLASS->sigesp_scb_report.php;M�TODO->uf_load_retenciones_cxp;ERROR->".$this->fun->uf_convertirmsg($this->SQL->message));
	   echo $this->SQL->message;
	 }
  else
     {
...
///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $lb_valido = false;
	   $this->io_msg->message("CLASS->sigesp_scb_report.php;M�TODO->uf_load_beneficiario_alterno;ERROR->".$this->fun->uf_convertirmsg($this->SQL->message));
	   echo $this->SQL->message;
	 }
  else
     {
...
function buscar_benef($parametros=array()){
		...
		{
			$mensaje = "CLASE->sigesp_scb_report SNO M�TODO->buscar_benef ERROR->".$this->SQL->message; 
...
function buscar_cheques_periodo($parametros=array()){
		...
		{
			$mensaje = "CLASE->sigesp_scb_report SNO M�TODO->buscar_cheques_periodo ERROR->".$this->SQL->message; 
			$this->io_msg->message($mensaje);			
		   	return false;
		}
...
function buscar_pensionado($parametros=array()){
		...
		{
			$mensaje = "CLASE->sigesp_scb_report SNO M�TODO->buscar_pensionado ERROR->".$this->SQL->message; 
			$this->io_msg->message($mensaje);			
		   	return false;
		}
