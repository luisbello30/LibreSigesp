<?php
	...
	if ($ls_conchk==1)
	{
		...
		for($li_p=0;$li_p<$li_total;$li_p++)
		{
			...
			$rs_data   = $io_report->uf_cargar_documentos($ls_codope1,$ld_fecdesde,$ld_fechasta,$ls_codban,$ls_ctaban,$ls_codconcep,$ls_estatus,$ls_orden,$ls_documento,$ls_conchk,&$lb_valido);
			//---------------------------------------------------------------------------------------------------------------------------
			...
			else // Imprimimos el reporte
			{
				...
				while($row=$io_sql->fetch_row($rs_data))
					...
					$lo_hoja->write($li_row, 1, utf8_decode("L: DOCUMENTOS SIN AFECTACIÓN CONTABLE"),$lo_titulo);
									  if ($ls_estatus=='A')
					 {
					   $li_row=$li_row+1;
				  ...
				  else
					 {
					   ...
					   $li_row=$li_row+1;
					   $lo_hoja->write($li_row, 2, utf8_decode("Total Créditos"),$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
					   $lo_hoja->write($li_row, 3, $ld_totcre, $lo_dataright);
			
					   $li_row=$li_row+1;
					   $lo_hoja->write($li_row, 2, utf8_decode("Total Débitos"),$lo_libro->addformat(array('bold'=>1,'font'=>'Verdana','align'=>'right','size'=>'10')));
