...
<?php
	...
	if(($ls_operacion!="") && (strcmp($ls_cuenta_banco,$ls_cuenta_bancodestino)== 0))
	{
	...
	{			
		...
		else
		{
			...
			$msg->message("Error en transacci�n".",".$in_classtrans->is_msg_error);	
		}
	...
	if($ls_operacion == "VERIFICAR_VAUCHER")
	{
		...
		if($lb_existe)
		{
			$msg->message("Nro de Voucher ya existe, favor indicar otro");
			$ls_chevau="";
		}
		else
		{
  ...
  <table width="722" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td><div align="right">
        <?php if($ls_opeorigen=="CH")
								{
									print "Monto Objeto a Retenci�n";
								}
								?>
       </div></td>
      <td><div align="left"> 
    ...
    <tr>
      ...
      <td style="text-align:left"><select name="cmboperacionorigen" id="cmboperacionorigen" style="width:110px;text-align:center" onChange="javascript:uf_verificar_operacion();" <?php echo $ls_disabled; ?>>
          ...
          <option value="ND" <?php print $lb_selND;?>>Nota de Debito</option>
        </select>      </td>
      <td><div align="right">
        <?php if($ls_opeorigen=="CH")
								{
...
function uf_verificar_operacion()
{
		...
		if((ls_codbanorigen!=ls_codbandestino)&&(li_status==0))
		{
			ls_opedestino = 'DP';
			f.cmboperaciondestino.value = ls_opedestino;
			f.operaciondestino.value = 'Dep�sito';
		...
		else
		{
			ls_opedestino = 'NC';
			f.cmboperaciondestino.value = ls_opedestino;
			f.operaciondestino.value = 'Nota de Cr�dito';
