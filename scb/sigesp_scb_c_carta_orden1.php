...
function uf_buscar_dt_cxpspg($as_numsol)
{
	...
	//////////////////////////////////////////////////////////////////////////////
		...
		$aa_dt_cxpspg=array();
		$ls_codemp=$this->dat["codemp"];
		 $ls_sql="SELECT b.numsol, b.numdoc, b.monto as montochq, rd.codrecdoc
				 FROM cxp_sol_banco b join
                                 cxp_dt_solicitudes dt on b.numsol=dt.numsol and b.codemp=dt.codemp
                                 join cxp_rd rd on dt.numrecdoc=rd.numrecdoc and b.codemp=rd.codemp
                                 and dt.codemp=rd.codemp
				 WHERE b.codemp='".$ls_codemp."' AND b.numsol ='".$as_numsol."' AND
		...
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$this->io_sql_aux->free_result($rs_dt_spgchq);
                      ...
                      //  echo 'raquel ';var_dump(array_key_exists("codestpro1",$this->ds_temp->data));die();

			if(array_key_exists("codestpro1",$this->ds_temp->data))
			{
                               // var_dump( $this->ds_temp->getRowCount("codestpro1"));
				if($this->ds_temp->getRowCount("codestpro1")>0)
				{
					...
					$this->ds_temp->group_by($arr_group,array('0'=>"monto"),$arr_group);
                     ...
                     //  die();
		$li_row=0;
                //busco si el documento tiene ya pagado
		...
		{
                       //var_dump( $row=$this->io_sql->fetch_row($rs_data_dtcxpspg));
                       //si el query anterior no trae nada es porque no ha pagado fraccionado, entonces se buscan los comprobantes
                       //que tienen el monto total causado
                       if (!$row=$this->io_sql->fetch_row($rs_data_dtcxpspg))
                       {
                                $ls_sql="SELECT codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,spg_cuenta,sum(monto) as monto ,descripcion,estcla ".
				"	FROM spg_dt_cmp ".
				" WHERE codemp='".$ls_codemp."' ".
				"   AND procede='CXPRCD' ".
				"   AND comprobante='".$codrecdoc."'".
				" GROUP BY codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,spg_cuenta,descripcion,estcla ".
				" UNION ".
				"SELECT codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,spg_cuenta,sum(spg_dt_cmp.monto) as monto ,descripcion,estcla ".
				"	FROM spg_dt_cmp, cxp_dt_solicitudes, cxp_sol_dc ".
