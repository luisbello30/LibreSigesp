...
if ($ls_operacion=="VERIFICAR_VAUCHER")
   {
	 ...
	 if ($lb_existe)
		{
		  $io_msg->message("Num de Voucher ya existe, favor indicar otro");	
		  $ls_chevau="";		
		}
					    ...
					    if ($lb_valido)
					       {
							...
							if ($ls_estretiva=='B')//Retenciones aplicadas desde el M�dulo de Cuentas Por Pagar y reflejadas en el M�dulo Banco.
							 ...
							 elseif($ls_estretiva=='C')
							 {
								  if ($ls_modageret=="B")/// se realiza el calculo de la ret. municipal
							      {
									$ldec_montotot=(round($ldec_montomov,2))-(round($ld_montotret,2));
								  }
								  else
								  {
								  	$ldec_montotot=$ldec_montomov;
  ...
  <table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td height="22" style="text-align:left">
        <input name="txtdocumento" type="text" id="txtdocumento" value="<?php print $ls_numdoc;?>" size="24" maxlength="15" onBlur="javascript:rellenar_cad(this.value,15,'doc');" style="text-align:center" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz '+'-()[]{}#/');" <?php print $ls_lectura; echo $ls_disabled;?> >
        <?php
...
function ue_guardar()
{
  ...
  if (lb_mesabi=='true')
     ...
     {
	   alert("Operaci�n No puede ser procesada, El Mes est� Cerrado !!!");
	 }
}
