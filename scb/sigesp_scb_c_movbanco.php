...
function uf_insert_movimiento($ls_codemp,$ls_codusu,$ls_codban,$ls_ctaban,$ls_numdoc,$ls_codope,$ldt_fecha,$ls_conmov,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_nomproben,$ldec_monto,$ldec_monobjret,$ldec_monret,$ls_chevau,$ls_estmov,$li_estmovint,$li_cobrapaga,$ls_estbpd,$ls_procede,$ls_estreglib,$ls_tipproben,$as_numordpagmin,$as_codtipfon,$li_estmovcob,&$as_numcontint,$as_reglibro)
{
	...
	{
		$as_numcontintaux=$as_numcontint;//raq1
		 ...
		 $ls_sql="INSERT INTO scb_movbco(codemp,codusu,codban,ctaban,numdoc,codope,fecmov,conmov,codconmov,cod_pro,ced_bene,nomproben,
				 ...
				 VALUES                ('".$ls_codemp."','".$ls_codusu."','".$ls_codban."','".$ls_ctaban."','".$ls_numdoc."',
										...
										'".$ls_chevau."','".$ls_estmov."',".$li_estmovint.",".$li_cobrapaga.", 0    ,'".$ls_estbpd."',
											0  ,'1900-01-01','".$ls_estreglib."','".$ls_tipproben."','1900-01-01','SCBBCH','--','".$as_numordpagmin."','".$as_codtipfon."','".$li_estmovcob."','".$as_reglibro."')";  
	}
	else
	{
		$ls_sql="INSERT INTO scb_movbco(codemp,codusu,codban,ctaban,numdoc,codope,fecmov,conmov,codconmov,cod_pro,ced_bene,nomproben,
				 ...
				 VALUES                ('".$ls_codemp."','".$ls_codusu."','".$ls_codban."','".$ls_ctaban."','".$ls_numdoc."',
										...
										'".$ls_cedbene."','".$ls_nomproben."',".$ldec_monto.",".$ldec_monobjret.",".$ldec_monret.",
										'".$ls_chevau."','".$ls_estmov."',".$li_estmovint.",".$li_cobrapaga.", 0    ,'".$ls_estbpd."',
											0  ,'1900-01-01','".$ls_estreglib."','".$ls_tipproben."','1900-01-01','SCBBCH','--','".$as_numordpagmin."','".$as_codtipfon."','".$li_estmovcob."','".$as_numcontint."','".$as_reglibro."')";										
	}
...
function uf_cargar_dt($as_numdoc,$as_codban,$as_ctaban,$as_codope,$ls_estmov,$objectScg,$li_row_scg,$ldec_mondeb,$ldec_monhab,$objectSpg,$li_temp_spg,$ldec_monto_spg,$objectSpi,$li_temp_spi,$ldec_monto_spi)
{
		...
		if ($rs_data===false)
		   {
			 $this->is_msg_error="Error en inserci�n, ".$this->fun->uf_convertirmsg($this->io_sql->message);
			 $lb_valido=false;
		   }
		else
		   {
...
function uf_delete_all_movimiento($ls_numdoc,$ls_codban,$ls_ctaban,$ls_codope,$ls_estmov)
{
	...
	if($lb_valido)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_delete_all_movimiento->Error al eliminar detalle de movimiento".$this->fun->uf_convertirmsg($this->io_sql->message);
			print $this->io_sql->message;
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_valido)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_delete_anulado->Error al eliminar detalle de movimiento".$this->fun->uf_convertirmsg($this->io_sql->message);
			print $this->io_sql->message;
			if($this->io_sql->errno==1451)
			{
				$this->is_msg_error="No se pudo eliminar el Movimiento,posee relaciones";	
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($lb_valido)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_actualiza_documento->Error al modificar el numero de documento".$this->fun->uf_convertirmsg($this->io_sql->message);
			print $this->io_sql->message;
			if($this->io_sql->errno==1451)
			{
		...
		else
		{
			...
			{
				$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_actualiza_documento FUEFIN->Error al modificar el numero de documento".$this->fun->uf_convertirmsg($this->io_sql->message);	
				$lb_valido=false;
			}
			...
			if ($li_rsscg==false)
			{
				$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_actualiza_documento SCG->Error al modificar el numero de documento".$this->fun->uf_convertirmsg($this->io_sql->message);	
				$lb_valido=false;
			}
			...
			if ($li_rsspg==false)
			{
				$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_actualiza_documento SPG->Error al modificar el numero de documento".$this->fun->uf_convertirmsg($this->io_sql->message);	
				$lb_valido=false;
			}
			...
			if ($li_rsspi==false)
			{
				$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_actualiza_documento SPI->Error al modificar el numero de documento".$this->fun->uf_convertirmsg($this->io_sql->message);	
				$lb_valido=false;
			}
			...
			if ($li_rsmov==false)
			{
				$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_actualiza_documento MOVBCO->Error al modificar el numero de documento".$this->fun->uf_convertirmsg($this->io_sql->message);	
				$lb_valido=false;
			}
...
function uf_delete_all_dtmov($ls_codemp,$ls_numdoc,$ls_codban,$ls_ctaban,$ls_codope,$ls_estmov)
{
	...
	{
		$lb_valido=false;
		$this->is_msg_error="CLASS->sigesp_scb_c_movbanco.php.M�todo->uf_delete_all_dtmov->Error al eliminar detalle de movimiento".$this->fun->un_convertirmsg($this->io_sql->message);
	}
	else
	{
		$lb_valido=true;
...
//	   Description: Funci�n que busca y actualiza monto con su correspondiente en Bs.F.
//	    Creado Por: Ing. Nestor Falc�n.
//  Fecha Creaci�n: 15/08/2007 								Fecha �ltima Modificaci�n : 15/08/2007
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  return $lb_valido;
	...
	function uf_delete_fuentefinancimiento($as_codemp,$as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov)
	{
		...
		if($li_numrow===false)
		{
 			$lb_valido=false;
			print $this->io_sql->message;
			$this->msg->message("CLASE->Movimiento de Banco M�TODO->uf_delete_fuentefinancimiento ERROR->".$this->fun->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
		...
		else
		{
			...
			else
			{
				...
				if($li_numrow===false)
				{
					$lb_valido=false;
					print $this->io_sql->message;
					$this->msg->message("CLASE->Movimiento de Banco M�TODO->uf_check_insert_fuentefinancimiento ERROR->".$this->fun->uf_convertirmsg($this->io_sql->message));
				}
			}
		}
		return $lb_valido;
...
function uf_load_documentos_asociados($as_numordpagmin,$as_codtipfon)
{
  ...
  //		   Access: private
  //	    Arguments: 
  //	  Description: Funci�n que verifica si existen Recepciones de Documentos, Cheque o Notas de D�bitos
  //                   que utilizan un N�mero de Orden de Pago Ministerio para evitar su eliminaci�n.  
  //	   Creado Por: Ing. N�stor Falc�n
  //   Fecha Creaci�n: 19/02/2009.								Fecha �ltima Modificaci�n : 19/02/2009.
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  $lb_valido = true;
  $ls_sql    = "SELECT codemp
				  FROM scb_movbco 
  ...
  if ($rs_data===false)	
	 {
	   $this->is_msg_error="CLASS->sigesp_scb_c_movbco.php;M�todo->uf_load_documentos_asociados.".$this->fun->uf_convertirmsg($this->io_sql->message);
	   $lb_valido = false;
	 }
  else
	 {
...
function uf_select_pagados($as_numsol,$as_numdoc,$as_codban,$as_ctaban)
{
		...
		// Fecha Creaci�n: 15/03/2010. 								Fecha �ltima Modificaci�n : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$ls_codemp=$_SESSION["la_empresa"]["codemp"];
   	$li_pagado=0;
